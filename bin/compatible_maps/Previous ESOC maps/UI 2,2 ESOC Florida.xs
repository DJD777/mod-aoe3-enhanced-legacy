...
//	TRvariation = 1; // <--- TEST
	...
	int avoidDeerFar = rmCreateTypeDistanceConstraint("avoid Deer far", "Deer", 62.0);
	int avoidDeer = rmCreateTypeDistanceConstraint("avoid  Deer", "Deer", 36.0);
...
//	int avoidGoldVeryFar = rmCreateClassDistanceConstraint ("gold avoid gold very ", rmClassID("Gold"), 74.0);
	...
	if (isObserverMode == true)
	{
		...
		{ //1v1
			rmSetRandomSpawn1v1(0.80,0.50,  0.20,0.50);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3))
		{ // 2v2

			if (TRvariation == 1)
				rmSetRandomSpawn2v2(0.74,0.68,  0.78,0.48,  0.22,0.48,  0.26,0.68);
			else if (TRvariation == 2)
				rmSetRandomSpawn2v2(0.78,0.48,  0.82,0.70,  0.22,0.48,  0.18,0.70);
			else if (TRvariation == 3)
				rmSetRandomSpawn2v2(0.84,0.40,  0.76,0.60,  0.16,0.40,  0.24,0.60);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
		{ // 3v3
			if (TRvariation == 1)
				rmSetRandomSpawn3v3(0.74,0.70,  0.80,0.37,  0.77,0.53,  0.20,0.37,  0.26,0.70,  0.23,0.53);
			else if (TRvariation == 2)
				rmSetRandomSpawn3v3(0.81,0.58,  0.76,0.44,  0.86,0.72,  0.19,0.58,  0.24,0.44,  0.14,0.72);
			else if (TRvariation == 3)
				rmSetRandomSpawn3v3(0.80,0.55,  0.76,0.72,  0.84,0.38,  0.24,0.72,  0.20,0.55,  0.16,0.38);
		}
	}
	else
	{
...
//	rmSetAreaEdgeFilling(swampAreaID, 0.5);
	...
	for (i=0; < islandcount)
	{
		...
		rmSetAreaSize(islandID, rmAreaTilesToFraction(260), rmAreaTilesToFraction(300+5*PlayerNum));
		if (isWinterSeason == true) {
...
//	rmAddObjectDefToClass(playergold2ID, classStartingResource);
	...
	if (isWinterSeason == true) {
		rmAddObjectDefItem(playerTreeID, "TreeGreatLakesSnow", rmRandInt(1,1), 2.0);
	} else {
	...
	rmSetObjectDefMinDistance(playerherdID, 18.0);
...
//		rmPlaceObjectDefAtLoc(playergold2ID, i, rmXMetersToFraction(xsVectorGetX(TCLoc)), rmZMetersToFraction(xsVectorGetZ(TCLoc)));
	...
	// Silver mines
	int silverminecount = 5+2*PlayerNum; 
	...
	// Tin mines
	int tinminecount = 1+1*PlayerNum; 
