...
void main(void)
{
   ...
   rmSetStatusText("",0.01);
	...
	int size=1.9*sqrt(PlayerNum*playerTiles);
...
// ---------------------------------------------- TradeRoute and Cliffs -----------------------------------------------
   ...
   int cliffID1 = rmCreateArea("cliff middle");
...
// ------------------------------------ Design : middle cliffs  ------------------------------------------
   ...
   rmSetAreaCliffHeight(cliffIDB1, 6.0, 0.0, 0.0);
   ...
   rmSetAreaCliffHeight(cliffIDT1, 6.0, 0.0, 0.0);
...
// -------------------------------------------------------------- Let's place some design (oasis and forest ) ---------------------------------------------
 ...
 // ----------------------------------------------------- Players and blabla starting stuff -------------------------------------------------
	...
	if (isObserverMode == true)
	{
		...
		{ //1v1
			rmSetRandomSpawn1v1(0.25,0.73,  0.75,0.27);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3))
		{ // 2v2
			rmSetRandomSpawn2v2(0.84,0.46,  0.76,0.26,  0.24,0.76,  0.16,0.54);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
		{ // 3v3
			rmSetRandomSpawn3v3(0.84,0.61,  0.76,0.26,  0.85,0.42,  0.15,0.58,  0.16,0.39,  0.24,0.74);
		}
	}
	else
	{
	...
	rmSetObjectDefCreateHerd(StartBighornSheepID, true);
	rmSetObjectDefCreateHerd(StartBighornSheepID, true);
...
//   ******************************************** forests ************************************X marks the spot
   ...
   int failCont=0;
	...
	for (i=0; < northforestcount)
	{
		...
		rmSetAreaBaseHeight(YellowForest, 5.0);
		...
		for (j=0; < rmRandInt(4,5)) //18,20
		{
			...
			rmAddObjectDefItem(northtreeID, "ypTreeGinkgo", rmRandInt(1,2), 4.0); // 1,2
      ...
      rmAddObjectDefItem(northtreeID1, "UnderbrushRockies", rmRandInt(3,5), 4.0);
...
if (PlayerNum<=3)
{
	...
	rmAddObjectDefConstraint(BighornSheepID1, shortAvoidImpassableLand);
...
for (i=0;<PlayerNum*2)
    ...
    rmAddObjectDefConstraint(ElkIDT, shortAvoidImpassableLand);
...
for (i=0;<PlayerNum*2)
	...
	rmAddObjectDefConstraint(BighornSheepIDT, shortAvoidImpassableLand);
...
// -------------------------------------------------- Treasure time ---------------------------------------------------
	...
	rmPlaceObjectDefAtLoc(nuggetID1, 0, 0.5, 0.5, PlayerNum*5);
