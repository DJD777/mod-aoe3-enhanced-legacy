...
void main(void)
{
	...
	int avoidMuskdeerFar = rmCreateTypeDistanceConstraint("avoid Muskdeer far", "ypMuskdeer", 60.0);
	int avoidMuskdeer = rmCreateTypeDistanceConstraint("avoid Muskdeer", "ypMuskdeer", 44.0);
	int avoidMuskdeerShort = rmCreateTypeDistanceConstraint("avoid Muskdeer short", "ypMuskdeer", 30.0);
	int avoidMuskdeerMin = rmCreateTypeDistanceConstraint("avoid Muskdeer min", "ypMuskdeer", 10.0);
	...
	if (isObserverMode == true)
	{
		...
		{ //1v1
			rmSetRandomSpawn1v1(0.20, 0.60,  0.80, 0.40);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3))
		{ // 2v2
			rmSetRandomSpawn2v2(0.20, 0.60, .40, .90,  0.80, 0.40, .60, .10);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
		{ // 3v3
			rmSetRandomSpawn3v3(0.20, 0.60, .30, .75, .40, .90,  0.80, 0.40, .70, .25, .60, .10);
		}
	}
	else
	{
	...
	for (i=0; <6*PlayerNum)
	{
		...
		rmSetAreaBaseHeight(iceislandID, 1.5);
...
//	rmSetAreaMix(GoldZoneEID, "yellow_river_b");	// for testing
	...
	rmAddObjectDefItem(playeherdID, "ypMuskdeer", rmRandInt(8,8), 4.0);
	...
	rmAddObjectDefItem(player2ndherdID, "MuskOx", rmRandInt(8,8), 4.0);
	...
	for (j=0; < (2+1*PlayerNum)) {   
		...
		rmPlaceObjectDefAtLoc(Trees1ID, 0, 0.5, 0.5, 2+1*PlayerNum);
	}
	...
	for (j=0; < (2+1*PlayerNum)) {   
		...
		rmPlaceObjectDefAtLoc(Trees2ID, 0, 0.5, 0.5, 2+1*PlayerNum);
	}
	
	// Random Trees
	for (i=0; < 4+1*PlayerNum)
	{
		...
		rmPlaceObjectDefAtLoc(randomtreeID, 0, 0.50, 0.50, 4+1*PlayerNum);
	}
	...
	// Central Herds 
	int muskdeerherdcount = 4+2*PlayerNum;
		
	for (i=0; < muskdeerherdcount) {
		int centralherdID = rmCreateObjectDef("central herd"+i);
		rmAddObjectDefItem(centralherdID, "ypMuskdeer", rmRandInt(8,8), 6.0);
	...
	// Team 1 Herds 
	int muskdeerherd2count = PlayerNum/2;
		
	for (i=0; < muskdeerherd2count) {
		int team1herdID = rmCreateObjectDef("team 1 herd"+i);
		if (PlayerNum == 2)
			rmAddObjectDefItem(team1herdID, "ypMuskdeer", rmRandInt(8,8), 6.0);
		else
			rmAddObjectDefItem(team1herdID, "MuskOx", rmRandInt(8,8), 6.0);		
	...
	// Team 2 Herds 
	int muskdeerherd3count = PlayerNum/2;
		
	for (i=0; < muskdeerherd3count) {
		int team2herdID = rmCreateObjectDef("team 2 herd"+i);
		if (PlayerNum == 2)
			rmAddObjectDefItem(team2herdID, "ypMuskdeer", rmRandInt(8,8), 6.0);
		else
			rmAddObjectDefItem(team2herdID, "MuskOx", rmRandInt(8,8), 6.0);
