...
//	rmSetMapElevationHeightBlend(1);
	...
	rmTerrainInitialize("himalayas\ground_dirt7_himal", -6.0); //
	...
	int avoidGoldTypeMed = rmCreateTypeDistanceConstraint("coin avoids coin med", "gold", 25.0);
...
//	int avoidTownCenterMed = rmCreateTypeDistanceConstraint("avoid Town Center med", "townCenter", 40.0);
	...
	if (isObserverMode == true)
	{
		...
		{ //1v1
			rmSetRandomSpawn1v1(0.25,0.50,  0.75,0.50);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3))
		{ // 2v2
			rmSetRandomSpawn2v2(0.29,0.35,  0.29,0.65,  0.71,0.65,  0.71,0.35);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
		{ // 3v3
			rmSetRandomSpawn3v3(0.32,0.68,  0.32,0.32,  0.24,0.50,  0.76,0.50,  0.68,0.68,  0.68,0.32);
		}
	}
	else
	{
...
//	rmAddAreaInfluenceSegment(plateauID, 0.40, 0.50, 0.60, 0.50);
	...
	rmSetAreaTerrainType(plateauID, "himalayas\ground_dirt6_himal");
...
//	rmSetAreaMix(hillID, "himalayas_b");
	rmSetAreaTerrainType(hillID, "himalayas\ground_dirt8_himal");
...
//	Natsvariation = rmRandFloat(0.0,1.0); // > 0.66 x2 udasi, > 0.33 x2 bhakti, else 1 udasi and 1 bhakti
	...
	{
		nativeID0 = rmCreateGrouping("Udasi village A", "native udasi village "+1);
		nativeID1 = rmCreateGrouping("Udasi village B", "native udasi village "+1);
	}
	else if (Natsvariation > 0.33)
	{
		nativeID0 = rmCreateGrouping("Shaolin temple A", "native shaolin temple mongol 0"+1);
		nativeID1 = rmCreateGrouping("Shaolin temple A", "native shaolin temple mongol 0"+1);
	}
	else
	{
		nativeID0 = rmCreateGrouping("Udasi village A", "native udasi village "+1);
...
//			rmPlaceObjectDefAtLoc(playerNuggetID, i, rmXMetersToFraction(xsVectorGetX(TCLoc)), rmZMetersToFraction(xsVectorGetZ(TCLoc)));
	...
	int sidegoldcount = 4+1*PlayerNum;
	int plateaugoldcount = 2+1*PlayerNum;
	...
	// Lowground forest
	int forestlowcount = 14+3*PlayerNum;
...
//		rmAddAreaConstraint(forestlowID, avoidEdge);
	...
	int highgroundherdcount = 1+2*PlayerNum;
	int lowgroundherdcount = 3+2*PlayerNum;
...
//			rmAddObjectDefConstraint(lowgroundherdID, stayNE);
	...
	int treasure2count = 3+0.5*PlayerNum;
