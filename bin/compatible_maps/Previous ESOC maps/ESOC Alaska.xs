...
//	rmSetMapElevationHeightBlend(1);
	...
	int stayMiddle = rmCreateBoxConstraint("stay in the middle", 0.34-0.01*PlayerNum, 0.00, 0.66+0.01*PlayerNum, 1.00, 0.00);	
	...
	int avoidElk = rmCreateTypeDistanceConstraint("avoid Elk", "Elk", 42.0);
	int avoidElkShort = rmCreateTypeDistanceConstraint("avoid Elk short", "Elk", 35.0);
	int avoidElkMin = rmCreateTypeDistanceConstraint("avoid Elk min", "Elk", 4.0);
	...
	int avoidGold = rmCreateClassDistanceConstraint ("gold avoid gold med", rmClassID("Gold"), 40.0+0.67*PlayerNum);
	int avoidGoldFar = rmCreateClassDistanceConstraint ("gold avoid gold far", rmClassID("Gold"), 52.0+1*PlayerNum);
	...
	int avoidCliffFar = rmCreateClassDistanceConstraint("avoid cliff far", rmClassID("Cliffs"), 16+2*PlayerNum);
	...
	if (isObserverMode == true)
	{
		...
		{ //1v1
			rmSetRandomSpawn1v1(0.78, 0.45,  0.22, 0.45);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3))
		{ // 2v2
			rmSetRandomSpawn2v2(0.18, 0.35, 0.28, 0.62,  0.82, 0.35, 0.72, 0.62);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
		{ // 3v3
			rmSetRandomSpawn3v3(0.20, 0.32, 0.30, 0.65,  0.25,0.48,  0.80, 0.32, 0.70, 0.65,  0.75,0.48);
		}
	}
	else
	{
...
//	rmSetAreaWaterType(bayID, "araucania southern Coast"); 
...
//	rmSetAreaMaxBlobDistance(bayID, 20);
	...
	rmSetAreaSmoothDistance(bayID, 4+2*PlayerNum);
	rmBuildArea(bayID);
	...
	rmSetAreaMix(cornertopID, "nwt_grass1");
	...
	rmSetAreaSmoothDistance(cornertopID, 8+2*PlayerNum);
	...
	rmSetAreaMix(cornerbottomID, "nwt_grass1");
	...
	rmSetAreaSmoothDistance(cornerbottomID, 8+2*PlayerNum);
	...
	rmSetAreaSmoothDistance(landID, 8+2*PlayerNum);
...
//	rmAddAreaInfluenceSegment(spurtopID, 0.50, 0.08, 0.50, 0.40);
	rmSetAreaMix(spurtopID, "nwt_grass1");
//	rmSetAreaTerrainType(spurtopID, "new_england\ground2_cliff_ne"); // for testing
	...
	rmSetAreaSmoothDistance(spurtopID, 6+2*PlayerNum);
...
//	rmAddAreaInfluenceSegment(spurbottomID, 0.50, 0.08, 0.50, 0.40);
	rmSetAreaMix(spurbottomID, "nwt_grass1");
//	rmSetAreaTerrainType(spurbottomID, "new_england\ground2_cliff_ne"); // for testing
	...
	rmSetAreaSmoothDistance(spurbottomID, 6+2*PlayerNum);
	...
	for (i=0; < 2)
	{
		...
		if (i == 0)
		{
			...
			rmAddAreaTerrainLayer(cliffID, "araucania\ground_dirt3_ara", 0, 3+0.34*PlayerNum);
			rmAddAreaTerrainLayer(cliffID, "araucania\ground_snow1_ara", 3+0.34*PlayerNum, 5+0.34*PlayerNum);
			rmAddAreaTerrainLayer(cliffID, "araucania\ground_snow2_ara", 5+0.34*PlayerNum, 6+0.34*PlayerNum);
			rmAddAreaTerrainLayer(cliffID, "araucania\ground_snow3_ara", 6+0.34*PlayerNum, 8+0.34*PlayerNum);
		...
		else
		{
			...
			rmSetAreaTerrainType(cliffID, "araucania\ground_snow5_ara");
	...
	for (i=0; < 2)
	{
		...
		if (i == 0)
		{
			...
			rmAddAreaTerrainLayer(cliff2ID, "araucania\ground_dirt3_ara", 0, 3+0.34*PlayerNum);
			rmAddAreaTerrainLayer(cliff2ID, "araucania\ground_snow1_ara", 3+0.34*PlayerNum, 5+0.34*PlayerNum);
			rmAddAreaTerrainLayer(cliff2ID, "araucania\ground_snow2_ara", 5+0.34*PlayerNum, 6+0.34*PlayerNum);
			rmAddAreaTerrainLayer(cliff2ID, "araucania\ground_snow3_ara", 6+0.34*PlayerNum, 8+0.34*PlayerNum);
		...
		else
		{
			...
			rmSetAreaTerrainType(cliff2ID, "araucania\ground_snow5_ara");
	...
	for (i=0; < 2)
	{
		...
		if (i == 0)
		{
			...
			rmAddAreaTerrainLayer(cliff3ID, "araucania\ground_dirt3_ara", 0, 3+0.34*PlayerNum);
			rmAddAreaTerrainLayer(cliff3ID, "araucania\ground_snow1_ara", 3+0.34*PlayerNum, 5+0.34*PlayerNum);
			rmAddAreaTerrainLayer(cliff3ID, "araucania\ground_snow2_ara", 5+0.34*PlayerNum, 6+0.34*PlayerNum);
			rmAddAreaTerrainLayer(cliff3ID, "araucania\ground_snow3_ara", 6+0.34*PlayerNum, 8+0.34*PlayerNum);
		...
		else
		{
			...
			rmSetAreaTerrainType(cliff3ID, "araucania\ground_snow5_ara");
	...
	for (i=0; < 10+10*PlayerNum)
	{
		...
		rmSetAreaTerrainType(patchID, "NWterritory\ground_grass1a_nwt");
...
//	rmAddObjectDefConstraint(player2ndherdID, avoidTradeRouteShort);
	...
	rmAddObjectDefItem(player3rdherdID, "Moose", rmRandInt(5,5), 4.0);
...
//			rmPlaceObjectDefAtLoc(playerNuggetID, i, rmXMetersToFraction(xsVectorGetX(TCLoc)), rmZMetersToFraction(xsVectorGetZ(TCLoc)));
	...
	int goldcount = 1*PlayerNum;  
	if (PlayerNum >= 4)
		goldcount = PlayerNum-2-PlayerNum%2;
	int silvercount = 1+3*PlayerNum;
...
//		rmAddObjectDefConstraint(goldID, avoidTownCenterVeryFar);
	...
	// Mountain forest
	int mountainforestcount = 2*PlayerNum;
...
//		rmAddAreaConstraint(mforestID, avoidEdge);
	...
	// Valley forest
	int valleyforestcount = 4+6*PlayerNum;
	...
	for (i=1; < valleyforestcount+1)
	{
		...
		rmSetAreaTerrainType(vforestID, "NWterritory\ground_grass5_nwt");
...
//		rmAddAreaConstraint(vforestID, avoidEdge);
	...
	for (i=0; < berriescount)
	{
		...
		rmSetAreaTerrainType(berriesareaID, "NWterritory\ground_shoreline3_nwt");
...
//		}
	...
	int elkcount = 1+2*PlayerNum;
	int moosecount = 1+2*PlayerNum;
	...
	for (i=0; < elkcount)
	{
		...
		rmAddObjectDefItem(elkherdID, "Elk", rmRandInt(8,8), 6.0);
	...
	for (i=0; < moosecount)
	{
		...
		rmAddObjectDefItem(mooseherdID, "moose", rmRandInt(5,5), 4.0);
...
//			rmAddObjectDefConstraint(mooseherdID, stayNE);
	...
	int treasure1count = 2+0.5*PlayerNum;
	int treasure2count = 5+0.5*PlayerNum;
	int treasure3count = 0.5*PlayerNum;
	int treasure4count = 0.34*PlayerNum;
...
//		rmPlaceObjectDefAtLoc(Nugget1ID, 0, 0.50, 0.50);
	...
	int fishcount = 4+3*PlayerNum;
	int whalecount = 2+1.5*PlayerNum;
	...
	for (i=0; < fishcount)
	{
		...
		rmAddObjectDefItem(fishID, "FishSalmon", rmRandInt(3,3), 13.0);
