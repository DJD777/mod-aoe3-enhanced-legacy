...
//	rmSetMapElevationHeightBlend(1);
	...
	rmSetBaseTerrainMix("mongolia_forest"); // 
	...
	int avoidSerowFar = rmCreateTypeDistanceConstraint("avoid Serow far", "ypSerow", 48.0);
	int avoidSerow = rmCreateTypeDistanceConstraint("avoid Serow", "ypSerow", 45.0);
	...
	if (isObserverMode == true)
	{
		...
		{ //1v1
			rmSetRandomSpawn1v1(0.18,0.48,  0.82,0.48);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3))
		{ // 2v2
			rmSetRandomSpawn2v2(0.13,0.42,  0.23,0.66,  0.87,0.42,  0.77,0.66);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
		{ // 3v3
			rmSetRandomSpawn3v3(0.13,0.38,  0.19,0.55,  0.25,0.72,  0.87,0.38,  0.81,0.55,  0.75,0.72);
		}
	}
	else
	{
...
//	rmSetAreaMix(highgroundID, "rockies_snow");
	...
	rmSetAreaWaterType(SeaID, "coastal japan"); 
...
//	rmSetAreaMaxBlobDistance(SeaID, 20);
    ...
    rmSetAreaSize(spurID, 0.07, 0.07);
	rmSetAreaBaseHeight(spurID, 2.5); 
	rmSetAreaElevationType(spurID, cElevTurbulence);
	rmSetAreaElevationVariation(spurID, 4.5);
	rmSetAreaElevationMinFrequency(spurID, 0.05);
	rmSetAreaElevationOctaves(spurID, 2);
	rmSetAreaElevationPersistence(spurID, 0.5);
	rmSetAreaLocation(spurID, 0.5, 0.33);
	rmSetAreaTerrainType(spurID, "mongolia\ground_grass4_mongol");
    rmSetAreaCoherence(spurID, 0.65);
	rmSetAreaSmoothDistance(spurID, 16);
    rmBuildArea(spurID); 
	...
	for (i=0; < 10*PlayerNum)
	{
		...
		rmSetAreaTerrainType(patchID, "coastal_japan\ground_grass1_co_japan");
...
//			rmAddAreaConstraint(patchID, stayCloseSea);
	...
	if (i == 0)
	{
		...
		rmSetAreaMix(hillID, "mongolia_forest");	
...
//	rmSetAreaTerrainType(playerareaID, "new_england\ground2_cliff_ne"); // for testing
	...
	if (rmGetIsKOTH()) {
		...
		if (randLoc == 2) {
			...
			xLoc = 0.5;
			yLoc = 0.1;
			ypKingsHillLandfill(xLoc, yLoc, 0.007, 1.0, "mongolia_forest", 0);
		}
...
*/
	...
	rmSetObjectDefMinDistance(playeherdID, 16.0);
	...
	rmAddObjectDefItem(player2ndherdID, "ypSerow", rmRandInt(10,10), 7.0);
...
/*	// 3rd herd
	...
	rmAddObjectDefItem(player3rdherdID, "ypSerow", rmRandInt(7,7), 6.0);
    rmSetObjectDefMinDistance(player3rdherdID, 50);
    rmSetObjectDefMaxDistance(player3rdherdID, 52);
...
/*		
	...
	if (goldswitch == 1)
		...
		minevariation = 6;	
*/	
	int sidegoldcount = 1*PlayerNum;  
	if (minevariation == 1 || minevariation == 4 || minevariation == 6)
		sidegoldcount = 1*TeamNum; 
		
	int northgoldcount = 1+0.35*PlayerNum;
	int southgoldcount = 1+0.35*PlayerNum;
	if (minevariation == 7)
	{
		northgoldcount = 1*PlayerNum;
		southgoldcount = 1*PlayerNum;
	}
	...
	// Lowground forest
	int forestlowcount = 6*PlayerNum;
	if (PlayerNum >= 4)
		forestlowcount = 5*PlayerNum;
	if (PlayerNum >= 6)
		forestlowcount = 4*PlayerNum;
	...
	for (i=0; < forestlowcount)
	{
		...
		rmSetAreaTerrainType(forestlowID, "mongolia\ground_grass3_mongol");
...
//		rmAddAreaConstraint(forestlowID, avoidEdge);
	...
	// Highground forest
	int foresthighcount = 2*PlayerNum;
	...
	for (i=0; < foresthighcount)
	{
		...
		rmSetAreaTerrainType(foresthighID, "mongolia\ground_grass3_mongol");
...
//		rmAddAreaConstraint(foresthighID, avoidEdge);
	...
	int treelowcount = 4*PlayerNum;
	if (PlayerNum >= 4)
		treelowcount = 3*PlayerNum;
	if (PlayerNum >= 6)
		treelowcount = 2.5*PlayerNum;
	...
	// Random trees in the highground
		int treehighcount = 2+4*PlayerNum;
	if (PlayerNum >= 4)
		treehighcount = 3*PlayerNum;
	if (PlayerNum >= 6)
		treehighcount = 2.5*PlayerNum;
	...
	int lowgroundherdcount = 3.5*PlayerNum;
	int highgroundherdcount = 1+PlayerNum;
	...
	for (i=0; < lowgroundherdcount)
	{
		...
		rmAddObjectDefItem(lowgroundherdID, "ypSerow", rmRandInt(7,7), 7.0);
...
//		rmAddObjectDefConstraint(lowgroundherdID, avoidTradeRouteShort);
	...
	for (i=0; < highgroundherdcount)
	{
		...
		rmAddObjectDefItem(highgroundherdID, "ypMuskdeer", rmRandInt(8,8), 7.0);
...
//			rmAddObjectDefConstraint(highgroundherdID, stayNE);
	...
	int treasure1count = 4+0.5*PlayerNum;
	int treasure2count = 4+0.5*PlayerNum;
	int treasure3count = 1+0.5*PlayerNum;
	int treasure4count = 0.5*PlayerNum;
