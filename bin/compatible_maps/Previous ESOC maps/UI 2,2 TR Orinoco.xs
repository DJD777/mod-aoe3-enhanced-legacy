...
// Dec 06 - YP update
//***************************************
//dicktator_ edits logged here
//***************************************
//Inca in base/Inca outside of base - locked
//Players always spawn top - river always bot
//Fixed TC positions
//The river shallows have been replaced with consistently buildable land
//1v1 is locked to one crossing only - 2v2 will either spawn with one large crossing or two smaller crossings - 3v3  is locked to two crossings (one large one smaller)
//Two starting mines plus three additional mines for each player
//More trees
//Larger starting hunt + three additional hunts for each player
//520 coin treasure removed
...
void main(void)
{
   ...
   if (rmAllocateSubCivs(4) == true)
   {
		...
		{ //this is a mixed range and melee map
			...
			{
				rangedCiv = "Tupi"; //caribs
			}
			if(rmRandFloat(0,1) > 0.50)
			{ //Zapotec or Maya
				string meleeCiv = "Maya"; //zapotec
			}
			else
			{
				meleeCiv = "Maya"; //Maya
   ...
   // Picks the map size
	...
	int size=2.1*sqrt(PlayerNum*playerTiles);
   ...
   //int playerEdgeConstraint=rmCreateBoxConstraint("player edge of map", rmXTilesToFraction(6), rmZTilesToFraction(6), 1.0-rmXTilesToFraction(6), 1.0-rmZTilesToFraction(6), 0.01);
	...
	//int huntCrossingConstraint = rmCreateBoxConstraint("hunts not in crossing", 0.5, 0.6, 0.85, 0.75, 0);
   ...
   // These objects are all defined so they can be placed later
	...
	float topOrBottom = 1;
	//We are only concerned about top now
	...
	if (isObserverMode == true) {
		...
		if(PlayerNum == 2)
		{
			rmPlacePlayer(1, 0.79, 0.44);
			rmPlacePlayer(2, 0.44, 0.79);
			rmPlacePlayer(3, 0.225, 0.425);
			rmPlacePlayer(4, 0.425, 0.225);
			rmPlacePlayer(5, 0.8, 0.2);
			rmPlacePlayer(6, 0.2, 0.8);
		}
		else if(PlayerNum == 4)
		{
			rmPlacePlayer(1, 0.74, 0.49);
			rmPlacePlayer(2, 0.89, 0.49);
			rmPlacePlayer(3, 0.49, 0.74);
			rmPlacePlayer(4, 0.49, 0.89);
			rmPlacePlayer(5, 0.15, 0.425);
			rmPlacePlayer(6, 0.425, 0.15);
			rmPlacePlayer(7, 0.8, 0.2);
			rmPlacePlayer(8, 0.2, 0.8);
			/*rmSetPlacementTeam(0);
			rmPlacePlayersLine(0.74,0.49,0.89,0.49,0,0);
			rmSetPlacementTeam(1);
			rmPlacePlayersLine(0.49,0.74,0.49,0.89,0,0);*/
		}
		else if(PlayerNum == 6)
		{
			rmPlacePlayer(1, 0.70, 0.51);
			rmPlacePlayer(2, 0.815, 0.51);
			rmPlacePlayer(3, 0.93, 0.51);
			rmPlacePlayer(4, 0.51, 0.70);
			rmPlacePlayer(5, 0.51, 0.815);
			rmPlacePlayer(6, 0.51, 0.93);
			rmPlacePlayer(7, 0.15, 0.425);
			rmPlacePlayer(8, 0.425, 0.15);
			/*rmSetPlacementTeam(0);
			rmPlacePlayersLine(0.70,0.51,0.93,0.51,0,0);
			rmSetPlacementTeam(1);
			rmPlacePlayersLine(0.51,0.70,0.51,0.93,0,0);*/
		}
	} else {
   ...
   rmSetStatusText("",0.30);
	...
	// Middle River
	int riverID = rmRiverCreate(-1, "Amazon River", 5, 0, 6, 9);
	//rmRiverSetShallowRadius(riverID, 16+PlayerNum);
	
	int split = rmRandInt(1, 2);
	...
	if(PlayerNum == 2)
	{
		...
		rmRiverAddWaypoint(riverID, 0.7, 0.7);
		rmRiverAddWaypoint(riverID, 1, 1);
	}
	...
	if(PlayerNum == 4)
	{
		...
		if(split == 1)
		{
			...
			rmRiverAddWaypoint(riverID, 0.75, 0.75);
			rmRiverAddWaypoint(riverID, 1, 1);
		}
	}
	...
	if(PlayerNum == 6)
	{
		...
		rmRiverAddWaypoint(riverID, 0.53, 0.53);
		rmRiverAddWaypoint(riverID, 0.75, 0.75);
		rmRiverAddWaypoint(riverID, 1, 1);
		//rmRiverAddWaypoint(riverID, 0.5, 0.5);
		//rmRiverAddWaypoint(riverID, 0.75, 0.75);
	...
	int riverID2 = rmRiverCreate(-1, "Amazon River", 5, 0, 6, 9);

	//1v1 Cross
	if(PlayerNum == 2)
	{
		rmRiverAddWaypoint(riverID2, 0, 0);
		rmRiverAddWaypoint(riverID2, 0.1, 0.1);
		rmRiverAddWaypoint(riverID2, 0.2, 0.2);
	}
	...
	{
		rmRiverAddWaypoint(riverID2, 0, 0);
		rmRiverAddWaypoint(riverID2, 0.075, 0.075);
		rmRiverAddWaypoint(riverID2, 0.175, 0.175);
	}
	
	rmRiverSetBankNoiseParams(riverID2, 0.07, 2, 1.5, 10.0, 0.667, 2.0);
	rmRiverBuild(riverID2);
	
	
		if(split == 1)
		{
			int riverID3 = rmRiverCreate(-1, "Amazon River", 5, 0, 6, 9);
			if(PlayerNum == 4)
			{
				rmRiverAddWaypoint(riverID3, 0.347, 0.347);
				rmRiverAddWaypoint(riverID3, 0.35, 0.35);
				rmRiverAddWaypoint(riverID3, 0.353, 0.353);
			}
			
			if(PlayerNum == 6)
			{
				rmRiverAddWaypoint(riverID3, 0.312, 0.312);
				rmRiverAddWaypoint(riverID3, 0.315, 0.315);
				rmRiverAddWaypoint(riverID3, 0.318, 0.318	);
			}
			
			rmRiverSetBankNoiseParams(riverID3, 0.07, 2, 1.5, 10.0, 0.667, 2.0);
			rmRiverBuild(riverID3);
		}
   ...
   for(i=1; <numPlayer)
   {
	...
	{
		topleftVillageID = rmCreateGrouping("Carib village Top Left", "native tupi village "+rmRandInt(1,3));
	}
	else if (subCiv0 == rmGetCivID("Zapotec"))
	{
		topleftVillageID = rmCreateGrouping("Zapotec village Top Left", "native tupi village "+rmRandInt(1,3));
	}
	else
	{
		topleftVillageID = rmCreateGrouping("Maya village Top Left", "native tupi village "+rmRandInt(1,3));
	}
	...
	rmPlaceGroupingAtLoc(topleftVillageID, 0, 0.6, 0.87);

	//Bot Left Village
	int bottomleftVillageID = -1;
	if (subCiv1 == rmGetCivID("Tupi"))
	{
		bottomleftVillageID = rmCreateGrouping("Tupi village Bottom Left", "native maya village "+rmRandInt(1,3));
	}
	else if (subCiv1 == rmGetCivID("Caribs"))
	{
		bottomleftVillageID = rmCreateGrouping("Carib village Bottom Left", "native maya village "+rmRandInt(1,3));
	}
	else if (subCiv1 == rmGetCivID("Zapotec"))
	{
		bottomleftVillageID = rmCreateGrouping("Zapotec village Bottom Left", "native maya village "+rmRandInt(1,3));
	}
	else
	{
	...
	rmPlaceGroupingAtLoc(bottomleftVillageID, 0, 0.10, 0.56);
	...
	{
		topRightVillageID = rmCreateGrouping("Carib village Top Right", "native tupi village "+rmRandInt(1,3));
	}
	else if (subCiv2 == rmGetCivID("Zapotec"))
	{
		topRightVillageID = rmCreateGrouping("Zapotec village Top Right", "native tupi village "+rmRandInt(1,3));
	}
	else
	{
		topRightVillageID = rmCreateGrouping("Maya village Top Right", "native tupi village "+rmRandInt(1,3));
	}
	...
	{
		bottomRightVillageID = rmCreateGrouping("Tupi village Bottom Right", "native maya village "+rmRandInt(1,3));
	}
	else if (subCiv3 == rmGetCivID("Caribs"))
	{
		bottomRightVillageID = rmCreateGrouping("Carib village Bottom Right", "native maya village "+rmRandInt(1,3));
	}
	else if (subCiv3 == rmGetCivID("Zapotec"))
	{
		bottomRightVillageID = rmCreateGrouping("Zapotec village Bottom Right", "native maya village "+rmRandInt(1,3));
	}
	else
	{
	...
	int playerSilverID2 = rmCreateObjectDef("player mine2");
	rmAddObjectDefItem(playerSilverID2, "mine", 1, 0);
	rmAddObjectDefConstraint(playerSilverID2, avoidTownCenter);
	rmSetObjectDefMinDistance(playerSilverID2, 20.0);
	rmSetObjectDefMaxDistance(playerSilverID2, 22.0);
    rmAddObjectDefConstraint(playerSilverID2, avoidImpassableLand);

	int playerCapybaraID=rmCreateObjectDef("player Capybara");
    rmAddObjectDefItem(playerCapybaraID, "capybara", 16, 10.0);
    ...
    rmAddObjectDefItem(playerTreeID, "TreeAmazon", 15, 8.0);
	...
	for(i=numPlayer; <=cNumberPlayers){ rmPlaceObjectDefAtLoc(TCID, i, rmPlayerLocXFraction(i), rmPlayerLocZFraction(i));}
	...
	rmPlaceObjectDefAtLoc(playerSilverID2, 0, rmXMetersToFraction(xsVectorGetX(TCLoc)), rmZMetersToFraction(xsVectorGetZ(TCLoc)));
	...
	rmPlaceObjectDefAtLoc(nuggeteasyID, 0, 0.5, 0.5, PlayerNum/2);
	...
	rmPlaceObjectDefAtLoc(nuggethardID, 0, 0.5, 0.5, PlayerNum/2);
	...
	if(rmRandFloat(0,1) < 0.50) //only places more hard nuggets 50% of the time
		...
		rmPlaceObjectDefAtLoc(nuggethard2ID, 0, 0.5, 0.5, PlayerNum/2);
		} 
		//No 500c
	...
	//if (PlayerNum > 5)
		//silverCount = silverCount - 5; WHY
		//WHY
		//ES WTF
		//YOU RUINED SO MANY OF MY BOOMS WITH THIS ONE LINE OF CODE
		
	rmEchoInfo("silver count = "+silverCount);
	
	int silverID = -1;
	
	//Three more mines (in addition to two starting) that spawn a reasonable distance from base
	for(i=0; < silverCount)
	{
	  silverID = rmCreateObjectDef("silverFair "+i);
	...
	for(i=0; < silverCount)
	{
	  silverID = rmCreateObjectDef("silverFairOtherside "+i);
	  rmAddObjectDefItem(silverID, "mine", 1, 0.0);
      rmSetObjectDefMinDistance(silverID, 0.0);
      rmSetObjectDefMaxDistance(silverID, rmXFractionToMeters(0.2));
	  rmAddObjectDefConstraint(silverID, avoidCoin);
	  ...
	  rmAddObjectDefConstraint(silverID, coinEdgeConstraint);
	  //rmAddObjectDefConstraint(silverID, eastIslandID);
	  rmPlaceObjectDefAtLoc(silverID, 0, 0.75, 0.25);
	}
	...
	rmAddObjectDefItem(tapirEastID, "tapir", 12, 13);
	rmSetObjectDefMinDistance(tapirEastID, 0.0);
	rmSetObjectDefMaxDistance(tapirEastID, rmXFractionToMeters(0.25));
	...
	rmPlaceObjectDefAtLoc(tapirEastID, 0, 0.25, 0.75);
		}
	for (i=0; <tapirCount)
		{
	int tapirWestID = rmCreateObjectDef("tapir west herd " +i);
	rmAddObjectDefItem(tapirWestID, "tapir", 12, 13);
	rmSetObjectDefMinDistance(tapirWestID, 0.0);
	rmSetObjectDefMaxDistance(tapirWestID, rmXFractionToMeters(0.25));
	...
	rmPlaceObjectDefAtLoc(tapirWestID, 0, 0.75, 0.25);
		}

	// Forest areas

	if (PlayerNum > 4)
		numTries=2*PlayerNum;
	else
		numTries=3*PlayerNum;
	failCount=0;
	for (i=0; <numTries)
		{   
			int forestID=rmCreateArea("forestID"+i, westIslandID);
			rmSetAreaWarnFailure(forestID, false);
			rmSetAreaSize(forestID, rmAreaTilesToFraction(210), rmAreaTilesToFraction(220));
	...
	for (i=0; <numTries)
		{   
			...
			rmSetAreaSize(forestEastID, rmAreaTilesToFraction(210), rmAreaTilesToFraction(220));
			rmSetAreaForestType(forestEastID, "amazon rain forest");
			rmSetAreaForestDensity(forestEastID, 0.8);
  ...
  if(rmGetIsKOTH()) {
	...
	tapirCount =4;
		for (i=0; <tapirCount)
		{
			...
			rmSetObjectDefMaxDistance(tapirRandomID, rmXFractionToMeters(0.2));
