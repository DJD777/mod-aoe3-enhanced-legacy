...
//	rmEchoInfo("subCiv0 is Huron "+subCiv0);
	...
	int avoidGoat=rmCreateTypeDistanceConstraint("avoid yak", "ypgoat", 54.0+1*PlayerNum);
	...
	if (isObserverMode == true)
	{
		...
		{ //1v1
			rmSetRandomSpawn1v1(0.11, 0.52,  0.52, 0.11);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3))
		{ // 2v2
			rmSetRandomSpawn2v2(0.11, 0.48, 0.31, 0.60,  0.48, 0.11, 0.60, 0.31);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
		{ // 3v3
			rmSetRandomSpawn3v3(0.08, 0.45, 0.35, 0.64,  0.21,0.55,  0.45, 0.08, 0.64, 0.35,  0.55,0.21);
		}
	}
	else
	{
...
//	rmSetAreaTerrainType(lowlandareaID, "texas\cliff_top_tex"); // for testing
	...
	int river1ID = rmRiverCreate(-1, "coastal japan", 18, 8, 5+0.25*PlayerNum, 5+0.25*PlayerNum); // 
	if (TeamNum <= 2)
	...
	int river2ID = rmRiverCreate(-1, "coastal japan", 18, 8, 5+0.25*PlayerNum, 5+0.25*PlayerNum); // 
	...
	int river3ID = rmRiverCreate(-1, "coastal japan", 18, 8, 5+0.25*PlayerNum, 5+0.25*PlayerNum); // 
...
//			rmAddObjectDefItem(lowlandFtreeID, "UnderbrushDeccan", rmRandInt(2,3), 5.0);
			...
			rmAddObjectDefItem(lowlandFtreeID, "ypTreeMongolianFir", rmRandInt(1,2), 2.0); // 1,2
...
//			rmAddObjectDefConstraint(highlandFtreeID, avoidGoldTypeShort);
	...
	{	
		...
		rmAddObjectDefItem(randomlowtreeID, "ypTreeMongolianFir", rmRandInt(0,1), 1.0); 
...
//		rmAddObjectDefConstraint(randomlowtreeID, avoidEdge);
	...
	{	
		...
		rmAddObjectDefItem(RandomtreeID, "ypTreeMongolianFir", rmRandInt(0,1), 2.0); // 4,5
...
//			rmAddObjectDefConstraint(muskdeerID, stayInLowlandarea);
	...
	int nugget1count = 4+1*PlayerNum;
	int nugget2count = 2+1*PlayerNum; 
