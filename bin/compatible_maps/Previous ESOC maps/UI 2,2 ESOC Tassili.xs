...
void main(void)
{
   ...
   rmSetStatusText("",0.01);
	string forestType = "z45 Arabian Desert";
	...
	int size=1.7*sqrt(PlayerNum*playerTiles);
	rmSetMapSize(size, size*1.4);
     rmSetSeaLevel(0.0);
	rmSetMapElevationParameters(cElevTurbulence, 0.08, 1, 0.25, 6.0); // nombre sur la map , détail dans une hauteur , taille , hauteur
	rmTerrainInitialize("borneo\shoreline1_boneo", 4.0);
	rmSetMapType("sonora");
	rmSetMapType("grass");
	rmSetMapType("land");
   ...
   int terrainMix = rmCreateArea("terrainMix");
   rmSetAreaLocation(terrainMix, 0.5, 0.5); 
   rmSetAreaWarnFailure(terrainMix, false);
   rmSetAreaSize(terrainMix,0.8, 0.8);
   rmSetAreaCoherence(terrainMix, 0.3);
   rmSetAreaSmoothDistance(terrainMix, 15);
   rmSetAreaMix(terrainMix, "desertforestflr_cal");
...
// ---------------------------------------------- TradeRoute ------------------------------------------------
	...
	rmAddTradeRouteWaypoint(tradeRouteID, 0.95, 0.99); // -1
	rmAddRandomTradeRouteWaypoints(tradeRouteID, 0.78, 0.85,5,5); // -1
	rmAddRandomTradeRouteWaypoints(tradeRouteID, 0.78, 0.5,5,5); // -1
	rmAddRandomTradeRouteWaypoints(tradeRouteID, 0.78, 0.15,5,5); // -6
	rmAddTradeRouteWaypoint(tradeRouteID, 0.95, 0.01); // -6
	
    int tradeRouteID1 = rmCreateTradeRoute();
	rmSetObjectDefTradeRouteID(socketID1, tradeRouteID1);
	rmAddTradeRouteWaypoint(tradeRouteID1, 0.05, 0.01); // -1	
	rmAddRandomTradeRouteWaypoints(tradeRouteID1, 0.22, 0.15,5,5); // -1
	rmAddRandomTradeRouteWaypoints(tradeRouteID1, 0.22, 0.5,5,5); // -1
	rmAddRandomTradeRouteWaypoints(tradeRouteID1, 0.22, 0.85,5,5); // -6	
	rmAddTradeRouteWaypoint(tradeRouteID1, 0.05, 0.99); // -10
...
// ------------------------------------ Design : cliffs and forests ------------------------------------------
  ...
  if (PlayerNum==2)
  {
	  ...
	  rmSetAreaForestDensity(forestID, 0.75);
  ...
  else
  {
      ...
      rmSetAreaForestType(forestIDT, forestType);
      ...
      rmSetAreaForestType(forestID1T, forestType);
...
// OMG these fucking oasis were hard to code
  ...
  int Square = rmCreateBoxConstraint("the big swimming pool",0.33,0.32,0.65,0.68,00.1);
/*
   if (PlayerNum <4)
   {numTries = 220;} //170
   else if (PlayerNum <6)
...
*/
   ...
   int pondConstraint=rmCreateClassDistanceConstraint("ponds avoid ponds", rmClassID("pond"), 22.0);
   ...
   for(i=0; <numPonds)
   {
      ...
      rmSetAreaSize(smallPondID, rmAreaTilesToFraction(60*PlayerNum), rmAreaTilesToFraction(75*PlayerNum));
	  rmAddAreaConstraint(smallPondID, Square);
      rmSetAreaWaterType(smallPondID, "Constantinople");
      ...
      rmSetAreaMaxBlobDistance(smallPondID, 20.0);
      rmAddAreaToClass(smallPondID, pondClass);
      rmSetAreaCoherence(smallPondID, 0.);
      rmSetAreaSmoothDistance(smallPondID, 7);
      rmAddAreaConstraint(smallPondID, pondConstraint);
      rmAddAreaConstraint(smallPondID, avoidKingsHill);
	  rmSetAreaWarnFailure(smallPondID, false);
      rmBuildArea(smallPondID);
	  
	  int nearShore1 = rmCreateEdgeMaxDistanceConstraint("stay near ponds"+i, smallPondID, 3);
		for (j=0; < 24*PlayerNum)
		{
			int OasisTrees=rmCreateObjectDef(i+"middle oasis trees"+j);
			rmAddObjectDefItem(OasisTrees, "TreeCaribbean", 1, 1.0);
			rmSetObjectDefMinDistance(OasisTrees,0);
			rmSetObjectDefMaxDistance(OasisTrees, rmXFractionToMeters(0.5));
			rmAddObjectDefConstraint(OasisTrees, nearShore1);
			rmPlaceObjectDefAtLoc(OasisTrees,0,0.5,0.5);
		}
   }

	    int forestPlayer=rmCreateArea("forest Player");
      rmSetAreaSize(forestPlayer, 0.08, 0.08);
      rmSetAreaForestType(forestPlayer, forestType);
	  rmSetAreaForestDensity(forestPlayer, 1);
	  rmSetAreaForestClumpiness(forestPlayer, 1);
	  rmSetAreaForestUnderbrush(forestPlayer, 0.5);
	  ...
	  rmSetAreaWarnFailure(forestPlayer, false);
      rmBuildArea(forestPlayer);	  
	  
	 	  
	    int forestPlayer1=rmCreateArea("forest Player1");
      rmSetAreaSize(forestPlayer1, 0.08, 0.08);
      rmSetAreaForestType(forestPlayer1, forestType);
	  rmSetAreaForestDensity(forestPlayer1, 1);
	  rmSetAreaForestClumpiness(forestPlayer1, 1);
	  rmSetAreaForestUnderbrush(forestPlayer1, 0.5);
	  ...
	  rmSetAreaWarnFailure(forestPlayer1, false);
 ...
 // ----------------------------------------------------- Players and blabla starting stuff -------------------------------------------------
	...
	if (isObserverMode == true)
	{
		...
		{ //1v1
			rmSetRandomSpawn1v1(0.61,0.15,  0.39,0.85);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3))
		{ // 2v2
			rmSetRandomSpawn2v2(0.34,0.83,  0.65,0.83,  0.34,0.17,  0.65,0.17);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
		{ // 3v3
			rmSetRandomSpawn3v3(0.50,0.86,  0.32,0.81,  0.69,0.82,  0.32,0.19,  0.69,0.18,  0.50,0.14);
		}
	}
	else
	{
		...
		if (cNumberTeams ==2)
		{
			if (rmRandFloat(0,1)<0.5)
			{
				...
				rmSetPlacementSection(0.95 - bonusTeam, 0.05 + bonusTeam); // 0.5
				rmSetTeamSpacingModifier(0.25);
				rmPlacePlayersCircular(0.37, 0.37, 0);
			}
			else
...
if (PlayerNum>2) // Fair mines for team
{
...
// This is all the area in order to get symetrical mines on the map. 3 mines on the left side, 3 mines on the right side.
   ...
   rmSetAreaWarnFailure(LeftSide, false);
   ...
   rmSetAreaWarnFailure(RightSide, false);
...
//   ******************************************* hunts *****************************************
   if (PlayerNum == 2)
   {
	...
	rmPlaceObjectDefAtLoc(pronghornIDLT, 0, 0.12, 0.7, 1);
	
    int pronghornIDLB=rmCreateObjectDef("ibex left bottom herd");
	...
	rmPlaceObjectDefAtLoc(pronghornIDLB, 0, 0.12, 0.2, 1);
	...
	rmPlaceObjectDefAtLoc(pronghornIDRT, 0, 0.88, 0.8, 1);
	...
	rmPlaceObjectDefAtLoc(pronghornIDRB, 0, 0.88, 0.3, 1);
	...
	rmPlaceObjectDefAtLoc(pronghornID2, 0, 0.36, 0.54, 1);
	
	int pronghornID3=rmCreateObjectDef("pronghorn herd3");
    rmAddObjectDefItem(pronghornID3, "ypIbex", rmRandInt(9,10), 6.0);
    rmSetObjectDefMinDistance(pronghornID3, 0.0);
    rmSetObjectDefMaxDistance(pronghornID3, 4.0);
	rmAddObjectDefConstraint(pronghornID3, avoidAll);  
    rmSetObjectDefCreateHerd(pronghornID3, true);
	rmPlaceObjectDefAtLoc(pronghornID3, 0, 0.64, 0.46, 1);
   }
   else
   {
   ...
   rmSetObjectDefCreateHerd(pronghornID, true);
	rmPlaceObjectDefAtLoc(pronghornID, 0, 0.5, 0.5, PlayerNum*2);
   ...
   rmSetObjectDefCreateHerd(pronghornID1, true);
	rmPlaceObjectDefAtLoc(pronghornID1, 0, 0.5, 0.5, PlayerNum*3);  
...
// -------------------------------------------------- Treasure time ---------------------------------------------------  
	...
	rmPlaceObjectDefAtLoc(nuggetID1, 0, 0.5, 0.5, PlayerNum*3);
	...
	rmPlaceObjectDefAtLoc(nuggetID, 0, 0.5, 0.5, PlayerNum);
