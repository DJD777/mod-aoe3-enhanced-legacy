...
//	int avoidGoldVeryFar = rmCreateClassDistanceConstraint ("gold avoid gold very ", rmClassID("Gold"), 74.0);
	...
	int avoidPond = rmCreateClassDistanceConstraint("avoid pond", rmClassID("pond"), 54.0+2*PlayerNum);
	...
	if (isObserverMode == true)
	{
		...
		{ //1v1
			rmSetRandomSpawn1v1(0.60, 0.82,0.40, 0.18);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3))
		{ // 2v2
			rmSetRandomSpawn2v2(0.55, 0.86,  0.79,0.75,  0.45,0.14,  0.21,0.25);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
		{ // 3v3
			rmSetRandomSpawn3v3(0.55,0.85,  0.75,0.75,  0.85,0.55,  0.45,0.15,  0.25,0.25,  0.15,0.45);
		}
	}
	else
	{
...
//				rmAddObjectDefConstraint(LlamaID, avoidTownCenterFar);
	...
	rmAddObjectDefItem(playerTreeID, "TreePampas", rmRandInt(1,1), 2.0);
...
//	rmAddObjectDefToClass(playerTree2ID, classForest);
	...
	rmSetObjectDefMinDistance(playerherdID, 22.0);
	rmSetObjectDefMaxDistance(playerherdID, 24.0);
    ...
    rmSetObjectDefMinDistance(player2ndherdID, 46);
    rmSetObjectDefMaxDistance(player2ndherdID, 48);
	...
	// Silver mines
	int silverminecount = 1+3*PlayerNum; 
...
//		rmAddAreaToClass(mainforestID, classForest);
	...
	// Secondary forest
	int secondaryforestcount = 2+2*PlayerNum;
...
//		rmAddAreaToClass(secondaryforestID, classForest);
		...
		for (j=0; < rmRandInt(5,6)) 
		{
			...
			rmAddObjectDefItem(secondarytreeID, "TreePampas", rmRandInt(1,2), 2.0); // 1,2
	...
	//Rhea herds
	int Rheacount = 2+2*PlayerNum;
	
	for (i=0; < Rheacount)
	{
		int RheaherdID = rmCreateObjectDef("Rhea herd"+i);
		rmAddObjectDefItem(RheaherdID, "Rhea", rmRandInt(6,6), 5.0);
...
//		}
	...
	//Guanaco herds
	int Guanacocount = 2+2*PlayerNum;
	
	for (i=0; < Guanacocount)
	{
		int GuanacoherdID = rmCreateObjectDef("Guanaco herd"+i);
		rmAddObjectDefItem(GuanacoherdID, "Guanaco", rmRandInt(9,9), 6.0);
