...
void main(void) {
	...
	if (isObserverMode == true) {
		...
		int forestConstraint=rmCreateClassDistanceConstraint("forest vs. forest", classForest, 30.0);
		int forestConstraintShort=rmCreateClassDistanceConstraint("object vs. forest", classForest, 4.0);
		...
		int avoidSocket=rmCreateClassDistanceConstraint("socket avoidance", classSocket, 5.0);
		int avoidSocketMore=rmCreateClassDistanceConstraint("bigger socket avoidance", classSocket, 15.0);
		...
		int avoidPlateau=rmCreateClassDistanceConstraint("stuff vs. cliffs", classPlateau, 5.0);
		int classAnvil = rmDefineClass("classAnvil");
		int avoidAnvil=rmCreateClassDistanceConstraint("stuff vs. Anvil", classAnvil, 5.0);
		int classContinet = rmDefineClass("classContinent");
		int avoidContinent=rmCreateClassDistanceConstraint("stuff vs. Continent", classContinet, 5.0);
		
		int avoidAnvilFlag=rmCreateClassDistanceConstraint("stuff vs. Anvil flag", classAnvil, 15.0);
		int avoidContinentFlag=rmCreateClassDistanceConstraint("flag vs. Continent", classContinet, 15.0);
		...
		if (isObserverMode == true)
		{
			...
			{ //1v1
				rmSetRandomSpawn1v1(0.2,0.25,0.8,0.25);
			}
			else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3))
			{ // 2v2
				rmSetRandomSpawn2v2(0.2,0.21, 0.2,0.47,0.8, 0.21, 0.8,0.47);
			}
			else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
			{ // 3v3
				rmSetRandomSpawn3v3(0.2,0.21, 0.2,0.47,  0.2,0.34,  0.8, 0.21, 0.8,0.47,  0.8,0.34);
			}
		}
		else
		{
		...
		rmAddAreaToClass(continent, classContinet);
		rmSetAreaSize(continent, 0.64, 0.64);
		rmSetAreaLocation(continent, 0.5, 0.15);
		rmSetAreaTerrainType(continent, "texas\ground4_tex");
		...
		rmAddAreaToClass(cape, classAnvil);
		rmSetAreaSize(cape, 0.02, 0.02);
		rmSetAreaSmoothDistance(cape, 1);
		rmSetAreaLocation(cape, 0.38, 0.8);
		rmSetAreaTerrainType(cape, "texas\ground4_tex");
		...
		rmAddAreaToClass(cape2, classAnvil);
		rmSetAreaSmoothDistance(cape2, 1);
		rmSetAreaSize(cape2, 0.02, 0.02);
		rmSetAreaLocation(cape2, 0.62, 0.8);
		rmSetAreaTerrainType(cape2, "texas\ground4_tex");
		...
		rmAddAreaToClass(cape3, classAnvil);
		rmSetAreaSize(cape3, 0.03, 0.03);
		rmSetAreaLocation(cape3, 0.5, 0.83);
		rmSetAreaSmoothDistance(cape3, 1);
		rmSetAreaTerrainType(cape3, "texas\ground4_tex");
		...
		rmAddAreaToClass(cape4, classAnvil);
		rmSetAreaSize(cape4, 0.018, 0.018);
		rmSetAreaLocation(cape4, 0.5, 0.68);
		rmSetAreaTerrainType(cape4, "texas\ground4_tex");
...
/*		
		...
		rmPlaceObjectDefAtPoint(socketID, 0, socketLoc1);
		*/
		       	int classRight = rmDefineClass("rightHalfC");
		int classLeft = rmDefineClass("leftHalfC");
		int avoidRight = rmCreateClassDistanceConstraint("avoid right", classRight, 5.0);
		int avoidLeft = rmCreateClassDistanceConstraint("avoid left", classLeft, 5.0);

		int rightHalf = rmCreateArea("rightHalf");
		rmAddAreaToClass(rightHalf, classRight);
        rmSetAreaSize(rightHalf, .51, .51);
        rmSetAreaLocation(rightHalf, .2, 0.0);
		//rmSetAreaTerrainType(rightHalf, "Amazon\ground2_ama");
        rmSetAreaCoherence(rightHalf, 1.0);
		rmAddAreaInfluenceSegment(rightHalf, 0.2, 0.0, 0.2, 1.0);
        rmBuildArea(rightHalf);

		int leftHalf = rmCreateArea("leftHalf");
		rmAddAreaToClass(leftHalf, classLeft);
        rmSetAreaSize(leftHalf, .51, .51);
        rmSetAreaLocation(leftHalf, 0.8, .0);
        //rmSetAreaTerrainType(leftHalf, "rockies\groundsnow3_roc");
        rmSetAreaCoherence(leftHalf, 1.0);
		rmAddAreaInfluenceSegment(leftHalf, 0.8, 0.0, 0.8, 1.0);
        rmBuildArea(leftHalf);
		...
		rmAddRandomTradeRouteWaypoints(tradeRouteID, 0.5, 0.75, 0, 0);
		...
		rmPlaceObjectDefAtLoc(fish2ID, 0, 0.5, 0.5, 8*PlayerNum);
		rmSetStatusText("",0.9);
		...
		int leftWhale=rmCreateObjectDef("leftWhale");
		rmAddObjectDefItem(leftWhale, "HumpbackWhale", 1, 0.0);
		rmSetObjectDefMinDistance(leftWhale, 0.0);
		rmSetObjectDefMaxDistance(leftWhale, rmXFractionToMeters(0.5));
		rmAddObjectDefConstraint(leftWhale, whaleVsWhaleID);
		rmAddObjectDefConstraint(leftWhale, whaleLand);
		rmAddObjectDefConstraint(leftWhale, avoidRight);
		rmPlaceObjectDefAtLoc(leftWhale, 0, 0.5, 0.5, whaleNum);
		...
		rmAddObjectDefConstraint(rightWhale, avoidLeft);
		rmPlaceObjectDefAtLoc(rightWhale, 0, 0.5, 0.5, whaleNum);
		
		float zFlag = 0.85;
		float xDelta = 1.0/PlayerNum;
		...
		//rmPlaceObjectDefAtLoc(waterFlag, i, .5, .75, 1);
		rmAddObjectDefConstraint(waterFlag, avoidContinentFlag);
		rmAddObjectDefConstraint(waterFlag, avoidAnvilFlag);
		rmAddObjectDefConstraint(waterFlag, circleConstraint);
		rmPlaceObjectDefAtLoc(waterFlag, i, xFlag, zFlag, 1);
		xFlag = xFlag + xDelta;
		}
		
		int riverGrass = rmCreateTerrainMaxDistanceConstraint("riverGrass stays near the water", "land", false, 3.0);
		// Paint some grass near the shore
		int grassPatchTop=rmCreateArea("top shore grass");
		rmSetAreaSize(grassPatchTop, .2, .2);
		rmSetAreaLocation(grassPatchTop, .75, .99);
		rmSetAreaWarnFailure(grassPatchTop, false);
		//rmSetAreaSmoothDistance(grassPatchTop, 10);
		rmSetAreaCoherence(grassPatchTop, .8);
		rmSetAreaTerrainType(grassPatchTop, "texas\cliff_top_tex");
		rmAddAreaConstraint(grassPatchTop, riverGrass);
		rmBuildArea(grassPatchTop);
		
		// Paint some grass near the shore
		int grassPatchBot=rmCreateArea("bot river grass");
		rmSetAreaSize(grassPatchBot, .2, .2);
		rmSetAreaLocation(grassPatchBot, .25, .99);
		rmSetAreaWarnFailure(grassPatchBot, false);
		//rmSetAreaSmoothDistance(grassPatchBot, 10);
		rmSetAreaCoherence(grassPatchBot, .8);
		rmSetAreaTerrainType(grassPatchBot, "texas\cliff_top_tex");
		...
		rmSetAreaTerrainType(topRightShoreFix, "texas\ground4_tex");
		rmSetAreaBaseHeight(topRightShoreFix, -4.5);
		...
		int avoidPatch = rmCreateClassDistanceConstraint("avoid patch", classPatch, 22.0);
		...
		rmAddAreaToClass(leftPlateau, classPlateau);
		rmAddAreaTerrainReplacement(leftPlateau, "texas\ground4_tex", "texas\ground2_tex");//great_plains\ground8_gp
		rmSetAreaCliffType(leftPlateau, "Texas Grass");
		rmSetAreaCliffEdge(leftPlateau, 1, 1, 0.0, 0.0, 1); 
		rmSetAreaCliffPainting(leftPlateau, true, true, true, 1.5, true);
		rmSetAreaCliffHeight(leftPlateau, 3, 0.1, 0.5);
		...
		rmAddAreaTerrainReplacement(rampLarge, "texas\cliff_top_grass_tex", "texas\ground3_tex");
		...
		for (i=0; < PlayerNum*125){
			int patchID = rmCreateArea("the redder stuff1 "+i);
			rmSetAreaWarnFailure(patchID, false);
			rmSetAreaSize(patchID, rmAreaTilesToFraction(30), rmAreaTilesToFraction(51));
			rmAddAreaTerrainReplacement(patchID, "texas\ground4_tex", "texas\ground5_tex");
		...
		for (i=0; < PlayerNum*125){
			...
			rmAddAreaTerrainReplacement(patchID2, "texas\ground4_tex", "texas\ground5_tex");
			rmAddAreaToClass(patchID2, classPatch);
		...
		for (i=0; < PlayerNum*125){
			...
			rmSetAreaSize(patchID3, rmAreaTilesToFraction(51), rmAreaTilesToFraction(75));
			rmAddAreaTerrainReplacement(patchID3, "texas\ground2_tex", "texas\ground1_tex");
			rmAddAreaToClass(patchID3, classPatch);
	...
	nativeID0 = rmCreateGrouping("Apache village", "native Apache village "+4);
	...
	if(nativeSwitch == 0){
	rmPlaceGroupingAtLoc(nativeID0, 0, 0.62, 0.8);
	rmPlaceGroupingAtLoc(nativeID2, 0, 0.38, 0.8); 
	}else{
	rmPlaceGroupingAtLoc(nativeID0, 0, 0.38, 0.8);
	rmPlaceGroupingAtLoc(nativeID2, 0, 0.62, 0.8); 
	}
	rmSetStatusText("",0.5);
	...
	rmSetStatusText("",0.55);
		...
		rmPlaceObjectDefAtLoc(rheaxID, 0, 0.5, 0.5, 5*PlayerNum);
		rmSetStatusText("",0.8); 
		...
		rmPlaceObjectDefAtLoc(anvilNuggets, 0, 0.5, 0.8, 2*PlayerNum); 
		...
		rmPlaceObjectDefAtLoc(anvilMines, 0, 0.5, 0.8, .5*PlayerNum);
		...
		rmPlaceObjectDefAtLoc(mines, 0, 0.5, 0.5, 5*PlayerNum);
		...
		rmPlaceObjectDefAtLoc(leftMines, 0, 0.25, 0.45, 1*PlayerNum);
		...
		rmPlaceObjectDefAtLoc(rightMines, 0, 0.75, 0.45, 1*PlayerNum);
		...
		rmPlaceObjectDefAtLoc(nuggetID, 0, 0.5, 0.5, 3*(PlayerNum + 1));
		rmSetStatusText("",0.85);

		int avoidGrass=rmCreateClassDistanceConstraint("tree vs. grasses", classPatch, 1.0);
		
		//for (j=0; < (6*PlayerNum)) {   
			int StartAreaTree2ID=rmCreateObjectDef("plateau trees");
			rmAddObjectDefItem(StartAreaTree2ID, "TreeTexas", rmRandInt(11,12), rmRandFloat(10.0,11.0));//was 13,16 15,20
			rmAddObjectDefToClass(StartAreaTree2ID, classForest);
			...
			rmPlaceObjectDefAtLoc(StartAreaTree2ID, 0, 0.5, 0.5, 100*PlayerNum);
		//}
		/*
		...
		rmPlaceObjectDefAtLoc(fishID, 0, 0.5, 0.5, 3*PlayerNum);
*/

		int playerGoldID = rmCreateObjectDef("player starting silver");
		rmAddObjectDefItem(playerGoldID, "mine", 1, 0.0);
		rmAddObjectDefConstraint(playerGoldID, avoidCoin);
		rmSetObjectDefMinDistance(playerGoldID, 15.0);
		rmSetObjectDefMaxDistance(playerGoldID, 16.0);
		
		int playerGoldID2 = rmCreateObjectDef("player starting silver2");
		rmAddObjectDefItem(playerGoldID2, "mine", 1, 0.0);
		rmAddObjectDefConstraint(playerGoldID2, avoidCoin);
		rmSetObjectDefMinDistance(playerGoldID2, 19.0);
		rmSetObjectDefMaxDistance(playerGoldID2, 20.0);
	...
	else // fallback, mainly for observer mode where fairloc always fails
	{
		for(i=1; <=PlayerNum)
		{
			rmPlaceObjectDefAtLoc(playerGoldID2, 0, rmPlayerLocXFraction(i), rmPlayerLocZFraction(i));
		}
	}
