...
void main(void)
{
	...
	int avoidMuskdeer = rmCreateTypeDistanceConstraint("avoid muskdeer ", "ypMuskdeer", 30.0);
	int avoidMuskdeerShort = rmCreateTypeDistanceConstraint("avoid muskdeer short ", "ypMuskdeer", 24.0);
	int avoidMuskdeerMin = rmCreateTypeDistanceConstraint("avoid muskdeer min ", "ypMuskdeer", 8.0);
	...
	int avoidWater = rmCreateTerrainDistanceConstraint("avoid water ", "water", true, 12.0);
	...
	if (isObserverMode == true)
	{
		...
		{ //1v1
			rmSetRandomSpawn1v1(0.55, 0.25,  0.25, 0.55);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3))
		{ // 2v2
			rmSetRandomSpawn2v2(0.55, 0.25, 0.70, 0.40,  0.25, 0.55, 0.40, 0.70);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
		{ // 3v3
			rmSetRandomSpawn3v3(0.50, 0.30, 0.675, 0.35, 0.85, 0.40,  0.30, 0.50, 0.35, 0.675, 0.40, 0.85);
		}
	}
	else
	{
		if (cNumberTeams <= 2) // 1v1 and TEAM
		{
			if (teamZeroCount == 1 && teamOneCount == 1) // 1v1
			{
				...
				{
					rmPlacePlayer(1, 0.55, 0.25);
					rmPlacePlayer(2, 0.25, 0.55);
				}
				else
				{
					rmPlacePlayer(2, 0.55, 0.25);
			...
			else if (teamZeroCount == teamOneCount) // equal N of players per TEAM
			{
				if (teamZeroCount == 2) // 2v2
				{
					...
					rmPlacePlayersLine(0.55, 0.25, 0.70, 0.40, 0.00, 0.20);

					rmSetPlacementTeam(1);
					rmPlacePlayersLine(0.25, 0.55, 0.40, 0.70, 0.00, 0.20);
				}
				else // 3v3, 4v4
				{
					rmSetPlacementTeam(0);
					rmPlacePlayersLine(0.50, 0.30, 0.85, 0.40, 0.00, 0.20);

					rmSetPlacementTeam(1);
					rmPlacePlayersLine(0.30, 0.50, 0.40, 0.85, 0.00, 0.20);
				}
			}
				else
				{
	...
	rmSetAreaMix(mainlandID, "borneo_grass_a");
	...
	for (i=0; < 32)
	{
		...
		rmSetAreaTerrainType(patchID, "borneo\ground_sand2_borneo");
	...
	for (i=0; < 42)
	{
		...
		rmSetAreaTerrainType(patch2ID, "borneo\shoreline1_borneo");
...
//	rmSetAreaMix(GoldZoneEID, "yellow_river_b");	// for testing
	...
	if (rmGetIsKOTH() == true) {
		...
		ypKingsHillLandfill(xLoc, yLoc, 0.008, 1.0, "borneo_sand_a", 0);
	...
	rmRiverSetShallowRadius(river2ID, 5+PlayerNum);
	...
	rmRiverSetShallowRadius(river3ID, 5+PlayerNum);
	...
	rmRiverSetShallowRadius(river1ID, 6+2*PlayerNum);
	...
	if (TeamNum == 2) {
		...
		rmAddTradeRouteWaypoint(tradeRouteID, 0.50, 1.00);
	...
	rmAddObjectDefItem(playerTreeID, "ypTreeBorneo", rmRandInt(6,6), 5.0);
	...
	rmSetObjectDefMinDistance(playeherdID, 18.0);
	...
	// Main forest patches
	int mainforestcount = 10+1*PlayerNum;
	...
	for (i=0; < mainforestcount)
    {
		...
		for (j=0; < rmRandInt(8,9))
		{
			...
			rmAddObjectDefItem(foresttreeID, "ypTreeBorneo", rmRandInt(2,3), 8.0);
			rmAddObjectDefItem(foresttreeID, "ypTreeCeylon", rmRandInt(0,1), 8.0);
	...
	// Secondary forest
	int secondforestcount = 8+1*PlayerNum;
	...
	for (i=0; < secondforestcount)
	{
		...
		for (j=0; < rmRandInt(8,9))
		{
			...
			rmAddObjectDefItem(secondforesttreeID, "ypTreeBorneo", rmRandInt(1,2), 6.0);	 
	...
	for(i=0; < herdcount)
	{
		...
		rmAddObjectDefItem(commonhuntID, "ypMuskdeer", rmRandInt(8,8), 6.0);
	...
	int herd2count = 2+PlayerNum;
	
	for(i=0; < herd2count)
	{
		int commonhunt2ID = rmCreateObjectDef("common herd2 "+i);
		rmAddObjectDefItem(commonhunt2ID, "ypMuskdeer", rmRandInt(8,8), 6.0);
	...
	// Berries 
	int berrybushcount = 4+1*PlayerNum;
	...
	for (i=0; < 6+1*PlayerNum)
	{
		...
		rmAddObjectDefItem(randomtreeID, "ypTreeSaxaul", rmRandInt(4,5), 3.0);
	...
	// Treasures 
	int treasure1count = 4+1*PlayerNum;
	int treasure2count = 2+1*PlayerNum;
	int treasure3count = 1*PlayerNum/2;
	int treasure4count = 1*PlayerNum/2;
	...
	int deltatreasurecount = 1*PlayerNum;
	...
	// Sea Resources 
	int fishcount = -1;
	fishcount = 10+2*PlayerNum;
	int whalecount = 4+1*PlayerNum;
