...
void main(void) {
   ...
   // Picks the map size
	...
	if (TeamNum >= 3){
	size = 2.25 * sqrt(PlayerNum*playerTiles);
}
...
//spawnSwitch = 0.1;
	...
	if (isObserverMode == true)
	{
		...
		{ //1v1
			rmSetRandomSpawn1v1(0.3, 0.7,  0.7, 0.3);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3))
		{ // 2v2
			rmSetRandomSpawn2v2(0.4, 0.8, 0.2, 0.6,      0.6, 0.2, 0.8, 0.4);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
		{ // 3v3
			rmSetRandomSpawn3v3(0.4, 0.8, 0.2, 0.6, 0.3, 0.7,  0.6, 0.2, 0.8, 0.4, 0.7, 0.3);
...
//=======================================================
        ...
        rmSetAreaTerrainType(patchID, "araucania\ground_grass3_ara");
...
//   rmSetAreaEdgeFilling(PlayerArea1, 1.0);
	...
	rmAddAreaToClass(glacierBaseNW, rmClassID("center"));
		...
		rmSetAreaCliffType(cliffs, "Araucania North");
	...
	rmBuildArea(cliffs);
		...
		rmSetAreaCliffType(cliffs2, "Araucania North");
...
//		rmAddAreaToClass(ffaCliffs, rmClassID("classPlateau"));
		...
		rmSetAreaCliffType(ffaCliffs, "Araucania central Coast2");
...
//proper spacing
 ...
 //       rmAddObjectDefConstraint(goldID, avoidCoinMed);
        ...
        rmAddObjectDefItem(treeID, "TreePatagonia", rmRandInt(6,7), 5.0);
        ...
        rmSetObjectDefMinDistance(foodID, 9.0);
        rmSetObjectDefMaxDistance(foodID, 9.0);
...
if (rmGetPlayerCiv(i) == rmGetCivID("Japanese")) {
	...
	if (rmGetIsKOTH()) {
		...
		ypKingsHillLandfill(xLoc, yLoc, 0.01, 3.5, "araucania\ground_grass3_ara", 0);
	...
	*/
	...
	rmPlaceObjectDefAtLoc(eastmine, 0, 0.5, 0.5, 8*PlayerNum);
...
//	rmAddObjectDefConstraint(elkHunts, avoidCoin);	
	rmPlaceObjectDefAtLoc(elkHunts, 0, 0.5, 0.5, 6*PlayerNum);
...
//	rmAddObjectDefConstraint(nuggetID, avoidTownCenterMore); 
	...
	rmSetNuggetDifficulty(1, 1); 
	rmPlaceObjectDefAtLoc(nuggetID, 0, 0.5, 0.5, 9*PlayerNum);
...
int avoidTPSocket =rmCreateTypeDistanceConstraint("avoid trade socket", "SocketTradeRoute", 16.0);
	...
	//for (j=0; < (6*PlayerNum)) {   
		...
		rmAddObjectDefItem(mapTrees, "TreePatagoniaDirt", rmRandInt(4,5), rmRandFloat(11.0,12.0));
...
/*
		...
		rmAddObjectDefItem(marshPlants, "UnderbrushPatagoniaDirt", rmRandInt(4,5), rmRandFloat(8.0,9.0));		rmSetObjectDefMinDistance(marshPlants, 0);
		rmSetObjectDefMaxDistance(marshPlants, rmXFractionToMeters(0.5));
	//  	rmAddObjectDefConstraint(marshPlants, avoidCoinMed);	
	  //	rmAddObjectDefConstraint(marshPlants, avoidGold);	
		rmPlaceObjectDefAtLoc(marshPlants, 0, 0.5, 0.5, 24*PlayerNum);
*/
...
rmPlaceObjectDefInArea(bonusTrees, 0, cliffs, 6*PlayerNum);
rmPlaceObjectDefInArea(bonusTrees, 0, cliffs2, 6*PlayerNum);
	...
	rmPlaceObjectDefAtLoc(fishID, 0, 0.5, 0.5, 25*PlayerNum);
			...
			int spcPatch = rmCreateArea("stupid textures");
			rmSetAreaSize(spcPatch, 1.0, 1.0);
			rmAddAreaTerrainReplacement(spcPatch, "araucania\groundshore1_ara", "araucania\ground_grass3_ara");
		//	rmAddAreaTerrainReplacement(spcPatch, "araucania\ground_dirt2_ara", "araucania\ground_grass3_ara");

			rmBuildArea(spcPatch); 
