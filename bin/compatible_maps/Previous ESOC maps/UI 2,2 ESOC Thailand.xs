...
int avoidPlateauShort =rmCreateClassDistanceConstraint("stuff vs. cliffs smoll", rmClassID("classPlateau"), 8.0);
		...
		int avoidTree=rmCreateTypeDistanceConstraint("avoid trees", "TreePonderosaPine", 11.0);
...
int stayEast = rmCreatePieConstraint("Stay East",0.53,0.47, rmXFractionToMeters(0.2),rmXFractionToMeters(0.5), rmDegreesToRadians(0),rmDegreesToRadians(180));
 ...
 // =============Player placement ======================= 
	...
	if (isObserverMode == true)
	{
		...
		{ //1v1
			rmSetRandomSpawn1v1(0.65, 0.8,  0.65, 0.2);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3))
		{ // 2v2
			rmSetRandomSpawn2v2(0.65, 0.8, 0.78, 0.72,  0.65, 0.2, 0.78, 0.28);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
		{ // 3v3
			rmSetRandomSpawn3v3(0.5, 0.85, 0.65, 0.8, 0.78, 0.72,    0.5, 0.15, 0.65, 0.2, 0.78, 0.28);
		}
	}
	else {
...
//spawnSwitch = 0.1;
 ...
 rmSetAreaBaseHeight(continent2, 0.0);
   ...
   rmSetAreaEdgeFilling(continent2, 1.0);
        ...
        rmSetAreaTerrainType(patchID, "borneo\ground_grass3_borneo");
...
//==========random forests==============
	...
	rmSetAreaMaxBlobDistance(ffaCliffs, 12);
        rmSetAreaWaterType(ffaCliffs, "borneo water");
...
//        rmAddObjectDefItem(treeID, "UnderbrushBorneo", rmRandInt(1,2), 9.0);
        ...
        rmAddObjectDefItem(foodID, "ypSerow", 9, 8.0);
	...
	rmAddObjectDefConstraint(foodID, avoidHuntSmall);	
        ...
        rmSetObjectDefMinDistance(foodID2, 25.0);
        rmSetObjectDefMaxDistance(foodID2, 25.0);
	...
	rmAddObjectDefConstraint(foodID2, avoidHuntSmall);
        ...
        rmAddObjectDefItem(foodID3, "ypSerow", 9, 6.0);
        rmSetObjectDefMinDistance(foodID3, 36.0);
        rmSetObjectDefMaxDistance(foodID3, 36.0);
...
if (rmGetPlayerCiv(i) == rmGetCivID("Japanese")) {
		...
		int waterFlag = rmCreateObjectDef("HC water flag "+i);
         ...
         rmSetObjectDefMaxDistance(waterFlag, 71);
...
//centre hunts
	...
	rmAddObjectDefItem(pronghornHerd, "ypSerow", 9, 8.0);
	...
	rmPlaceObjectDefAtLoc(pronghornHerd, 0, 0.5, 0.5, 8*PlayerNum);
...
rmAddObjectDefConstraint(lowlandTrees, waterHunt);
	...
	rmPlaceObjectDefAtLoc(lowlandTrees, 0, 0.5, 0.5, 100*PlayerNum);   
...
//	rmAddObjectDefConstraint(LlamaTime, avoidPlateau);	
	...
	rmPlaceObjectDefAtLoc(LlamaTime, 0, 0.3, 0.3, 4*PlayerNum);
		...
		rmAddObjectDefItem(waterProps, "UnderbrushBorneo", rmRandInt(2,3), rmRandFloat(4.0,5.0));
		rmSetObjectDefMinDistance(waterProps, 0);
		rmSetObjectDefMaxDistance(waterProps, rmXFractionToMeters(0.47));
		rmAddObjectDefConstraint(waterProps, propsHateProps);	

		rmPlaceObjectDefAtLoc(waterProps, 0, 0.5, 0.5, 10*PlayerNum);
