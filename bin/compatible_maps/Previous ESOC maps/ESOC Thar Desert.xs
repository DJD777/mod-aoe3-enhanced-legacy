...
void main(void)
{
   ...
   if (PlayerNum >6)
	...
	rmTerrainInitialize("borneo\shoreline1_borneo", 3);
...
// X marks the spot
   ...
   int avoidDeer=rmCreateTypeDistanceConstraint("Deer avoids food", "ypIbex", 35.0);
   int avoidDeerShort=rmCreateTypeDistanceConstraint("Deer avoids food short", "ypIbex", 20.0);
...
float Player3Z = 0.5+0.33*_sin(angleT1+PI/6);
	...
	if (isObserverMode == true)
	{
		...
		{ //1v1
			rmSetRandomSpawn1v1(Player1X,Player1Z,1.0-Player1X,1.0-Player1Z);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3))
		{ // 2v2
			rmSetRandomSpawn2v2(Player1X,Player1Z, Player2X,Player2Z,  1.0-Player1X,1.0-Player1Z,  1.0-Player2X,1.0-Player2Z);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
		{ // 3v3
			rmSetRandomSpawn3v3(Player1X,Player1Z, Player2X,Player2Z,  Player3X,Player3Z,  1.0-Player1X,1.0-Player1Z,  1.0-Player2X,1.0-Player2Z,  1.0-Player3X,1.0-Player3Z);
		}
	}
	else
	{
      ...
      for(i=1; <numPlayer)
   {
		...
		rmSetAreaTerrainType(id, "borneo\ground_sand1_borneo");
...
// ------------------------------------------------------ Starting Ressources ---------------------------------------------------------------------------
	...
	rmAddObjectDefItem(StartDeerID1, "ypIbex",rmRandInt(6,6), 9.0);
	...
	rmAddObjectDefItem(StartDeerID11, "ypIbex",rmRandInt(10,11), 9.0);
	rmSetObjectDefMinDistance(StartDeerID11, 40.0);
...
if (PlayerNum>2)
   ...
   rmAddObjectDefItem(IbexID, "ypIbex", IbexSize, 8.0);
   ...
   rmAddObjectDefItem(IbexID1, "ypIbex", IbexSize, 8.0);
   ...
   rmSetObjectDefCreateHerd(ElifentID1, true);
	...
	for (i=0; < northforestcount)
	{
		...
		rmSetAreaTerrainType(YellowForest, "borneo\ground_sand1_borneo");
...
//	---------------------------------------------------- Nuggets ----------------------------------------------------------------------------------------
	...
	rmPlaceObjectDefAtLoc(nuggetID, 0, 0.5, 0.5, PlayerNum*3+2);						   
