...
void main(void)
{
	...
	// Picks a default water height
		rmSetSeaLevel(-8.0);
		rmSetMapElevationParameters(cElevTurbulence, 0.05, 3, 0.4, 1.0); 
	...
	//Define some classes. These are used later for constraints.
		...
		int classPlateau = rmDefineClass ("classPlateau");	
	...
	// Resource avoidance
		...
		int avoidForest=rmCreateClassDistanceConstraint("avoid forest", rmClassID("Forest"), 34.0); 
		int avoidForestMed=rmCreateClassDistanceConstraint("avoid forest med", rmClassID("Forest"), 32.0); 
		int avoidForestShort=rmCreateClassDistanceConstraint("avoid forest short", rmClassID("Forest"), 24.0);
		int avoidForestMin=rmCreateClassDistanceConstraint("avoid forest min", rmClassID("Forest"), 12.0);
		int avoidDeerFar = rmCreateTypeDistanceConstraint("avoid deer far", "pronghorn", 60.0);
		int avoidDeer = rmCreateTypeDistanceConstraint("avoid deer", "pronghorn", 45.0);
		int avoidDeerShort = rmCreateTypeDistanceConstraint("avoid deer short", "pronghorn", 20.0);
		int avoidDeerMin = rmCreateTypeDistanceConstraint("avoid deer min", "pronghorn", 5.0);
		...
		int avoidGoldTypeMed = rmCreateTypeDistanceConstraint("coin avoids coin med ", "gold", 40.0);
		...
		int avoidGold = rmCreateClassDistanceConstraint ("gold avoid gold med", rmClassID("Gold"), 36.0);
		...
		int avoidWhale=rmCreateTypeDistanceConstraint("avoid whale", "fish", 28.0+1*PlayerNum);
		int avoidSheep=rmCreateTypeDistanceConstraint("sheep avoid sheep", "sheep", 50.0);
		...
		int avoidTradeRouteShort = rmCreateTradeRouteDistanceConstraint("trade route short", 12.0);
		...
		int avoidTradeRouteSocketMin = rmCreateTypeDistanceConstraint("avoid trade route socket min", "socketTradeRoute", 8.0);
	...
	// Land and terrain constraints
		...
		int avoidWaterFar = rmCreateTerrainDistanceConstraint("avoid water far ", "water", true, 32);
		...
		int avoidPatch = rmCreateClassDistanceConstraint("patch avoid patch", rmClassID("patch"), 20.0);
		int avoidPatch2 = rmCreateClassDistanceConstraint("patch avoid patch 2", rmClassID("patch2"), 2.0);
		int avoidPatch3 = rmCreateClassDistanceConstraint("patch avoid patch 3", rmClassID("patch3"), 2.0);
	...
	if (isObserverMode == true)
	{
		...
		{ //1v1
			rmSetRandomSpawn1v1(0.29, 0.65,  0.65, 0.29);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3))
		{ // 2v2
			rmSetRandomSpawn2v2(0.4, 0.7, 0.15, 0.65,  0.7, 0.4, 0.65, 0.15);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
		{ // 3v3
			rmSetRandomSpawn3v3(0.45, 0.7, 0.2875, 0.675, 0.125, 0.65,  0.7, 0.45, 0.675, 0.2875, 0.65, 0.125);
		}
	}
	else
	{
		if (cNumberTeams <= 2) // 1v1 and TEAM
		{
			if (teamZeroCount == 1 && teamOneCount == 1) // 1v1
			{
				...
				{
					rmPlacePlayer(1, 0.29, 0.65);
					rmPlacePlayer(2, 0.65, 0.29);
				}
				else
				{
					rmPlacePlayer(2, 0.29, 0.65);
					rmPlacePlayer(1, 0.65, 0.29);
				}
			...
			else if (teamZeroCount == teamOneCount) // equal N of players per TEAM
			{
				if (teamZeroCount == 2) // 2v2
				{
					...
					rmPlacePlayersLine(0.4, 0.7, 0.15, 0.65, 0.00, 0.20);

					rmSetPlacementTeam(1);
					rmPlacePlayersLine(0.7, 0.4, 0.65, 0.15, 0.00, 0.20);
				}
				else // 3v3, 4v4
				{
					rmSetPlacementTeam(0);
					rmPlacePlayersLine(0.45, 0.7, 0.125, 0.65, 0.00, 0.20);

					rmSetPlacementTeam(1);
					rmPlacePlayersLine(0.7, 0.45, 0.65, 0.125, 0.00, 0.20);
				}
			}
			else
			{
	...
	rmSetAreaMix(continent, "newengland_grass");
	rmSetAreaBaseHeight(continent, -2.0);
	rmSetAreaCoherence(continent, 1.0); 
	rmSetAreaObeyWorldCircleConstraint(continent, false);
//	rmAddAreaInfluenceSegment(continent, 0.0, 0.4, 1.0, 0.4); 
	rmSetAreaSmoothDistance(continent, 12);
	...
	int centralLake=rmCreateArea("centralLake");
        rmSetAreaLocation(centralLake, 0.475, 0.475);
		rmSetAreaSize(centralLake, .05, .05);      
        rmSetAreaWaterType(centralLake, "great lakes"); // New England Coast
        rmSetAreaBaseHeight(centralLake, 0.6);
        rmSetAreaCoherence(centralLake, .8);
		rmSetAreaObeyWorldCircleConstraint(centralLake, false);
        rmBuildArea(centralLake);

		//King's Island
			int kingislandID=rmCreateArea("King's Island");
			if (PlayerNum < 5)
				rmSetAreaSize(kingislandID, 0.0065, 0.0065);
			else 
				rmSetAreaSize(kingislandID, 0.0035, 0.0035);
			rmSetAreaLocation(kingislandID, 0.475, 0.475);
			rmSetAreaMix(kingislandID, "newengland_grass");
	...
	if (rmGetIsKOTH() == true) {		
		...
		int avoidKOTHFar = rmCreateAreaDistanceConstraint("avoid KOTH far", kingislandID, 30.0);
	...
	int plateauID = rmCreateArea("cliffs");
		...
		rmSetAreaSize(plateauID, 0.0265, 0.0265); 
	//	rmSetAreaTerrainType(plateauID, "new_england\cliff_inland_top_ne"); 
		rmSetAreaCliffType(plateauID, "New England Inland Grass");
		rmSetAreaCliffEdge(plateauID, 1, 1.0, 0.0, 0.0, 0); 
		...
		rmSetAreaCoherence(plateauID, 0.85);
		rmSetAreaLocation(plateauID, 1.0, 1.0);		
		rmAddAreaInfluenceSegment(plateauID, 1.0, 1.0, 0.77, 0.77);
		rmSetAreaObeyWorldCircleConstraint(plateauID, false);
		rmBuildArea(plateauID);

		int avoidPlateau = rmCreateAreaDistanceConstraint("avoid plateau", plateauID, 12.0);
		int avoidPlateauFar = rmCreateAreaDistanceConstraint("avoid plateau far", plateauID, 30.0);
		int stayPlateau = rmCreateAreaMaxDistanceConstraint("stay in plateau", plateauID, 0);

	int plateau2ID = rmCreateArea("cliffs2");
		rmAddAreaToClass(plateau2ID, rmClassID("classPlateau"));
		rmSetAreaSize(plateau2ID, 0.0139, 0.0139); 
	//	rmSetAreaTerrainType(plateau2ID, "new_england\cliff_inland_top_ne"); 
		rmSetAreaCliffType(plateau2ID, "New England Inland Grass");
		rmSetAreaCliffEdge(plateau2ID, 1, 1.0, 0.0, 0.0, 0); 
		...
		rmSetAreaCoherence(plateau2ID, 0.85);
		rmSetAreaLocation(plateau2ID, 0.5725, 0.5725);		
		rmAddAreaInfluenceSegment(plateau2ID, 0.5725, 0.5725, 0.65, 0.65);
		rmSetAreaObeyWorldCircleConstraint(plateau2ID, false);
		rmBuildArea(plateau2ID);

		int avoidPlateau2 = rmCreateAreaDistanceConstraint("avoid plateau2", plateau2ID, 12.0);
...
//	rmSetAreaMix(playerareaID, "testmix");
	...
	// Gold Zone South
	int goldzoneSID = rmCreateArea("gold zone south");
	rmSetAreaSize(goldzoneSID, 0.11, 0.11);
	rmSetAreaLocation(goldzoneSID, 0.25, 0.35);
	rmAddAreaInfluencePoint(goldzoneSID, 0.35, 0.25);	
	rmSetAreaWarnFailure(goldzoneSID, false);
//	rmSetAreaMix(goldzoneSID, "yellow_river_b");	// for testing
	rmSetAreaCoherence(goldzoneSID, 1.0); 
	rmSetAreaElevationType(goldzoneSID, cElevTurbulence);
	rmSetAreaElevationVariation(goldzoneSID, 5.0);
	rmSetAreaElevationMinFrequency(goldzoneSID, 0.04);
	rmSetAreaElevationOctaves(goldzoneSID, 3);
	rmSetAreaElevationPersistence(goldzoneSID, 0.4);      
	rmSetAreaObeyWorldCircleConstraint(goldzoneSID, false);
	rmBuildArea(goldzoneSID);	

	// Gold Zone NE
	int goldzoneNEID = rmCreateArea("gold zone NE");
	if (PlayerNum == 2) {
		rmSetAreaSize(goldzoneNEID, 0.055, 0.055);
		rmSetAreaLocation(goldzoneNEID, 0.95, 0.45);
		rmAddAreaInfluenceSegment(goldzoneNEID, 0.95, 0.45, 0.85, 0.65);	
		}
	else {
		rmSetAreaSize(goldzoneNEID, 0.075, 0.075);
		rmSetAreaLocation(goldzoneNEID, 0.90, 0.30);
		rmAddAreaInfluenceSegment(goldzoneNEID, 0.90, 0.30, 0.85, 0.65);	
		}
	rmSetAreaWarnFailure(goldzoneNEID, false);
//	rmSetAreaMix(goldzoneNEID, "yellow_river_b");	// for testing
	rmSetAreaCoherence(goldzoneNEID, 1.0); 
	rmSetAreaElevationType(goldzoneNEID, cElevTurbulence);
	rmSetAreaElevationVariation(goldzoneNEID, 5.0);
	rmSetAreaElevationMinFrequency(goldzoneNEID, 0.04);
	rmSetAreaElevationOctaves(goldzoneNEID, 3);
	rmSetAreaElevationPersistence(goldzoneNEID, 0.4);      
	rmSetAreaObeyWorldCircleConstraint(goldzoneNEID, false);
	rmBuildArea(goldzoneNEID);	

	// Gold Zone NW
	int goldzoneNWID = rmCreateArea("gold zone NW");
	if (PlayerNum == 2) {
		rmSetAreaSize(goldzoneNWID, 0.055, 0.055);
		rmSetAreaLocation(goldzoneNWID, 0.45, 0.95);
		rmAddAreaInfluenceSegment(goldzoneNWID, 0.45, 0.95, 0.65, 0.85);	
		}
	else {
		rmSetAreaSize(goldzoneNWID, 0.075, 0.075);
		rmSetAreaLocation(goldzoneNWID, 0.30, 0.90);
		rmAddAreaInfluenceSegment(goldzoneNWID, 0.30, 0.90, 0.65, 0.85);	
		}
...
//	rmSetAreaMix(goldzoneNWID, "yellow_river_b");	// for testing
	...
	int avoidGoldZoneS = rmCreateAreaDistanceConstraint("avoid gold zone ", goldzoneSID, 20.0);
	int stayGoldZoneS = rmCreateAreaMaxDistanceConstraint("stay in gold zone ", goldzoneSID, 0.0);	
	int avoidGoldZoneNE = rmCreateAreaDistanceConstraint("avoid gold zone NE ", goldzoneNEID, 20.0);
	int stayGoldZoneNE = rmCreateAreaMaxDistanceConstraint("stay in gold zone NE ", goldzoneNEID, 0.0);	
	...
	for (i=0; < 12*PlayerNum)
    {
        ...
        rmSetAreaSize(patch1ID, rmAreaTilesToFraction(85), rmAreaTilesToFraction(70));
		rmSetAreaTerrainType(patch1ID, "araucania\ground12_ara"); 
		...
		rmAddAreaConstraint(patch1ID, avoidTownCenterShort);
		rmAddAreaConstraint(patch1ID, avoidPatch);
		rmAddAreaConstraint(patch1ID, avoidPlateau);
		rmAddAreaConstraint(patch1ID, avoidPlateau2);
		rmAddAreaConstraint(patch1ID, avoidKOTH);
	...
	for (i=0; < 12*PlayerNum)
    {
        ...
        rmSetAreaSize(patch2ID, rmAreaTilesToFraction(05), rmAreaTilesToFraction(05));
		rmSetAreaTerrainType(patch2ID, "new_england\cliff_inland_top_ne");		// new_england\cliff_edge_ne
		...
		rmAddAreaConstraint(patch2ID, stayPlateau);
	...
	for (i=0; < 8*PlayerNum)
    {
        ...
        rmSetAreaSize(patch3ID, rmAreaTilesToFraction(05), rmAreaTilesToFraction(05));
		rmSetAreaTerrainType(patch3ID, "new_england\cliff_inland_top_ne");		// new_england\cliff_edge_ne
		...
		rmAddAreaConstraint(patch3ID, stayPlateau2);
	...
	int socketID=rmCreateObjectDef("sockets to dock Trade Posts");
		...
		rmAddTradeRouteWaypoint(tradeRouteID, 0.55, 0.05);
		rmAddTradeRouteWaypoint(tradeRouteID, 0.3, 0.15);
		rmAddTradeRouteWaypoint(tradeRouteID, 0.2, 0.2);
		rmAddTradeRouteWaypoint(tradeRouteID, 0.15, 0.3);
		rmAddTradeRouteWaypoint(tradeRouteID, 0.05, 0.55);

        rmBuildTradeRoute(tradeRouteID, "dirt");
 
        vector socketLoc1 = rmGetTradeRouteWayPoint(tradeRouteID, 0.075);
        ...
        if (PlayerNum < 4) {
			...
			socketLoc1 = rmGetTradeRouteWayPoint(tradeRouteID, 0.29);
			rmPlaceObjectDefAtPoint(socketID, 0, socketLoc1);

			socketLoc1 = rmGetTradeRouteWayPoint(tradeRouteID, 0.50);
			rmPlaceObjectDefAtPoint(socketID, 0, socketLoc1);

			socketLoc1 = rmGetTradeRouteWayPoint(tradeRouteID, 0.71);
			rmPlaceObjectDefAtPoint(socketID, 0, socketLoc1);
			}	
		socketLoc1 = rmGetTradeRouteWayPoint(tradeRouteID, 0.925);
        rmPlaceObjectDefAtPoint(socketID, 0, socketLoc1);
		}
   ...
   if (rmAllocateSubCivs(4) == true)
   {
      ...
      rmEchoInfo("subCiv0 is Iroquois "+subCiv0);
      if (subCiv0 >= 0)
	...
	nativeID0 = rmCreateGrouping("Iroquois village", "native Iroquois village "+2);
	...
	rmPlaceGroupingAtLoc(nativeID0, 0, 0.8, 0.2);

	nativeID1 = rmCreateGrouping("Iroquois village ", "native Iroquois village "+4);
	...
	rmPlaceGroupingAtLoc(nativeID1, 0, 0.2, 0.8);
	
	nativeID2 = rmCreateGrouping("Huron village", "native Huron village "+4);
	...
	nativeID3 = rmCreateGrouping("Huron village ", "native Huron village "+1);
	...
	rmPlaceGroupingAtLoc(nativeID3, 0, 0.8, 0.5);
	...
	rmAddObjectDefItem(playerTreeID, "treeNewEngland", rmRandInt(9,9), 4.0);
	...
	rmAddObjectDefItem(playerherdID, "elk", rmRandInt(10,10), 6.0);
    ...
    rmAddObjectDefItem(playerherd2ID, "moose", rmRandInt(3,3), 6.0);
    rmSetObjectDefMinDistance(playerherd2ID, 27);
    ...
    rmAddObjectDefItem(playerherd3ID, "elk", rmRandInt(10,10), 6.0);
    rmSetObjectDefMinDistance(playerherd3ID, 40);
    rmSetObjectDefMaxDistance(playerherd3ID, 40);
	...
	rmAddObjectDefConstraint(playerherd3ID, avoidWater);
	rmAddObjectDefConstraint(playerherd3ID, avoidGoldZoneS);

	// Starting treasures
	int playerNuggetID = rmCreateObjectDef("player nugget"); 
	rmAddObjectDefItem(playerNuggetID, "Nugget", 1,	0.0);
	rmSetNuggetDifficulty(1, 1);
	rmSetObjectDefMinDistance(playerNuggetID, 36.0);
	...
	rmAddObjectDefConstraint(playerNuggetID, avoidWater);
	...
	//	rmPlaceObjectDefAtLoc(playerherd3ID, i, rmXMetersToFraction(xsVectorGetX(TCLoc)), rmZMetersToFraction(xsVectorGetZ(TCLoc)));
        ...
        rmSetObjectDefMinDistance(waterFlag, 24.0);
        rmSetObjectDefMaxDistance(waterFlag, 26.0);
        rmPlaceObjectDefAtLoc(waterFlag, i, 0.48, 0.48, 1);
		rmAddObjectDefConstraint(waterFlag, avoidLand);
		rmAddObjectDefConstraint(waterFlag, avoidPlateau);
		rmAddObjectDefConstraint(waterFlag, avoidPlateau2);
	...
	if (PlayerNum > 2) {
		...
		for(i=0; < goldcount) {
			...
			if (TeamNum == 2)
				rmAddObjectDefConstraint(goldID, stayGoldZoneS);
			...
			rmPlaceObjectDefAtLoc(goldID, 0, 0.30, 0.30);
		}
	}
	else {
		int goldAID = rmCreateObjectDef("goldA"+i);
		rmAddObjectDefItem(goldAID, "Mine", 1, 0.0);
		rmSetObjectDefMinDistance(goldAID, rmXFractionToMeters(0.00));
		rmSetObjectDefMaxDistance(goldAID, rmXFractionToMeters(0.00));
		rmAddObjectDefToClass(goldAID, classGold);
		rmPlaceObjectDefAtLoc(goldAID, 0, 0.35, 0.08);
		
		int goldBID = rmCreateObjectDef("goldB"+i);
		rmAddObjectDefItem(goldBID, "Mine", 1, 0.0);
		rmSetObjectDefMinDistance(goldBID, rmXFractionToMeters(0.00));
		rmSetObjectDefMaxDistance(goldBID, rmXFractionToMeters(0.00));
		rmAddObjectDefToClass(goldBID, classGold);
		rmPlaceObjectDefAtLoc(goldBID, 0, 0.08, 0.35);

		int goldCID = rmCreateObjectDef("goldC"+i);
		rmAddObjectDefItem(goldCID, "Mine", 1, 0.0);
		rmSetObjectDefMinDistance(goldCID, rmXFractionToMeters(0.00));
		rmSetObjectDefMaxDistance(goldCID, rmXFractionToMeters(0.00));
		rmAddObjectDefToClass(goldCID, classGold);
		rmPlaceObjectDefAtLoc(goldCID, 0, 0.45, 0.30);
		
		int goldDID = rmCreateObjectDef("goldD"+i);
		rmAddObjectDefItem(goldDID, "Mine", 1, 0.0);
		rmSetObjectDefMinDistance(goldDID, rmXFractionToMeters(0.00));
		rmSetObjectDefMaxDistance(goldDID, rmXFractionToMeters(0.00));
		rmAddObjectDefToClass(goldDID, classGold);
		rmPlaceObjectDefAtLoc(goldDID, 0, 0.30, 0.45);
	}
	
	// NE Mines 	
	int goldNEcount = PlayerNum; 

	for(i=0; < goldNEcount)
	{
		int goldNEID = rmCreateObjectDef("goldNE "+i);
		rmAddObjectDefItem(goldNEID, "Mine", 1, 0.0);
		rmSetObjectDefMinDistance(goldNEID, rmXFractionToMeters(0.00));
		rmSetObjectDefMaxDistance(goldNEID, rmXFractionToMeters(0.30));
		rmAddObjectDefToClass(goldNEID, classGold);
		rmAddObjectDefConstraint(goldNEID, avoidGold);
		rmAddObjectDefConstraint(goldNEID, avoidTownCenterFar);
		if (TeamNum == 2)
			rmAddObjectDefConstraint(goldNEID, stayGoldZoneNE);
		rmAddObjectDefConstraint(goldNEID, avoidPlateauFar);
		rmAddObjectDefConstraint(goldNEID, avoidPlateau2Far);
		rmAddObjectDefConstraint(goldNEID, avoidNatives);
		rmAddObjectDefConstraint(goldNEID, avoidEdgeMore);
		rmPlaceObjectDefAtLoc(goldNEID, 0, 0.90, 0.50);
	}
	
	// NW Mines 	
	int goldNWcount = PlayerNum; 

	for(i=0; < goldNWcount)
	{
		int goldNWID = rmCreateObjectDef("goldNW "+i);
		rmAddObjectDefItem(goldNWID, "Mine", 1, 0.0);
		rmSetObjectDefMinDistance(goldNWID, rmXFractionToMeters(0.00));
		rmSetObjectDefMaxDistance(goldNWID, rmXFractionToMeters(0.30));
		rmAddObjectDefToClass(goldNWID, classGold);
		rmAddObjectDefConstraint(goldNWID, avoidGold);
		rmAddObjectDefConstraint(goldNWID, avoidTownCenterFar);
		if (TeamNum == 2)
			rmAddObjectDefConstraint(goldNWID, stayGoldZoneNW);
		rmAddObjectDefConstraint(goldNWID, avoidPlateauFar);
		rmAddObjectDefConstraint(goldNWID, avoidPlateau2Far);
		rmAddObjectDefConstraint(goldNWID, avoidNatives);
		rmAddObjectDefConstraint(goldNWID, avoidEdgeMore);
		rmPlaceObjectDefAtLoc(goldNWID, 0, 0.50, 0.90);
	}

	// Static Herds
		int staticherdAID = rmCreateObjectDef("static herdA "+i);
		rmAddObjectDefItem(staticherdAID, "pronghorn", rmRandInt(8,8), 6.0);
		rmSetObjectDefMinDistance(staticherdAID, rmXFractionToMeters(0.00));
		rmSetObjectDefMaxDistance(staticherdAID, rmXFractionToMeters(0.00));
		rmSetObjectDefCreateHerd(staticherdAID, true);
		rmAddObjectDefConstraint(staticherdAID, avoidNatives);
		rmAddObjectDefConstraint(staticherdAID, avoidGoldTypeMin);
		rmAddObjectDefConstraint(staticherdAID, avoidForestMin);		
		if (PlayerNum == 2)
			rmPlaceObjectDefAtLoc(staticherdAID, 0, 0.70, 0.50);

		int staticherdBID = rmCreateObjectDef("static herdB "+i);
		rmAddObjectDefItem(staticherdBID, "pronghorn", rmRandInt(8,8), 6.0);
		rmSetObjectDefMinDistance(staticherdBID, rmXFractionToMeters(0.00));
		rmSetObjectDefMaxDistance(staticherdBID, rmXFractionToMeters(0.00));
		rmSetObjectDefCreateHerd(staticherdBID, true);
		rmAddObjectDefConstraint(staticherdBID, avoidNatives);
		rmAddObjectDefConstraint(staticherdBID, avoidGoldTypeMin);
		rmAddObjectDefConstraint(staticherdBID, avoidForestMin);		
		if (PlayerNum == 2)
			rmPlaceObjectDefAtLoc(staticherdBID, 0, 0.50, 0.70);
			
	// Text
	rmSetStatusText("",0.70);

	// Main forest patches
	int mainforestcount = 8+2*PlayerNum;
	...
	for (i=0; < mainforestcount)
    {
        ...
        rmSetAreaSize(forestpatchID, rmAreaTilesToFraction(85), rmAreaTilesToFraction(85));
		...
		rmAddAreaConstraint(forestpatchID, avoidGoldTypeShort);
		rmAddAreaConstraint(forestpatchID, avoidKOTH);
		rmAddAreaConstraint(forestpatchID, avoidDeerMin);
		...
		for (j=0; < rmRandInt(6,7))
		{
			...
			rmAddObjectDefItem(foresttreeID, "TreeSaguenay", rmRandInt(1,2), 5.0);  
			rmAddObjectDefItem(foresttreeID, "treeNewEngland", rmRandInt(0,1), 5.0);  
	...
	// Secondary forest
	int secondforestcount = 4+1*PlayerNum;
	...
	for (i=0; < secondforestcount) {
		...
		rmSetAreaSize(secondforestID, rmAreaTilesToFraction(55), rmAreaTilesToFraction(55));
		...
		rmAddAreaConstraint(secondforestID, avoidTownCenter);
		rmAddAreaConstraint(secondforestID, avoidStartingResources);
		rmAddAreaConstraint(secondforestID, avoidPlateau);
		rmAddAreaConstraint(secondforestID, avoidPlateau2);
		rmAddAreaConstraint(secondforestID, avoidWaterShort);
		rmAddAreaConstraint(secondforestID, avoidTradeRouteSocket);
		rmAddAreaConstraint(secondforestID, avoidGoldTypeShort);
		rmAddAreaConstraint(secondforestID, avoidKOTH);
		rmAddAreaConstraint(secondforestID, avoidDeerMin);
		...
		for (j=0; < rmRandInt(6,7)) {
			...
			rmAddObjectDefItem(secondforesttreeID, "treeNewEngland", rmRandInt(1,2), 5.0);
			rmSetObjectDefMinDistance(secondforesttreeID, rmXFractionToMeters(0.00));
			rmSetObjectDefMaxDistance(secondforesttreeID, rmXFractionToMeters(0.45));
	...
	if (PlayerNum > 2) {
		...
		for(i=0; < herdcount) {
			...
			rmAddObjectDefItem(deerherdID, "pronghorn", rmRandInt(8,8), 6.0);
			rmSetObjectDefMinDistance(deerherdID, rmXFractionToMeters(0.0));
			rmSetObjectDefMaxDistance(deerherdID, rmXFractionToMeters(0.48));
			rmSetObjectDefCreateHerd(deerherdID, true);
			rmAddObjectDefConstraint(deerherdID, avoidGoldTypeMin);
			rmAddObjectDefConstraint(deerherdID, avoidEdge);
			rmAddObjectDefConstraint(deerherdID, avoidTradeRouteSocketMin);
			...
			rmAddObjectDefConstraint(deerherdID, avoidDeerShort);
			rmAddObjectDefConstraint(deerherdID, avoidWater);
			if (TeamNum == 2)
				rmAddObjectDefConstraint(deerherdID, stayGoldZoneS);
			rmPlaceObjectDefAtLoc(deerherdID, 0, 0.30, 0.30);
			}
		}
	else {
		int deerAID = rmCreateObjectDef("deer herdA"+i);
		rmAddObjectDefItem(deerAID, "pronghorn", rmRandInt(8,8), 6.0);
		rmSetObjectDefMinDistance(deerAID, rmXFractionToMeters(0.0));
		rmSetObjectDefMaxDistance(deerAID, rmXFractionToMeters(0.0));
		rmSetObjectDefCreateHerd(deerAID, true);
		rmPlaceObjectDefAtLoc(deerAID, 0, 0.55, 0.08);

		int deerBID = rmCreateObjectDef("deer herdB"+i);
		rmAddObjectDefItem(deerBID, "pronghorn", rmRandInt(8,8), 6.0);
		rmSetObjectDefMinDistance(deerBID, rmXFractionToMeters(0.0));
		rmSetObjectDefMaxDistance(deerBID, rmXFractionToMeters(0.0));
		rmSetObjectDefCreateHerd(deerBID, true);
		rmPlaceObjectDefAtLoc(deerBID, 0, 0.08, 0.55);

		int deerCID = rmCreateObjectDef("deer herdC"+i);
		rmAddObjectDefItem(deerCID, "pronghorn", rmRandInt(8,8), 6.0);
		rmSetObjectDefMinDistance(deerCID, rmXFractionToMeters(0.0));
		rmSetObjectDefMaxDistance(deerCID, rmXFractionToMeters(0.0));
		rmSetObjectDefCreateHerd(deerCID, true);
		rmPlaceObjectDefAtLoc(deerCID, 0, 0.35, 0.20);

		int deerDID = rmCreateObjectDef("deer herdD"+i);
		rmAddObjectDefItem(deerDID, "pronghorn", rmRandInt(8,8), 6.0);
		rmSetObjectDefMinDistance(deerDID, rmXFractionToMeters(0.0));
		rmSetObjectDefMaxDistance(deerDID, rmXFractionToMeters(0.0));
		rmSetObjectDefCreateHerd(deerDID, true);
		rmPlaceObjectDefAtLoc(deerDID, 0, 0.20, 0.35);
		}
	
	// NE Herds 
	int herd2count = PlayerNum;
	
	for(i=0; < herd2count)
	{
		int deerherd2ID = rmCreateObjectDef("deer herd2 "+i);
		rmAddObjectDefItem(deerherd2ID, "pronghorn", rmRandInt(8,8), 6.0);
		rmSetObjectDefMinDistance(deerherd2ID, rmXFractionToMeters(0.0));
		rmSetObjectDefMaxDistance(deerherd2ID, rmXFractionToMeters(0.30));
		rmSetObjectDefCreateHerd(deerherd2ID, true);
		rmAddObjectDefConstraint(deerherd2ID, avoidPlateau);
		rmAddObjectDefConstraint(deerherd2ID, avoidPlateau2);
		rmAddObjectDefConstraint(deerherd2ID, avoidNatives);
		rmAddObjectDefConstraint(deerherd2ID, avoidGoldTypeMin);
		rmAddObjectDefConstraint(deerherd2ID, avoidForestMin);
		rmAddObjectDefConstraint(deerherd2ID, avoidDeerShort);
		rmAddObjectDefConstraint(deerherd2ID, avoidStartingResources);
		rmAddObjectDefConstraint(deerherd2ID, stayGoldZoneNE);
		rmPlaceObjectDefAtLoc(deerherd2ID, 0, 0.90, 0.50);
	}

	// NW Herds 
	int herd3count = PlayerNum;
	
	for(i=0; < herd3count)
	{
		int deerherd3ID = rmCreateObjectDef("deer herd3 "+i);
		rmAddObjectDefItem(deerherd3ID, "pronghorn", rmRandInt(8,8), 6.0);
		rmSetObjectDefMinDistance(deerherd3ID, rmXFractionToMeters(0.0));
		rmSetObjectDefMaxDistance(deerherd3ID, rmXFractionToMeters(0.30));
		rmSetObjectDefCreateHerd(deerherd3ID, true);
		rmAddObjectDefConstraint(deerherd3ID, avoidPlateau);
		rmAddObjectDefConstraint(deerherd3ID, avoidPlateau2);
		rmAddObjectDefConstraint(deerherd3ID, avoidNatives);
		rmAddObjectDefConstraint(deerherd3ID, avoidGoldTypeMin);
		rmAddObjectDefConstraint(deerherd3ID, avoidForestMin);
		rmAddObjectDefConstraint(deerherd3ID, avoidDeerShort);
		rmAddObjectDefConstraint(deerherd3ID, avoidStartingResources);
		rmAddObjectDefConstraint(deerherd3ID, stayGoldZoneNW);
		rmPlaceObjectDefAtLoc(deerherd3ID, 0, 0.50, 0.90);
	}

	// Random Trees
	for (i=0; < 20+5*PlayerNum)
	{
		int randomtreeID = rmCreateObjectDef("random tree"+i);
		rmAddObjectDefItem(randomtreeID, "TreeSaguenay", rmRandInt(2,3), 2.0);
		...
		rmAddObjectDefConstraint(randomtreeID, avoidWaterShort);
	...
	// ____________________ King's "Treasures" ____________________
	if (rmGetIsKOTH() == false) {
		// Island mine
		int islandgoldID = rmCreateObjectDef("island mine");
		rmAddObjectDefItem(islandgoldID, "Minetin", 1, 0);
		rmSetObjectDefMinDistance(islandgoldID, 0.0);
		rmSetObjectDefMaxDistance(islandgoldID, 10.0);
		rmAddObjectDefToClass(islandgoldID, classStartingResource);
		rmAddObjectDefToClass(islandgoldID, classGold);
		rmAddObjectDefConstraint(islandgoldID, avoidStartingResourcesShort);
		rmAddObjectDefConstraint(islandgoldID, stayKOTH);
		rmPlaceObjectDefAtLoc(islandgoldID, 0, 0.48, 0.48);
		
		// Island berries
		int islandberriesID = rmCreateObjectDef("island berries");
		rmAddObjectDefItem(islandberriesID, "berrybush", rmRandInt(2,2), 2.0);
		rmSetObjectDefMinDistance(islandberriesID, 0.0);
		rmSetObjectDefMaxDistance(islandberriesID, 8.0);
		rmAddObjectDefToClass(islandberriesID, classStartingResource);
		rmAddObjectDefConstraint(islandberriesID, stayKOTH);
		rmAddObjectDefConstraint(islandberriesID, avoidStartingResourcesShort);
		rmPlaceObjectDefAtLoc(islandberriesID, 0, 0.48, 0.48);
		
		// Island trees
		int islandtreesID = rmCreateObjectDef("island trees");
		rmAddObjectDefItem(islandtreesID, "treeNewEngland", rmRandInt(3,4), 3.0);
		rmSetObjectDefMinDistance(islandtreesID, 0.0);
		rmSetObjectDefMaxDistance(islandtreesID, 8.0);
		rmAddObjectDefToClass(islandtreesID, classStartingResource);
		rmAddObjectDefToClass(islandtreesID, classForest);
		rmAddObjectDefConstraint(islandtreesID, avoidStartingResourcesShort);
		rmAddObjectDefConstraint(islandtreesID, stayKOTH);
		rmPlaceObjectDefAtLoc(islandtreesID, 0, 0.48, 0.48);
		
/*		// Island crates	
		int islandCrateID= rmCreateObjectDef("island crates"); 
		rmAddObjectDefItem(islandCrateID, "CrateofFood", rmRandInt(0,1), 5);
		rmAddObjectDefItem(islandCrateID, "CrateofWood", rmRandInt(1,1), 5);
		rmAddObjectDefItem(islandCrateID, "CrateofCoin", rmRandInt(1,1), 5);
		rmSetObjectDefMinDistance(islandCrateID, 0.0);
		rmSetObjectDefMaxDistance(islandCrateID, 14.0);
		rmAddObjectDefConstraint(islandCrateID, stayKOTH);
		rmAddObjectDefConstraint(islandCrateID, avoidStartingResourcesShort);
		rmPlaceObjectDefAtLoc(islandCrateID, 0, 0.48, 0.48);
	*/
		}
	...
	// Treasures 3
		...
		rmSetObjectDefMaxDistance(Nugget3ID, rmXFractionToMeters(0.10));
		...
		rmAddObjectDefConstraint(Nugget3ID, stayGoldZoneS);
		
		int nugget3count = PlayerNum/2;
		
		for (i=0; < nugget3count)
		{
			rmPlaceObjectDefAtLoc(Nugget3ID, 0, 0.30, 0.30);
		}
	...
	// Treasures 1
		...
		rmAddObjectDefConstraint(NuggetID, avoidDeerMin);
		...
		rmAddObjectDefConstraint(NuggetID, avoidPlateau2);
		rmAddObjectDefConstraint(NuggetID, avoidTradeRouteSocketShort);
		
		int nuggetcount = 4+1*PlayerNum;
	...
	for (i=0; < sheepsCount)
	{
		...
		rmAddObjectDefConstraint(sheepID, avoidPlateau);
		rmAddObjectDefConstraint(sheepID, avoidPlateau2);
		rmAddObjectDefConstraint(sheepID, avoidTownCenterMed);
		rmAddObjectDefConstraint(sheepID, avoidKOTH);
		rmAddObjectDefConstraint(sheepID, avoidWaterShort);
		rmAddObjectDefConstraint(sheepID, avoidNativesShort);
		rmAddObjectDefConstraint(sheepID, avoidTradeRouteSocketShort);
		rmAddObjectDefConstraint(sheepID, avoidGoldMin);
		rmAddObjectDefConstraint(sheepID, avoidGoldZoneS);
		rmAddObjectDefConstraint(sheepID, avoidForestMin);
		rmAddObjectDefConstraint(sheepID, avoidNuggetShort);
	...
	// ____________________ Fishes ____________________
		...
		rmAddObjectDefItem(fishID, "fishsalmon", rmRandInt(1,1), 6.0);
		rmSetObjectDefMinDistance(fishID, 0.00);
		rmSetObjectDefMaxDistance(fishID, rmXFractionToMeters(0.20));
		rmAddObjectDefConstraint(fishID, avoidFish);
		rmAddObjectDefConstraint(fishID, avoidLand);
	
		for (i=0; < 2+PlayerNum)
		{
				rmPlaceObjectDefAtLoc(fishID, 0, 0.48, 0.48);
		}
