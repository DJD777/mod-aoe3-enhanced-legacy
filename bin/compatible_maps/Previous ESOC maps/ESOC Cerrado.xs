...
int stayEast = rmCreatePieConstraint("Stay East",0.53,0.47, rmXFractionToMeters(0.2),rmXFractionToMeters(0.5), rmDegreesToRadians(0),rmDegreesToRadians(180));
	...
	if (isObserverMode == true)
	{
		...
		{ //1v1
			rmSetRandomSpawn1v1(0.225, 0.6,  0.775, 0.4);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3))
		{ // 2v2
			rmSetRandomSpawn2v2(0.225, 0.6, 0.45, 0.8,  0.775, 0.4, 0.55, 0.2);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
		{ // 3v3
			rmSetRandomSpawn3v3(0.2, 0.65, 0.4, 0.8, 0.25, 0.4, 0.8, 0.35, 0.6, 0.2, 0.75, 0.6);
		}
	}
	else {
	...
	//*************ffa placement********
	   ...
	   rmSetAreaMix(continent2, "mongolia_grass_b");
...
for(i=1; < PlayerNum + 1) {
	...
	rmAddObjectDefConstraint(playerStart, avoidWaterShort);
        ...
        rmSetObjectDefMinDistance(goldID, 22.0);
        ...
        rmAddObjectDefItem(treeID, "TreeAraucania", rmRandInt(6,7), 7.0);
        ...
        rmSetObjectDefMinDistance(foodID, 16.0);
	...
	rmAddObjectDefConstraint(foodID, avoidCoin);	
        ...
        rmSetObjectDefMaxDistance(foodID2, 20.0);
...
rmAddObjectDefConstraint(playerNuggetID, avoidCoin);
	...
	*/
	...
	rmPlaceObjectDefAtLoc(eastmine, 0, 0.5, 0.5, 7*PlayerNum);
}
	
    int tapirsForDays = rmCreateObjectDef("tapirsForDays");
	rmAddObjectDefItem(tapirsForDays, "tapir", rmRandInt(8,8), 7.0);
...
//	rmAddObjectDefConstraint(tapirsForDays, stayLow);
	...
	rmPlaceObjectDefAtLoc(tapirsForDays, 0, 0.5, 0.5, 6*PlayerNum);


  	int berryspam = rmCreateObjectDef("so many berries");
	rmAddObjectDefItem(berryspam, "BerryBush", rmRandInt(5,5), 4.0);
	rmAddObjectDefItem(berryspam, "UnderbrushBorneo", rmRandInt(2,3), 4.0);
	...
	rmPlaceObjectDefAtLoc(berryspam, 0, 0.5, 0.5, 18*PlayerNum);
...
//==========hills==============
	...
	rmAddAreaConstraint(islandW, avoidCoin);
            ...
            rmSetAreaBaseHeight(islandW, 2.0);
        rmSetAreaSmoothDistance(islandW, 8);
        rmSetAreaHeightBlend(islandW, 2);
	...
	//for (j=0; < (6*PlayerNum)) {   
		...
		rmAddObjectDefItem(mapTrees, "TreeAraucania", rmRandInt(1,1), rmRandFloat(5.0,6.0));
        rmAddObjectDefItem(mapTrees, "TreePampas", rmRandInt(1,2), 4.0);
...
//	rmAddObjectDefConstraint(mapTrees, circleConstraint);
	...
	//}
		...
		rmAddObjectDefItem(bonusTrees, "UnderbrushPampas", rmRandInt(2,3), rmRandFloat(8.0,11.0));
	rmAddObjectDefItem(bonusTrees, "UnderbrushBorneo", rmRandInt(1,2), 7.0);
	...
	rmPlaceObjectDefInArea(bonusTrees, 0, continent2, 15*PlayerNum);
