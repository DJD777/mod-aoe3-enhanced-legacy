...
//	rmEchoInfo("subCiv0 is Huron "+subCiv0);
	...
	if (isObserverMode == true)
	{
		...
		{ //1v1
			rmSetRandomSpawn1v1(0.16, 0.64,  0.64, 0.16);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3))
		{ // 2v2
			rmSetRandomSpawn2v2(0.10, 0.60, 0.35, 0.65,  0.60, 0.10, 0.65, 0.35);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
		{ // 3v3
			rmSetRandomSpawn3v3(0.60, 0.10, 0.65, 0.38,  0.625,0.24,  0.10, 0.60, 0.38, 0.65,  0.24,0.625);
		}
	}
	else
	{
...
//		rmAddObjectDefConstraint(RandomtreeID, avoidEdge);
	...
	//South Elks
	int elkcount = 2+3*PlayerNum;
	
	for(i=0; < elkcount)
	{
		int elkID = rmCreateObjectDef("south Elk"+i);
		rmAddObjectDefItem(elkID, "Elk", rmRandInt(10,10), 7.0);
