...
//	int stayNearTradeRoute = rmCreateTradeRouteMaxDistanceConstraint("stay near trade route", 5.0);
	...
	if (isObserverMode == true)
	{
		...
		{ //1v1
			rmSetRandomSpawn1v1(0.18,0.50,  0.82,0.50);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3))
		{ // 2v2
			rmSetRandomSpawn2v2(0.15,0.36,  0.15,0.64,  0.85,0.36,  0.85,0.64);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
		{ // 3v3
			rmSetRandomSpawn3v3(0.12,0.50,  0.19,0.29,  0.19,0.71,  0.81,0.71,  0.88,0.50,  0.81,0.29);
		}
	}
	else
	{
...
//		rmAddAreaConstraint(NWpatchID, avoidImpassableLandShort);
	...
	// Grass
	for (i=0; < 30+5*PlayerNum)
	{
		int GrassID = rmCreateObjectDef("drygrass"+i);
		rmAddObjectDefItem(GrassID, "TreeGreatPlainsGrass", rmRandInt(3,4), 6.0); 
...
//		rmPlaceObjectDefAtLoc(playergold3ID, i, rmXMetersToFraction(xsVectorGetX(TCLoc)), rmZMetersToFraction(xsVectorGetZ(TCLoc)));
	...
	int goldCount = 4+2*PlayerNum;  // 3,3 
...
*/	
	...
	int bisoncount = 1+2*PlayerNum;
	int pronghorncount = 1+1*PlayerNum;
	...
	for(i=0; < bisoncount)
	{
		...
		rmAddObjectDefItem(bisonID, "bison", rmRandInt(12,12), 10.0);
		...
		for(i=0; < pronghorncount)
	{
		...
		rmAddObjectDefItem(pronghornID, "pronghorn", rmRandInt(9,9), 8.0);
...
*/	// ******** Embellishments ********
	...
	for (i=1; < 3)
	{
		...
		rmAddAreaConstraint(geyserareaID, avoidPronghornMin); 
		rmAddAreaConstraint(geyserareaID, avoidBisonMin);
