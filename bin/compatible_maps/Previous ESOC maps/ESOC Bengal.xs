...
//	rmSetMapElevationHeightBlend(1);
	...
	int avoidBuffalo=rmCreateTypeDistanceConstraint("avoid water buffalo", "ypWaterBuffalo", 50.0);
	...
	int avoidIslandShort=rmCreateClassDistanceConstraint("avoid island short", classIsland, 10.0+PlayerNum);
	...
	if (isObserverMode == true)
	{
		...
		{ //1v1
			rmSetRandomSpawn1v1(0.46,0.88,  0.88,0.45);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3))
		{ // 2v2
			rmSetRandomSpawn2v2(0.52,0.88,  0.32,0.82,  0.82,0.32,  0.88,0.52);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
		{ // 3v3
			rmSetRandomSpawn3v3(0.40,0.86,  0.56,0.92,  0.24,0.80,  0.86,0.40,  0.80,0.24,  0.92,0.56);
		}
	}
	else
	{
...
//		rmAddObjectDefConstraint(commongoldID, avoidTradeRouteSocket);
	...
	// Inland forest
	int forestinlandcount = 10*PlayerNum;
	if (PlayerNum >= 4)
		forestinlandcount = 8*PlayerNum;
	if (PlayerNum >= 6)
		forestinlandcount = 6*PlayerNum;
...
//		rmAddAreaConstraint(forestinlandID, avoidEdge);
	...
	// Marsh forest
	int forestimarshcount = 10*PlayerNum;
	if (PlayerNum >= 4)
		forestimarshcount = 8*PlayerNum;
	if (PlayerNum >= 6)
		forestimarshcount = 6*PlayerNum;
...
//			rmAddObjectDefConstraint(forestmarshtreeID, avoidImpassableLandShort);
	...
	int nilgaiherdcount = 6*PlayerNum; //15
...
//			rmAddObjectDefConstraint(lowgroundherdID, stayNE);
	...
	int treasure1count = 5+PlayerNum;
	int treasure2count = 3+PlayerNum;
	int treasure34count = PlayerNum;
...
//		rmAddObjectDefConstraint(Nugget1ID, avoidTradeRouteSocket);
	...
	int buffalocount = 3+2*PlayerNum;
	
	
	for (i=0; < buffalocount)
	{
		int buffaloID=rmCreateObjectDef("water buffalo"+i);
		rmAddObjectDefItem(buffaloID, "ypWaterBuffalo", 1, 1.0);
		...
		rmAddObjectDefConstraint(buffaloID, avoidIslandMin);
		if (TeamNum <= 2)
			rmAddObjectDefConstraint(buffaloID, staySouthPart);
	...
	if (rmGetIsKOTH()) {
		...
		ypKingsHillLandfill(xLoc, yLoc, 0.025, 1.0, "yellow_river_forest", 0);
