...
void main(void)
{
   ...
   int playerTiles = 11000;
	...
	int classSecrets = rmDefineClass("secrets");
...
// ------------------------------------------------------ Contraints ---------------------------------------------------------------------------
	...
	int avoidStartingUnitsSmall=rmCreateClassDistanceConstraint("objects avoid starting unitss", classStartingUnit, 5.0);
	...
	// Resource avoidance
	int forestConstraint=rmCreateClassDistanceConstraint("forest vs. forest", classForest, 20.0);
	int avoidCoin=rmCreateTypeDistanceConstraint("avoid coin", "gold", 60.0);
	...
	int avoidCliffs=rmCreateClassDistanceConstraint("cliff vs. cliff", classCliff, 12.0);
	int avoidCliffsMin=rmCreateClassDistanceConstraint("cliff vs. cliff min", classCliff, 1.0);
	...
	int avoidSocket=rmCreateClassDistanceConstraint("socket avoidance", classSocket, 8.0);
    int avoidNatives = rmCreateClassDistanceConstraint("avoid Natives", classNative, 35.0);
    int avoidNativesShort = rmCreateClassDistanceConstraint("avoid Natives short", classNative, 10.0);
	...
	int shortAvoidImportantItem = rmCreateClassDistanceConstraint("secrets etc avoid each other short", classImportantItem, 8.0);
	int avoidWater = rmCreateTerrainDistanceConstraint("avoid water far", "land", true, 7.0);
...
// ------------------------------------------------------ Design & Trade Route ---------------------------------------------------------------------------
   ...
   rmAddAreaToClass(lakeID, classBay);
   ...
   rmBuildArea(lakeID);
	...
	rmSetAreaBaseHeight(centerIsland, 3.0);
	rmSetAreaHeightBlend(centerIsland, 3.0);
	...
	int landContraint = rmCreatePieConstraint("Land boundaries",0.5,0.5, rmXFractionToMeters(0.2),rmXFractionToMeters(0.5), rmDegreesToRadians(50),rmDegreesToRadians(310));
   
    int landAround=rmCreateArea("land around");
    rmSetAreaSize(landAround, 0.38, 0.38);
	...
	rmAddAreaConstraint(landAround, landContraint);
	rmAddAreaToClass(landAround, classIsland);
	rmSetAreaCoherence(landAround, 0.7);
	rmSetAreaSmoothDistance(landAround, 10);
  ...
  // check for KOTH game mode
   ...
   for(i=0; <10)
   {
	...
	rmAddGroupingToClass(SeminolesVillageID1, classImportantItem);
	rmAddGroupingToClass(SeminolesVillageID1, classNative);
	...
	rmAddGroupingToClass(SeminolesVillageID, classImportantItem);
	rmAddGroupingToClass(SeminolesVillageID, classNative);
...
// ------------------------------------------------------ Starting Ressources ---------------------------------------------------------------------------
	...
	if (isObserverMode == true)
	{
		...
		{ //1v1
			rmSetRandomSpawn1v1(0.17,0.54,  0.84,0.52);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3))
		{ // 2v2
			rmSetRandomSpawn2v2(0.15,0.54,  0.27,0.22,  0.85,0.52,  0.71,0.21);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
		{ // 3v3
			rmSetRandomSpawn3v3(0.15,0.54,  0.27,0.22,  0.17,0.37,  0.71,0.21,  0.85,0.52,  0.82,0.35);
		}
	}
	else
	{
		if (PlayerNum==2 && cNumberTeams ==2)
		 {
		...
		else if (PlayerNum==8 && cNumberTeams ==2)
		{
			rmSetPlacementTeam(1);
	...
	rmAddObjectDefToClass(startingUnits, classStartingUnit);
	...
	rmAddObjectDefToClass(TCID, classPlayer);
	rmAddObjectDefToClass(TCID, classStartingUnit);
	...
	rmAddObjectDefToClass(startingUnits, classStartingUnit);
	rmSetObjectDefMinDistance(StartTurkeyID, 12.0);
	rmSetObjectDefMaxDistance(StartTurkeyID, 15.0);
	...
	rmAddObjectDefItem(StartAreaTreeID, "TreeBayou", rmRandInt(4,6), 4.0);
	rmAddObjectDefToClass(startingUnits, classStartingUnit);
	...
	rmAddObjectDefToClass(playerNuggetID, classSecrets);
...
if (PlayerNum>2)
	...
	float middleChance=rmRandFloat(0,1);

	if ((middleChance>0.75 && cNumberNonGaiaPlayers==2) || (middleChance>0.25 && cNumberNonGaiaPlayers>2))
	{
		int silverIDmiddle = -1;
		silverIDmiddle = rmCreateObjectDef("silver middle");
		rmAddObjectDefItem(silverIDmiddle, "Mine", 1, 0.0);
		rmSetObjectDefMinDistance(silverIDmiddle, 0.0);
		rmSetObjectDefMaxDistance(silverIDmiddle, 15.0);
		rmAddObjectDefConstraint(silverIDmiddle, avoidTradeRoute);
		rmPlaceObjectDefAtLoc(silverIDmiddle, 0, 0.5, 0.5);
	}
	
   // ************************* Forest **************************	
    int numTries=6*PlayerNum+6;
    int failCount=0;
	int northforestcount = PlayerNum*10; // 6
	...
	for (i=0; < northforestcount)
	{
		...
		rmSetAreaTerrainType(YellowForest, "amazon\river3_am");
		rmSetAreaBaseHeight(YellowForest, 3.0);
				...
				for (j=0; < rmRandInt(2,3)) //18,20
		{
			...
			rmAddObjectDefItem(northtreeID1, "TreeKapok", rmRandInt(2,3), 3.0); // 1,2
			rmSetObjectDefMinDistance(northtreeID1, 0);
			rmSetObjectDefMaxDistance(northtreeID1, rmXFractionToMeters(0.5));
			rmAddObjectDefToClass(northtreeID1, classForest);
					...
					for (j=0; < rmRandInt(5,6)) //18,20
		{
			...
			rmAddObjectDefItem(underbrushID, "underbrushLake", rmRandInt(1,2), 4.0); // 1,2
			rmSetObjectDefMinDistance(underbrushID, 0);
			rmSetObjectDefMaxDistance(underbrushID, rmXFractionToMeters(0.5));
			rmAddObjectDefToClass(underbrushID, classForest);
	...
	int ElkPlacement=0;
	float ElkPositionX=-1;
	float ElkPositionZ=-1;
	result=0;
	leaveWhile=0;
	
	int ElkSize = rmRandInt(10,11);
   int ElkID=rmCreateObjectDef("Elk herd");
   rmAddObjectDefItem(ElkID, "Elk", ElkSize, 8.0);
   rmSetObjectDefMinDistance(ElkID, 0.0);
   rmSetObjectDefMaxDistance(ElkID, rmXFractionToMeters(0.0));
   rmAddObjectDefConstraint(ElkID, avoidAll);
   rmAddObjectDefConstraint(ElkID, avoidelk);
   rmAddObjectDefConstraint(ElkID, avoidTCFar2);
   rmAddObjectDefConstraint(ElkID, avoidCoinTree);
   rmAddObjectDefConstraint(ElkID, avoidEdgeGold);
   rmSetObjectDefCreateHerd(ElkID, true);


   int ElkID1=rmCreateObjectDef("Elk herd bottom");
   rmAddObjectDefItem(ElkID1, "Elk", ElkSize, 8.0);
   rmSetObjectDefMinDistance(ElkID1, 0.0);
   rmSetObjectDefMaxDistance(ElkID1, 0.0);
   rmSetObjectDefCreateHerd(ElkID1, true);


	while (ElkPlacement<numberOfHunts)
	{   
		ElkPositionX=rmRandFloat(0.52,0.98);
		ElkPositionZ=rmRandFloat(0.02,0.98);
		result=rmPlaceObjectDefAtLoc(ElkID, 0, ElkPositionX, ElkPositionZ,1);
		if (result!=0)
		{
			rmPlaceObjectDefAtLoc(ElkID1, 0, 1.0-ElkPositionX, ElkPositionZ,1);
			ElkPlacement++;
			leaveWhile=0;
		}
		else
...
// ------------------------------------------------ Nuggets -------------------------------------------------------------------
		...
		rmAddObjectDefConstraint(nugget3, avoidCoinTree);
		rmPlaceObjectDefAtLoc(nugget3, 0, 0.5, 0.5, 4*PlayerNum);
...
// ------------------------------------------------ Embelissment --------------------------------------------------------------
	...
	rmPlaceObjectDefAtLoc(randomEagleTreeID, 0, 0.5, 0.5, 2*PlayerNum);
	...
	rmPlaceObjectDefAtLoc(randomWaterTreeID, 0, 0.5, 0.5, 10*PlayerNum);
	rmPlaceObjectDefAtLoc(randomTurtlesID, 0, 0.5, 0.5, 2*PlayerNum);
	rmPlaceObjectDefAtLoc(randomWaterRocksID, 0, 0.5, 0.5, 20*PlayerNum);
	rmPlaceObjectDefAtLoc(randomGeeseID, 0, 0.5, 0.5, 2*PlayerNum);
	rmPlaceObjectDefAtLoc(randomDucksID, 0, 0.5, 0.5, 2*PlayerNum);	
