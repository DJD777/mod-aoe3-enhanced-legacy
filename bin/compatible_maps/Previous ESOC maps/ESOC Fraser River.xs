...
void main(void)
{
	...
	rmSetBaseTerrainMix("rockies_grass"); 
	rmTerrainInitialize("himalayas\cliff_himal_top", -4.00); 
	...
	if (isObserverMode == true)
	{
		...
		{ //1v1
			rmSetRandomSpawn1v1(0.85, 0.50,  0.15, 0.50);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3))
		{ // 2v2
				rmSetRandomSpawn2v2(0.85, 0.60, 0.85, 0.40,  0.15, 0.40, 0.15, 0.60);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
		{ // 3v3
				rmSetRandomSpawn3v3(0.85, 0.70, 0.90, 0.50, 0.85, 0.30,  0.15, 0.30, 0.10, 0.50, 0.15, 0.70);
		}
	}
	else
	{
	...
	if (spawnChooser > 0.00)
		...
		rmSetAreaMix(centralvalleyID, "yukon grass");  // himalayas_b
...
//	rmSetAreaMix(goldzoneNWID, "yellow_river_b");	// for testing
	...
	// ____________________ River ____________________
		int riverID = rmRiverCreate(-1, "Northwest Territory Water", 4+PlayerNum/2, 4+PlayerNum/2, 5, 5);  
		if (TeamNum <= 2) {
		...
		rmRiverSetShallowRadius(riverID, 7+2*PlayerNum);
		if (TeamNum <= 2) {
		...
		int river2ID = rmRiverCreate(-1, "yukon river", 5, 5, 5, 5);  
		if (TeamNum <= 2) {
		...
		rmRiverSetShallowRadius(river2ID, 6+2*PlayerNum);
		if (TeamNum <= 2) {
	...
	if (spawnChooser > 0.00) {
		...
		rmSetAreaCliffType(plateauID, "rocky mountain2"); 
		rmSetAreaTerrainType(plateauID, "himalayas\ground_dirt2_himal"); // himalayas\cliff_himal_top
		...
		rmSetAreaCliffType(plateau2ID, "rocky mountain2"); 
		rmSetAreaTerrainType(plateau2ID, "himalayas\ground_dirt2_himal"); 	// himalayas\cliff_himal_top
		...
		for (i=0; < 32)
		{
			...
			rmSetAreaTerrainType(patchID, "bayou\ground5_bay"); // rockies\groundforest_roc 
			...
			rmAddAreaConstraint(patchID, stayValley);
			rmAddAreaConstraint(patchID, avoidPlateau1);
			rmAddAreaConstraint(patchID, avoidPlateau2);
			rmBuildArea(patchID); 
		}
		...
		for (i=0; < 32)
		{
			...
			rmSetAreaSize(patch2ID, rmAreaTilesToFraction(55), rmAreaTilesToFraction(55));
			rmSetAreaTerrainType(patch2ID, "himalayas\cliff_himal_top"); // rockies\groundsnow5_roc
			...
			rmAddAreaConstraint(patch2ID, stayPlateau1);	
		...
		for (i=0; < 32)
		{
			int patch3ID = rmCreateArea("third patch"+i);
			rmSetAreaWarnFailure(patch3ID, false);
			rmSetAreaObeyWorldCircleConstraint(patch3ID, true);
			rmSetAreaSize(patch3ID, rmAreaTilesToFraction(55), rmAreaTilesToFraction(55));
			rmSetAreaTerrainType(patch3ID, "himalayas\cliff_himal_top"); // rockies\groundsnow5_roc
			rmAddAreaToClass(patch3ID, rmClassID("patch3"));
			rmSetAreaMinBlobs(patch3ID, 1);
			rmSetAreaMaxBlobs(patch3ID, 5);
			rmSetAreaMinBlobDistance(patch3ID, 13.0);
			rmSetAreaMaxBlobDistance(patch3ID, 30.0);
	...
	rmAddObjectDefItem(playerTreeID, "TreeRockiesSnow", rmRandInt(6,6), 5.0);
	...
	rmAddObjectDefItem(playeherdID, "elk", rmRandInt(8,8), 4.0);
	rmSetObjectDefMinDistance(playeherdID, 18);
	...
	rmAddObjectDefItem(player2ndherdID, "elk", rmRandInt(8,8), 6.0);
	...
	rmAddObjectDefItem(player3rdherdID, "elk", rmRandInt(6,6), 6.0);
	...
	rmSetObjectDefMinDistance(extraberrywagonID, 12.0);
	...
	if (spawnChooser < 0.00) {
		...
		rmSetAreaCliffType(border1ID, "rocky mountain2"); 
		rmSetAreaTerrainType(border1ID, "rockies\groundsnow3_roc");
	...
	//	rmAddAreaConstraint(border1ID, avoidTownCenterMin);
		...
		rmSetAreaCliffType(border2ID, "rocky mountain2"); 
		rmSetAreaTerrainType(border2ID, "rockies\groundsnow3_roc");
	...
	//	rmAddAreaConstraint(border2ID, avoidTownCenterMin);
		...
		for (i=0; <10*PlayerNum)
		{
			...
			rmSetAreaTerrainType(cliffID, "rockies\groundsnow3_roc");
	...
	rmSetStatusText("",0.70);
		...
		for (i=0; < 32)
		{
			int grasspatchID = rmCreateArea("grass patch"+i);
			rmSetAreaWarnFailure(grasspatchID, false);
			rmSetAreaObeyWorldCircleConstraint(grasspatchID, true);
			rmSetAreaSize(grasspatchID, rmAreaTilesToFraction(75), rmAreaTilesToFraction(100));
			rmSetAreaTerrainType(grasspatchID, "rockies\ground5_roc");   // himalayas\cliff_himal_top
			rmAddAreaToClass(grasspatchID, rmClassID("patch"));
			rmSetAreaMinBlobs(grasspatchID, 1);
			rmSetAreaMaxBlobs(grasspatchID, 5);
			rmSetAreaMinBlobDistance(grasspatchID, 13.0);
			rmSetAreaMaxBlobDistance(grasspatchID, 30.0);
			rmSetAreaCoherence(grasspatchID, 0.0);
			rmAddAreaConstraint(grasspatchID, avoidPatch);
			rmAddAreaConstraint(grasspatchID, avoidGoldMin);
			rmAddAreaConstraint(grasspatchID, avoidTownCenterShort);
			rmAddAreaConstraint(grasspatchID, avoidCliff);
			rmBuildArea(grasspatchID); 
		}
	
		// snow patches
		for (i=0; < 32)
		{
			int snowpatch1ID = rmCreateArea("1st snow patch"+i);
			rmSetAreaWarnFailure(snowpatch1ID, false);
			rmSetAreaObeyWorldCircleConstraint(snowpatch1ID, true);
			rmSetAreaSize(snowpatch1ID, rmAreaTilesToFraction(100), rmAreaTilesToFraction(100));
			rmSetAreaTerrainType(snowpatch1ID, "rockies\groundsnow5_roc");
			rmAddAreaToClass(snowpatch1ID, rmClassID("patch2"));
			rmSetAreaMinBlobs(snowpatch1ID, 1);
			rmSetAreaMaxBlobs(snowpatch1ID, 5);
			rmSetAreaMinBlobDistance(snowpatch1ID, 13.0);
			rmSetAreaMaxBlobDistance(snowpatch1ID, 30.0);
			rmSetAreaCoherence(snowpatch1ID, 0.0);
			rmAddAreaConstraint(snowpatch1ID, avoidPatch);
			rmAddAreaConstraint(snowpatch1ID, avoidPatch2);
			rmAddAreaConstraint(snowpatch1ID, avoidGoldMin);
			rmAddAreaConstraint(snowpatch1ID, stayValley);			
			rmBuildArea(snowpatch1ID); 
		}
	}
	...
	// Main Forest
	int mainforestcount = 4+4*PlayerNum;
	...
	for (i=0; < mainforestcount)
    {
		...
		for (j=0; < rmRandInt(7,8))
		{
			...
			if (spawnChooser > 0.00) {
				rmAddObjectDefItem(foresttreeID, "TreeNorthwestTerritory", rmRandInt(1,2), 5.0);
				rmAddObjectDefItem(foresttreeID, "TreeBayou", rmRandInt(1,2), 5.0);
				}
			else {
				rmAddObjectDefItem(foresttreeID, "TreeYukon", rmRandInt(3,4), 3.0);
			}
	...
	// Random Trees
	for (i=0; < 6+1*PlayerNum)
	{
		int randomtreeID = rmCreateObjectDef("random tree "+i);
		rmAddObjectDefItem(randomtreeID, "TreeRockiesSnow", rmRandInt(3,4), 2.0);
	...
	for (i=0; < 6+1*PlayerNum)
	{
		...
		rmAddObjectDefItem(randomtree2ID, "TreeRockiesSnow", rmRandInt(3,4), 2.0);
	...
	int statichunt1ID = rmCreateObjectDef("statichunt1 ");
		rmAddObjectDefItem(statichunt1ID, "elk", rmRandInt(8,8), 4.0);
	...
	int statichunt2ID = rmCreateObjectDef("statichunt2 ");
		rmAddObjectDefItem(statichunt2ID, "elk", rmRandInt(8,8), 4.0);
	...
	int statichunt3ID = rmCreateObjectDef("statichunt3 ");
		rmAddObjectDefItem(statichunt3ID, "elk", rmRandInt(8,8), 4.0);
	...
	int statichunt4ID = rmCreateObjectDef("statichunt4 ");
		rmAddObjectDefItem(statichunt4ID, "elk", rmRandInt(8,8), 4.0);
	...
	// NW Herds
	int elkherdcount = 3+1*PlayerNum;
		
	for (i=0; < elkherdcount)
	{
		int NWherdID = rmCreateObjectDef("NW herd"+i);
		rmAddObjectDefItem(NWherdID, "elk", rmRandInt(8,8), 4.0);
	...
	// SE Herds
	int elkherd2count = 3+1*PlayerNum;
		
	for (i=0; < elkherd2count)
	{
		int SEherdID = rmCreateObjectDef("SE herd"+i);
		rmAddObjectDefItem(SEherdID, "elk", rmRandInt(8,8), 4.0);
	...
	// Random River Herds
	if (TeamNum > 2) {
		int elkherd3count = 8+2*PlayerNum;
		...
		for (i=0; < elkherd3count)
		{
			...
			rmAddObjectDefItem(lowgroundherdID, "elk", rmRandInt(8,8), 3.0);
