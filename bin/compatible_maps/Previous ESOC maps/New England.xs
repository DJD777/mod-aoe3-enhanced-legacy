...
//	whichVariation = rmRandInt(1,2);
   ...
   */
   ...
   int bigContinentConstraint=rmCreateClassDistanceConstraint("avoid bonus island", classbigContinent, 18.0+PlayerNum);
  ...
  // Set up player starting locations.
	if (isObserverMode == true) {
		...
		if (rmGetPlayerTeam(1)!=rmGetPlayerTeam(2)){ //1v1
			rmSetRandomSpawn1v1(0.2000,0.5006,  0.8000,0.5000);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3)){ // 2v2
			rmSetRandomSpawn2v2(0.2000,0.7999,  0.2000,0.5002,  0.8000,0.8000,  0.8000,0.5004);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4)){ // 3v3
			rmSetRandomSpawn3v3(0.2000,0.5002,  0.2000,0.8000,  0.2000,0.6504,  0.8000,0.6504,  0.8000,0.8000,  0.8000,0.5003);
		}
	} else {
...
//   rmAddObjectDefItem(lakeTreeID, "TreeNewEngland", 2, 2.0);
   ...
   rmSetObjectDefMaxDistance(lakeTreeID, 0);
	...
	if ( whichVariation < 3 )
	{
		...
		rmBuildArea(smalllakeID);
      	...
      	numLakeTrees = rmRandInt(9,11);
     		for (j=0; <numLakeTrees)
         	   rmPlaceObjectDefInArea(lakeTreeID, 0, rmAreaID("small lake 1"), rmRandInt(2,3));
		...
		rmBuildArea(smalllakeID2);
      	...
      	numLakeTrees = rmRandInt(10,13);
     		for (j=0; <numLakeTrees)
         	   rmPlaceObjectDefInArea(lakeTreeID, 0, rmAreaID("small lake 2"), rmRandInt(2,3));

		// Two extra lakes?  Sometimes - only if more than two players.
		if ( extraLakes == 2 && PlayerNum > 2 )
		{
   ...
   rmAddObjectDefItem(playerDeerID, "deer", rmRandInt(7,8), 7.0);
   rmSetObjectDefMinDistance(playerDeerID, 18);
   rmSetObjectDefMaxDistance(playerDeerID, 20);
   ...
   rmAddObjectDefItem(farDeerID, "deer", rmRandInt(7,8), 7.0);
...
//   rmAddObjectDefConstraint(farDeerID, avoidTownCenter40);
   ...
   rmAddObjectDefItem(fartherDeerID, "deer", rmRandInt(7,8), 7.0);
...
//   rmAddObjectDefConstraint(fartherDeerID, avoidTownCenter50);
   ...
   // Define and place Nuggets
	...
	rmPlaceObjectDefInArea(nuggetID, 0, bigContinentID, PlayerNum*5);
   ...
   */
   ...
   rmPlaceObjectDefAtLoc(fishID, 0, 0.5, 0.0, PlayerNum*5);
...
// Fixed forest
   ...
   int fixedsilverID = -1;
	...
	int silverID = -1;
	int silverWestID = -1;
	int silverCount = 1*PlayerNum;	// 3 per player in each half of map, plus starting 1.
	...
	// FORESTS
   int numTries=2*PlayerNum;
   if (PlayerNum == 2)
      numTries=3*PlayerNum;
   if (PlayerNum == 3)
      numTries=2.5*PlayerNum;
   int failCount=0;
   ...
   rmAddObjectDefConstraint(deerID, longPlayerConstraint);
	...
	rmPlaceObjectDefInArea(deerID, 0, bigContinentID, PlayerNum*5);
   ...
   rmPlaceObjectDefInArea(deerWID, 0, bigContinentID, PlayerNum);

	// Text
   rmSetStatusText("",0.80);

	for(i=1; < PlayerNum*2+1)
	{
...
//		rmAddObjectDefConstraint(sheepID, avoidBonusIsland2);
   ...
   /*
	...
	// rmPlaceObjectDefInArea(bigDecorationID, 0, bigContinentID, PlayerNum*2);
...
//   rmPlaceObjectDefAtLoc(StragglerTreeID, 0, 0.5, 0.6, 1);
   ...
   treePattern = rmRandInt(1,4);
	...
	int stoneWallType = -1;
	int stoneWallID = -1;
	int stoneWallCount = PlayerNum*2;
	...
	for(i=0; < stoneWallCount)
	{
      ...
      rmSetGroupingMaxDistance(stoneWallID, rmXFractionToMeters(0.5));
		...
		rmAddGroupingConstraint(stoneWallID, avoidClearing);
