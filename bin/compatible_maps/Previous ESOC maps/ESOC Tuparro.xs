...
//water as base, with continent placed at similar height creates the random water/land patches
	...
	if (isObserverMode == true)
	{
		...
		{ //1v1
			rmSetRandomSpawn1v1(0.52, 0.18,  0.52, 0.82);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3))
		{ // 2v2
			rmSetRandomSpawn2v2(0.40, 0.15, 0.65, 0.15,  0.40, 0.85, 0.65, 0.85);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
		{ // 3v3
			rmSetRandomSpawn3v3(0.40, 0.15, 0.525, 0.15, 0.65, 0.15,  0.40, 0.85, 0.525, 0.85, 0.65, 0.85);
		}
	}
	else {
...
//**Andes build start**
		...
		rmSetAreaCliffType(andesBase, "Texas Grass");
	...
	//	rmAddAreaConstraint(andesBase, avoidTradeRouteSmall);
      rmSetAreaMix(andesBase, "pampas_grass");
...
//end landforms & natives
        ...
        rmSetObjectDefMinDistance(goldID2, 27.0);
	...
	rmAddObjectDefConstraint(goldID2, avoidWaterShort);
        ...
        rmAddObjectDefItem(treeID, "treeamazon", rmRandInt(6,9), 10.0);
        ...
        rmAddObjectDefItem(foodID, "Tapir", 6, 8.0);
        ...
        rmAddObjectDefItem(foodID3, "Tapir", 8, 8.0);
...
//	rmAddObjectDefConstraint(mapHunts, circleConstraint);
	...
	rmPlaceObjectDefAtLoc(mapHunts, 0, 0.5, 0.5, 7*PlayerNum);
...
//	rmAddObjectDefConstraint(northMines, circleConstraint);
	rmPlaceObjectDefAtLoc(northMines, 0, 0.5, 0.5, 2*PlayerNum);
...
//	rmAddObjectDefConstraint(southMines, circleConstraint);
	rmPlaceObjectDefAtLoc(southMines, 0, 0.5, 0.5, 2*PlayerNum);
...
//	rmAddObjectDefConstraint(nuggetID, avoidPlateau);	
	...
	rmSetStatusText("",0.7);
		...
		rmAddObjectDefItem(mapTrees, "treeamazon", rmRandInt(14,18), rmRandFloat(12.0,16.0));
...
if (PlayerNum == 2)
{
	...
	rmPlaceObjectDefInArea(bonusMines, 0, andesBase, 1*PlayerNum);
	...
	rmPlaceObjectDefInArea(bonusMines2, 0, andesBase, 1*PlayerNum);
}
...
//		rmAddObjectDefConstraint(bonusTrees, avoidCoin);
	...
	rmPlaceObjectDefInArea(bonusTrees, 0, andesBase, 8*PlayerNum);
