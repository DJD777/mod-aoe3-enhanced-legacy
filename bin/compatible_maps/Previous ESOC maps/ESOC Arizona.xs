...
//	rmSetBaseTerrainMix("painteddesert_groundmix_4"); //
	...
	int mesaavoidTradeRoute = rmCreateTradeRouteDistanceConstraint("mesa avoid trade route", 1+0.5*PlayerNum);
	...
	// Resource avoidance
	int avoidForestBig=rmCreateClassDistanceConstraint("avoid forest big", classForestBig, 50.0+2*PlayerNum); //15.0
...
//	int shortAvoidCliff=rmCreateClassDistanceConstraint("short stuff vs. cliff", classCliff, 8.0);
	...
	int avoidBisonFar = rmCreateTypeDistanceConstraint("avoid bison far", "bison", (55.0-2*PlayerNum));
...
//	int avoidGoldMed = rmCreateTypeDistanceConstraint("coin avoids coin", "gold", 30.0);
	...
	int avoidGoldFar = rmCreateClassDistanceConstraint ("avoid gold far", classGold, (66.0-2*PlayerNum));
	int avoidGoldShort = rmCreateClassDistanceConstraint ("avoid gold short", classGold, 8.0);
	int avoidNuggetFar = rmCreateTypeDistanceConstraint("nugget avoid nugget Far", "AbstractNugget", (60.0-1*PlayerNum));
	...
	int avoidPatch = rmCreateClassDistanceConstraint("avoid patch", classPatch, 40.0+2*PlayerNum);
	...
	int avoidPatch2 = rmCreateClassDistanceConstraint("avoid patch 2", classPatch2, 10.0+2*PlayerNum);
	...
	if (isObserverMode == true)
	{
		...
		{ //1v1
			rmSetRandomSpawn1v1(0.26,0.26,  0.74,0.74);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3))
		{ // 2v2
			rmSetRandomSpawn2v2(0.55,0.88,  0.88,0.55,  0.45,0.12,  0.12,0.45);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
		{ // 3v3
			rmSetRandomSpawn3v3(0.87,0.43,  0.77,0.77,  0.43,0.87,  0.57,0.13,  0.13,0.57,  0.23,0.23);
		}
	}
	else
	{
	...
	if (TeamNum <3 && rmGetIsKOTH() == false) // TEAM
	{	
	...
	rmSetAreaMix(grassareaID, "grandcanyon_rock_7");
...
//	rmSetAreaTerrainType(playerareaID, "new_england\ground2_cliff_ne");
	...
	int avoidPlayerArea = rmCreateClassDistanceConstraint ("avoid player area", classPlayer, 24.0+2*PlayerNum);
	...
	rmSetAreaSize(mesasurfaceID, rmAreaTilesToFraction(220*numPlayer), rmAreaTilesToFraction(220*numPlayer));  
...
//	rmSetAreaTerrainType(mesasurfaceID, "texas\ground5_tex");
	...
	rmSetAreaSmoothDistance(mesasurfaceID, 2+PlayerNum);
	rmBuildArea(mesasurfaceID);
	int stayInMesasurface = rmCreateAreaMaxDistanceConstraint("stay in mesa surface", mesasurfaceID, 0.0);
	int avoidMesasurface = rmCreateAreaDistanceConstraint("avoid mesa surface", mesasurfaceID, 20.0+2*PlayerNum);
	...
	{	
		...
		if (rmGetIsKOTH())
		{
			...
			rmSetAreaSize(mesaID, rmAreaTilesToFraction(160*PlayerNum/2), rmAreaTilesToFraction(160*PlayerNum/2));
		}
	...
	//	rmAddAreaConstraint (mesaID, buildMesaNorth);
		...
		rmSetAreaSmoothDistance(mesaID, 1+0.5*PlayerNum);
		...
		if (rmGetIsKOTH())
		{
			...
			rmSetAreaSize(mesa2ID, rmAreaTilesToFraction(160*PlayerNum/2), rmAreaTilesToFraction(160*PlayerNum/2));
		}
	...
	//	rmAddAreaConstraint (mesa2ID, buildMesaSouth);
		...
		rmSetAreaSmoothDistance(mesa2ID, 1+0.5*PlayerNum);
	...
	else
	{
		...
		if (rmGetIsKOTH()) 
		{
			...
			rmSetAreaSize(mesa3ID, rmAreaTilesToFraction(160*PlayerNum), rmAreaTilesToFraction(160*PlayerNum));  
		...
		rmSetAreaSmoothDistance(mesa3ID, 1+0.5*PlayerNum);
...
//		rmSetAreaMix(patch2ID, "texas_grass");
	...
	//Desert patches
	int desertpatchcount = 1*PlayerNum;
	...
	for (i=0; < desertpatchcount)
    {
        ...
        rmSetAreaSize(patchID, rmAreaTilesToFraction(280+10*PlayerNum), rmAreaTilesToFraction(320+10*PlayerNum));
		...
		rmSetAreaSmoothDistance(patchID, 6+1*PlayerNum);
...
//		rmAddAreaConstraint(patchID, avoidEdge);
	...
	rmSetAreaSize(internalpatch, rmAreaTilesToFraction(300+10*PlayerNum), rmAreaTilesToFraction(300+10*PlayerNum));	
	rmSetAreaObeyWorldCircleConstraint(internalpatch, false);
	rmSetAreaTerrainType(internalpatch, "sonora\ground2_son"); //borneo\shoreline1_borneo
//	rmAddAreaTerrainLayer(internalpatch, "texas\ground4_tex", 0, 1);
	rmSetAreaSmoothDistance(internalpatch, 4+1*PlayerNum);
...
//	rmAddObjectDefConstraint(playergold2ID, avoidCenter);
	...
	rmSetObjectDefMinDistance(playerbisonID, 14.0);
...
//		rmPlaceObjectDefAtLoc(playergoldID, i, rmXMetersToFraction(xsVectorGetX(TCLoc)), rmZMetersToFraction(xsVectorGetZ(TCLoc)));
	...
	int copperCount = 3*PlayerNum;  // 3,3 
...
//		}
	...
	//Forest
	int bigforestcount = 4+4*PlayerNum;
...
//		rmAddAreaToClass(bigforestID, classForest);
	...
	// Random trees 
	int treesmallcount = 6+8*PlayerNum;
...
//		rmAddObjectDefConstraint(bisonID, avoidTradeRoute);
	...
	int	nugget2count = 4+PlayerNum; 
	int	nugget3count = PlayerNum; 
	int	nugget4count = -1;
	if (TeamNum <= 2)
		nugget4count = TeamNum+(PlayerNum/TeamNum); 
	else
		nugget4count = PlayerNum/2;
