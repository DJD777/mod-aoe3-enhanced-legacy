...
//	string nativeName1 = "native Huron village";
	...
	int avoidElk = rmCreateTypeDistanceConstraint("elk avoids elk", "elk", 44.0);
	int avoidElkShort = rmCreateTypeDistanceConstraint("elk avoids elk short", "elk", 28.0);
	int avoidElkMin = rmCreateTypeDistanceConstraint("elk avoids elk min", "elk", 5.0);
...
//	int avoidTownCenterMed=rmCreateTypeDistanceConstraint("avoid Town Center med", "townCenter", 40.0);
	...
	if (isObserverMode == true)
	{
		...
		{ //1v1
			rmSetRandomSpawn1v1(0.23,0.75,  0.75,0.23);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3))
		{ // 2v2
			rmSetRandomSpawn2v2(0.64,0.15,  0.81,0.28,  0.28,0.81,  0.15,0.64);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
		{ // 3v3
			rmSetRandomSpawn3v3(0.45,0.11,  0.65,0.14,  0.81,0.27,  0.11,0.45,  0.14,0.65,  0.27,0.81);
		}
	}
	else
	{
...
//		rmSetAreaTerrainType(IceareaID, "great_lakes\ground_ice1_gl");
	...
	if (randomfrozen >= 0.5)
	{
		...
		rmSetAreaSize(IslandgrassID, rmAreaTilesToFraction(60+20*PlayerNum), rmAreaTilesToFraction(160+20*PlayerNum));
...
//		rmSetAreaElevationVariation(IslandgrassID, 0.5);
		...
		rmSetAreaSize(Islandgrass2ID, rmAreaTilesToFraction(60+20*PlayerNum), rmAreaTilesToFraction(160+20*PlayerNum));
...
//		rmSetAreaElevationVariation(Islandgrass2ID, 0.5);
		...
		rmSetAreaSize(Islandgrass3ID, rmAreaTilesToFraction(60+20*PlayerNum), rmAreaTilesToFraction(200+20*PlayerNum));
...
//	rmAddObjectDefToClass(playerTreeID, classForest);
	...
	if (randomfrozen >= 0.5)
		rmAddObjectDefItem(playermooseID, "moose", rmRandInt(1,1), 5.0);
	else
		rmAddObjectDefItem(playermooseID, "muskOx", rmRandInt(1,1), 5.0);
	rmSetObjectDefMinDistance(playermooseID, 8.0);
	rmSetObjectDefMaxDistance(playermooseID, 9.0);
...
//	rmAddObjectDefConstraint(playermooseID, avoidStartingResources);
	...
	if (randomfrozen >= 0.5)
		rmAddObjectDefItem(player2ndherdID, "elk", rmRandInt(9,9), 7.0);
	else
...
//	rmAddObjectDefConstraint(player2ndherdID, avoidTradeRouteShort);
	...
	rmAddObjectDefConstraint(player3rdherdID, avoidElk);
...
//		rmPlaceObjectDefAtLoc(playergold3ID, i, rmXMetersToFraction(xsVectorGetX(TCLoc)), rmZMetersToFraction(xsVectorGetZ(TCLoc)));
	...
	// ************* Mines **************
		...
		int goldCount = 3+2*PlayerNum;  // 3,3
...
//		rmAddAreaConstraint(forestID, avoidEdge);
	...
	int randomtreecount = 4+1*PlayerNum;
	...
	for (i=0; < randomtreecount)
	{
		...
		rmAddObjectDefConstraint(randomtreenorthID, avoidElkMin);
	...
	int northherdcount = 3+0.5*PlayerNum;
	int middleherdcount = 2+1*PlayerNum;
	int southherdcount = 2+1*PlayerNum;
	...
	for (i=0; < northherdcount)
	{
		...
		if (randomfrozen >= 0.5)
			rmAddObjectDefItem(northerdID, "moose", rmRandInt(6,6), 7.0);
		else
			rmAddObjectDefItem(northerdID, "muskOx", rmRandInt(6,6), 7.0);
...
//		rmAddObjectDefConstraint(northerdID, avoidTradeRouteSocket);
		...
		rmAddObjectDefConstraint(northerdID, avoidElk);
	...
	for (i=0; < middleherdcount)
	{
		...
		if (randomfrozen >= 0.5)
			rmAddObjectDefItem(middleherdID, "elk", rmRandInt(8,8), 8.0);
		else
			rmAddObjectDefItem(middleherdID, "caribou", rmRandInt(9,9), 8.0);
		...
		rmAddObjectDefConstraint(middleherdID, avoidElk);
...
//			rmAddObjectDefConstraint(middleherdID, avoidBayWaterFar);
	...
	for (i=0; < southherdcount)
	{
		...
		{
			if (mooseorelk == 0)
				rmAddObjectDefItem(southherdID, "moose", rmRandInt(6,6), 7.0);
			else
				rmAddObjectDefItem(southherdID, "elk", rmRandInt(8,8), 8.0);
		}
		else
		{
			if (mooseorelk == 0)
				rmAddObjectDefItem(southherdID, "muskOx", rmRandInt(6,6), 7.0);
			else
				rmAddObjectDefItem(southherdID, "caribou", rmRandInt(9,9), 8.0);
		}
		...
		rmAddObjectDefConstraint(southherdID, avoidElk);
	...
	if (randomfrozen >= 0.5)
		treasure2count = 8+0.5*PlayerNum;
	else
		treasure2count = 7+0.5*PlayerNum;
		treasure3count = 1+0.5*PlayerNum;
		treasure4count = 1;
		if (PlayerNum >= 4)
			treasure4count = 0.5*PlayerNum;
	...
	int fishcount = -1;
	if (randomfrozen >= 0.5)
	fishcount = 6+3*PlayerNum;
	else
	fishcount = 3+3*PlayerNum;
	int whalecount = 2+1*PlayerNum;
...
/*	// Underbrush grass
	if (randomfrozen >= 0.5)
	{
		for (i=0; < 50)
		{
			int UnderbrushgrassID = rmCreateObjectDef("underbrush grass "+i);
			rmAddObjectDefItem(UnderbrushgrassID, "GroundPropsSag", rmRandInt(1,1), 2.0);
			rmAddObjectDefItem(UnderbrushgrassID, "UnderbrushCoastalJapan", rmRandInt(4,5), 5.0);
			rmSetObjectDefMinDistance(UnderbrushgrassID, 0);
			rmSetObjectDefMaxDistance(UnderbrushgrassID, rmXFractionToMeters(0.7));
			rmAddObjectDefToClass(UnderbrushgrassID, rmClassID("grass"));
			rmAddObjectDefConstraint(UnderbrushgrassID, avoidGrass);
			rmAddObjectDefConstraint(UnderbrushgrassID, avoidImpassableLand);
			rmAddObjectDefConstraint(UnderbrushgrassID, avoidTradeRouteShort);
			rmAddObjectDefConstraint(UnderbrushgrassID, avoidTradeRouteSocket);
			rmAddObjectDefConstraint(UnderbrushgrassID, avoidNatives);
			rmAddObjectDefConstraint(UnderbrushgrassID, avoidGoldTypeShort);
			rmAddObjectDefConstraint(UnderbrushgrassID, avoidTownCenter);
			rmAddObjectDefConstraint(UnderbrushgrassID, avoidIsland1);
			rmAddObjectDefConstraint(UnderbrushgrassID, avoidIsland2);
			rmPlaceObjectDefAtLoc(UnderbrushgrassID, 0, 0.45, 0.45);
		}
	}
	else
	{
...
*/
	...
	else
	{
		for (i=0; < 50)
		{
			int UnderbrushsnowID = rmCreateObjectDef("underbrush snow "+i);
			rmAddObjectDefItem(UnderbrushsnowID, "Underbrushyukon", rmRandInt(1,2), 3.0);
			rmSetObjectDefMinDistance(UnderbrushsnowID, 0);
			rmSetObjectDefMaxDistance(UnderbrushsnowID, rmXFractionToMeters(0.7));
			rmAddObjectDefToClass(UnderbrushsnowID, rmClassID("grass"));
			rmAddObjectDefConstraint(UnderbrushsnowID, avoidGrass);
			rmAddObjectDefConstraint(UnderbrushsnowID, avoidImpassableLandShort);
			rmAddObjectDefConstraint(UnderbrushsnowID, avoidTradeRouteShort);
			rmAddObjectDefConstraint(UnderbrushsnowID, avoidTradeRouteSocket);
			rmAddObjectDefConstraint(UnderbrushsnowID, avoidNatives);
			rmAddObjectDefConstraint(UnderbrushsnowID, avoidGoldTypeShort);
			rmAddObjectDefConstraint(UnderbrushsnowID, avoidTownCenter);
			rmAddObjectDefConstraint(UnderbrushsnowID, avoidIceArea);
			rmPlaceObjectDefAtLoc(UnderbrushsnowID, 0, 0.45, 0.45);
		}
/*		for (i=0; < 10) //10
		{
			int WaterRockbig2ID = rmCreateObjectDef(" rock2 big"+i);
			rmAddObjectDefItem(WaterRockbig2ID, "GroundPropsYuk", rmRandInt(1,1), 5.0); //GroundPropsYuk
			rmSetObjectDefMinDistance(WaterRockbig2ID, 0);
			rmSetObjectDefMaxDistance(WaterRockbig2ID, rmXFractionToMeters(0.50));
			rmAddObjectDefToClass(WaterRockbig2ID, rmClassID("rocks"));
			rmAddObjectDefConstraint(WaterRockbig2ID, avoidRockFar);
			rmAddObjectDefConstraint(WaterRockbig2ID, avoidImpassableLandShort);
			rmAddObjectDefConstraint(WaterRockbig2ID, avoidIceArea);
			rmAddObjectDefConstraint(WaterRockbig2ID, avoidTradeRouteSocket);
			rmAddObjectDefConstraint(WaterRockbig2ID, avoidTradeRouteShort);
			rmAddObjectDefConstraint(WaterRockbig2ID, avoidPlayerArea1);
			rmAddObjectDefConstraint(WaterRockbig2ID, avoidPlayerArea2);
			rmAddObjectDefConstraint(WaterRockbig2ID, avoidNatives);
			rmAddObjectDefConstraint(WaterRockbig2ID, avoidForestMin);
			rmAddObjectDefConstraint(WaterRockbig2ID, avoidGoldTypeShort);
			rmPlaceObjectDefAtLoc(WaterRockbig2ID, 0, 0.50, 0.50); //0.50, 0.50
		}
*/
	}
	...
	if (randomfrozen >= 0.5)
	{
		for (i=0; < 16+2*PlayerNum)
		{
			int RockcoastID = rmCreateObjectDef("rockcoast"+i);
			if (randomfrozen >= 0.5)
				rmAddObjectDefItem(RockcoastID, "CoastalRocksSaguenay", rmRandInt(1,1), 5.0);
			else
				rmAddObjectDefItem(RockcoastID, "RiverPropsYuk", rmRandInt(1,1), 5.0);
			rmSetObjectDefMinDistance(RockcoastID, 0);
			rmSetObjectDefMaxDistance(RockcoastID, rmXFractionToMeters(0.24));
			rmAddObjectDefToClass(RockcoastID, rmClassID("rocks"));
			rmAddObjectDefConstraint(RockcoastID, avoidRock);
			rmAddObjectDefConstraint(RockcoastID, avoidForestMin);
			rmAddObjectDefConstraint(RockcoastID, avoidGoldTypeShort);
			rmAddObjectDefConstraint(RockcoastID, rockVsLand);
			rmAddObjectDefConstraint(RockcoastID, rockVsWater);
			rmAddObjectDefConstraint(RockcoastID, avoidIsland1);
			rmAddObjectDefConstraint(RockcoastID, avoidIsland2);
			rmAddObjectDefConstraint(RockcoastID, avoidIsland3);
			rmPlaceObjectDefAtLoc(RockcoastID, 0, 0.65, 0.65);
		}

		for (i=0; < 5)
		{
			int RockIsland1ID = rmCreateObjectDef("rock island1"+i);
			if (randomfrozen >= 0.5)
				rmAddObjectDefItem(RockIsland1ID, "CoastalRocksSaguenay", rmRandInt(1,1), 5.0);
			else
				rmAddObjectDefItem(RockIsland1ID, "RiverPropsYuk", rmRandInt(1,1), 5.0);
			rmSetObjectDefMinDistance(RockIsland1ID, 0);
			rmSetObjectDefMaxDistance(RockIsland1ID, rmXFractionToMeters(0.24));
			rmAddObjectDefToClass(RockIsland1ID, rmClassID("rocks"));
			rmAddObjectDefConstraint(RockIsland1ID, avoidRock);
			rmAddObjectDefConstraint(RockIsland1ID, avoidIsland1);
			rmAddObjectDefConstraint(RockIsland1ID, stayNearIsland1);
			rmPlaceObjectDefAtLoc(RockIsland1ID, 0, 0.65, 0.65);
		}

		for (i=0; < 5)
		{
			int RockIsland2ID = rmCreateObjectDef("rock island2"+i);
			if (randomfrozen >= 0.5)
				rmAddObjectDefItem(RockIsland2ID, "CoastalRocksSaguenay", rmRandInt(1,1), 5.0);
			else
				rmAddObjectDefItem(RockIsland2ID, "RiverPropsYuk", rmRandInt(1,1), 5.0);
			rmSetObjectDefMinDistance(RockIsland2ID, 0);
			rmSetObjectDefMaxDistance(RockIsland2ID, rmXFractionToMeters(0.24));
			rmAddObjectDefToClass(RockIsland2ID, rmClassID("rocks"));
			rmAddObjectDefConstraint(RockIsland2ID, avoidRock);
			rmAddObjectDefConstraint(RockIsland2ID, avoidIsland2);
			rmAddObjectDefConstraint(RockIsland2ID, stayNearIsland2);

			rmPlaceObjectDefAtLoc(RockIsland2ID, 0, 0.65, 0.65);
		}
	}
	// ****************************************************
