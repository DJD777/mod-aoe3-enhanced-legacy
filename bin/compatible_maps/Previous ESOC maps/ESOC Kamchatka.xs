...
//	rmSetMapElevationHeightBlend(1);
	...
	rmSetMapType("himalayas"); //("northwestTerritory");
	rmSetMapType("grass");
	rmSetMapType("land");
//	rmSetLightingSet("nwterritory");
	if (season >= 0.5)
		rmSetLightingSet("himalayas"); //california
	else
		rmSetLightingSet("himalayas"); //great lakes winter
...
//	int avoidForestFar=rmCreateClassDistanceConstraint("forest vs forest", rmClassID("Forest"), 35.0); //15.0
	...
	int avoidMuskdeerFar = rmCreateTypeDistanceConstraint("avoid muskdeer far", "ypmuskdeer", 44.0);
	int avoidMuskdeer = rmCreateTypeDistanceConstraint("avoid muskdeer", "ypmuskdeer", 37.0);
	int avoidMuskdeerShort = rmCreateTypeDistanceConstraint("avoid muskdeer short", "ypmuskdeer", 26.0);
...
//	int avoidGoldMed = rmCreateTypeDistanceConstraint("coin avoids coin", "gold", 30.0);
	...
	int avoidHole = rmCreateClassDistanceConstraint("avoid hole", rmClassID("Hole"), 6.0+1*PlayerNum);
	
	
	// Unit avoidance
	int avoidStartingUnits = rmCreateClassDistanceConstraint("objects avoid starting units", rmClassID("startingUnit"), 35.0);
	int avoidPlayerArea =  rmCreateClassDistanceConstraint("avoid players area", rmClassID("Player"), 6.0-0.5*PlayerNum);
	...
	if (isObserverMode == true)
	{
		...
		{ //1v1
			rmSetRandomSpawn1v1(0.64,0.16,  0.64,0.84);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3))
		{ // 2v2
			rmSetRandomSpawn2v2(0.74,0.22,  0.56,0.16,  0.74,0.78,  0.56,0.84);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
		{ // 3v3
			rmSetRandomSpawn3v3(0.72,0.22,  0.54,0.19,  0.36,0.16,  0.72,0.78,  0.54,0.81,  0.36,0.84);
		}
	}
	else // non-OBS mode
	{
	...
	{
		//river1
		int riverID1 = rmRiverCreate(-1, "Northwest Territory Water", 4, 6, 4+PlayerNum/2, 4+PlayerNum/2); //  (-1, "Northwest Territory Water", 4, 6, 5, 5)
		...
		//river2
		int riverID2 = rmRiverCreate(-1, "Northwest Territory Water", 4, 6, 4+PlayerNum/2, 4+PlayerNum/2); //  (-1, "Northwest Territory Water", 4, 6, 5, 5);
		...
		//shallows
		rmRiverSetShallowRadius(riverID1, 8+PlayerNum/2);
		rmRiverSetShallowRadius(riverID2, 8+PlayerNum/2);
		if (TeamNum <= 2)
		{
			if (Layoutvariation == 0) 
			{
...
//		rmSetAreaTerrainType(frozenriver2ID, "great_lakes\ground_ice1_gl");
		...
		rmSetAreaMix(traderoutepass1ID, "rockies_grass_snowa");
		...
		rmSetAreaMix(traderoutepass2ID, "rockies_grass_snowa");
		...
		rmSetAreaMix(traderoutepass3ID, "rockies_grass_snowa");
		...
		rmSetAreaMix(traderoutepass4ID, "rockies_grass_snowa");
		...
		int holecount = rmRandInt(2,3)*PlayerNum;
		if (PlayerNum >= 4) 
			holecount = 2*PlayerNum;
	...
	for (i = 0;  < cliffcount)
	{
		...
		rmSetAreaCliffHeight(CliffID, 4.5, 0.0, 1.0);
	...
	rmSetAreaSize(playerareaID, (0.084-0.0082*PlayerNum), (0.084-0.0082*PlayerNum));
...
//	rmSetAreaMix(playerareaID, "rockies_snow");
	...
	rmAddObjectDefItem(playerbighornID, "bighornsheep", 3, 2.0);
	rmSetObjectDefMinDistance(playerbighornID, 12.0);
	rmSetObjectDefMaxDistance(playerbighornID, 16.0);
	rmSetObjectDefCreateHerd(playerbighornID, true);
	...
	if (season >= 0.5)
		...
		rmAddObjectDefItem(playerCaribouID, "ypmuskdeer", rmRandInt(7,7), 6.0);
    ...
    if (season >= 0.5)
		...
		rmAddObjectDefItem(CaribouID5, "ypmuskdeer", rmRandInt(7,7), 6.0);
...
//	rmAddObjectDefConstraint(CaribouID5, avoidForestMin);
    ...
    if (season >= 0.5)
		...
		rmAddObjectDefItem(CaribouID6, "ypmuskdeer", rmRandInt(7,7), 6.0);
...
//	rmAddObjectDefConstraint(CaribouID6, avoidForestMin);
	...
	int extragoldCount = 4*PlayerNum;
	if (PlayerNum >= 4)
		extragoldCount = 4*PlayerNum-PlayerNum/2;
...
//		rmAddObjectDefConstraint(extragoldID, avoidCenter);
	...
	rmSetStatusText("",0.70);
   	...
   	// ******** FORESTS ********
	int mainforestcount = 8*PlayerNum;
	if (PlayerNum >= 4)
		mainforestcount = 5*PlayerNum;
	...
	for (i=0; < mainforestcount)
	{
		...
		if (season >= 0.5)
			...
			rmSetAreaTerrainType(mainforestID, "rockies\ground1_roc");
...
//			rmAddObjectDefToClass(maintreeID, classForest);
	...
	int randomforestcount = 6*PlayerNum;
	if (PlayerNum >= 4)
		randomforestcount = 4*PlayerNum;
	...
	// Caribou herds
	int cariboucount = 3*PlayerNum;
	if (PlayerNum >= 4)
		cariboucount = 3*PlayerNum-PlayerNum/2;
	
	for (i=1; < cariboucount+1)
	{
		int CaribouID = rmCreateObjectDef("caribou herd"+i);
		if (season >= 0.5)
			rmAddObjectDefItem(CaribouID, "caribou", rmRandInt(7,7), 7.0);
		else
			rmAddObjectDefItem(CaribouID, "ypmuskdeer", rmRandInt(7,7), 7.0);
...
//	rmAddObjectDefConstraint(elkID, avoidGoldMin);
	...
	int nugget1count = 4+PlayerNum/2; 
	int nugget2count = 2+0.5*PlayerNum;
	int nugget3count = 0.5*PlayerNum;
	...
	// Goats
	int goatcount = 4*PlayerNum;
	if (PlayerNum >= 4)
		goatcount = 4*PlayerNum-PlayerNum/2;
