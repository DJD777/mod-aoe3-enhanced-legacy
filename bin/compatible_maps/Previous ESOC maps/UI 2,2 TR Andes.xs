...
// modified by Rikikipu - September 2016
...
void main(void)
{
   ...
   // Picks the map size
	...
	if (PlayerNum<7)
		int size=2.1*sqrt(multBy*playerTiles);
	else
		size=2.0*sqrt(PlayerNum*playerTiles);
   ...
   // Avoid impassable land
	...
	int avoidCliffFar=rmCreateClassDistanceConstraint("stuff vs. cliff far", rmClassID("classCliff"), 18.0);
   ...
   int playerSide = rmRandInt(1,2);
	...
	if (isObserverMode == true) {
		...
		{ //1v1
			...
			rmPlacePlayer(3, 0.95, 0.6);
			rmPlacePlayer(4, 0.95, 0.4);
			rmPlacePlayer(5, 0.05, 0.45);
			//rmPlacePlayer(6, 0.05, 0.55);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3))
		{ // 2v2
			rmPlacePlayer(1, 0.25, 0.8);
			rmPlacePlayer(2, 0.5, 0.8);
			rmPlacePlayer(3, 0.26, 0.2);
			rmPlacePlayer(4, 0.5, 0.2);
			rmPlacePlayer(5, 0.95, 0.6);
			rmPlacePlayer(6, 0.95, 0.4);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
		{ // 3v3
			rmPlacePlayer(1, 0.25, 0.8);
			rmPlacePlayer(2, 0.5, 0.8);
			rmPlacePlayer(3, 0.38, 0.8);
			rmPlacePlayer(4, 0.26, 0.2);
			rmPlacePlayer(5, 0.5, 0.2);
			rmPlacePlayer(6, 0.38, 0.2);
		}
	} else {
		...
		{	rmSetPlacementTeam(0);
			rmPlacePlayersLine(0.28,0.8,0.5,0.8,0,0);
			
			rmSetPlacementTeam(1);
			rmPlacePlayersLine(0.28,0.2,0.5,0.2,0,0);
		}
	}
  ...
  if(rmGetIsKOTH())
  {
	...
	{  
		...
		rmPlaceGroupingAtLoc(incaVillageAID, 0, 0.84, 0.72);
	}
	...
	{  
		...
		rmPlaceGroupingAtLoc(incavillageBID, 0, 0.7, 0.83);
	}

	if (subCiv2 == rmGetCivID("Incas"))
	{  
		int incavillageCID = -1;
		incavillageCID = rmCreateGrouping("right city", "native inca village "+2);
		...
		rmPlaceGroupingAtLoc(incavillageCID, 0, 0.84, 0.28);
	}
	...
	{  
		...
		rmPlaceGroupingAtLoc(incavillageDID, 0, 0.7, 0.17);
	}
...
/*	rmAddObjectDefConstraint(TCID, playerEdgeConstraint);
	...
	rmSetObjectDefMinDistance(playerMedSilverID, 30.0);
	rmSetObjectDefMaxDistance(playerMedSilverID, 35.0);
	...
	int playerGuanacoID=rmCreateObjectDef("player guanaco");
      rmAddObjectDefItem(playerGuanacoID, "guanaco", rmRandInt(20,20), 16.0);
	...
	rmAddObjectDefConstraint(playerGuanacoID, avoidCliffFar);
      rmSetObjectDefCreateHerd(playerGuanacoID, true);

	int farGuanacoID=rmCreateObjectDef("player far guanaco");
      rmAddObjectDefItem(farGuanacoID, "guanaco", rmRandInt(8,10), 12.0);
	...
	int playerTreeID=rmCreateObjectDef("player trees");
      rmAddObjectDefItem(playerTreeID, "TreeAndes", 15, 8.0);
...
// Fixing Cliffs by Rikikipu
	int cliffIDA1=rmCreateArea("cliffa1");
	rmSetAreaSize(cliffIDA1, 0.01, 0.01);
	...
	rmSetAreaSize(cliffIDA2, 0.005, 0.005);
	...
	rmSetAreaSize(cliffIDA3, 0.015, 0.015);
	rmSetAreaLocation(cliffIDA3,1.0,0.6);
	rmAddAreaInfluenceSegment(cliffIDA3,1.0,0.6,0.85,0.6);
	...
	rmSetAreaSize(cliffIDB1, 0.01, 0.01);
	...
	rmSetAreaSize(cliffIDB2, 0.005, 0.005);
	...
	rmSetAreaSize(cliffIDB3, 0.015, 0.015);
	rmSetAreaLocation(cliffIDB3,1.0,0.4);
	rmAddAreaInfluenceSegment(cliffIDB3,1.0,0.4,0.85,0.4);
	...
	rmPlaceObjectDefAtLoc(nuggeteasyNWID, 0, 0.5, 0.5, PlayerNum/2);
	...
	rmSetStatusText("",0.65);
		...
		rmPlaceObjectDefAtLoc(nuggetmediumID, 0, 0.5, 0.5, PlayerNum/2);
		...
		rmPlaceObjectDefAtLoc(nuggetmediumNWID, 0, 0.5, 0.5, PlayerNum/2);
	...
	rmPlaceObjectDefAtLoc(nuggethardID, 0, 0.5, 0.5, PlayerNum/4);
	...
	rmPlaceObjectDefAtLoc(nuggethardNWID, 0, 0.5, 0.5, PlayerNum/4);
...
// Extra tree clumps near players - to ensure fair access to wood
   ...
   rmAddObjectDefItem(extraTreesID, "TreeAndes", 10, 5.0);
   ...
   rmAddObjectDefItem(extraTrees2ID, "TreeAndes", 10, 6.0);
...
// Huntables
      ...
      int guanacoCount = 0;
		guanacoCount =PlayerNum*4;
	for (i=0; <guanacoCount/2)
	{
		int guanacoID = rmCreateObjectDef("guanaco herd " +i);
		rmAddObjectDefItem(guanacoID, "guanaco", rmRandInt(8,10), 11);
		rmSetObjectDefMinDistance(guanacoID, 0.0);
		rmSetObjectDefMaxDistance(guanacoID, rmXFractionToMeters(0.3));
		...
		rmPlaceObjectDefAtLoc(guanacoID, 0, 0.45, 0.75);
	}

	for (i=0; <guanacoCount/2)
	{
		int guanacoSEID = rmCreateObjectDef("guanaco SE herd " +i);
		rmAddObjectDefItem(guanacoSEID, "guanaco", rmRandInt(8,10), 11);
		rmSetObjectDefMinDistance(guanacoSEID, 0.0);
		rmSetObjectDefMaxDistance(guanacoSEID, rmXFractionToMeters(0.3));
		...
		rmPlaceObjectDefAtLoc(guanacoSEID, 0, 0.45, 0.25);
	}
...
// Forest areas
	if (PlayerNum < 4)
		...
		numTries=6*PlayerNum;
	else if (PlayerNum == 6)
		numTries=5*PlayerNum;
	else
	...
	{   
		...
		rmSetAreaSize(forestnorthID, rmAreaTilesToFraction(200), rmAreaTilesToFraction(240));
