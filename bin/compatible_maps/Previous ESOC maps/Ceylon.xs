...
void main(void)
{
  ...
  // Set up player starting locations.
	if (isObserverMode == true) {
		...
		if (rmGetPlayerTeam(1)!=rmGetPlayerTeam(2)){ //1v1
			rmSetRandomSpawn1v1(.25,.2,  .75,.2);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3)){ // 2v2
			rmSetRandomSpawn2v2(0.3,0.14,  0.1,0.65,  0.7,0.14,  0.9,0.65);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4)){ // 3v3
			rmSetRandomSpawn3v3(0.3,0.14,  0.1,0.65,  0.1,0.4,  0.7,0.14,  0.9,0.65,  0.9,0.4);
		}
	} else {
	  if (TeamNum == 2) {
	...
	float playerFraction=rmAreaTilesToFraction(500 + PlayerNum*150);
	for(i=1; <numPlayer)
	{
  ...
  rmAddObjectDefConstraint(goldID, mesaConstraint); // keep the gold mines in the same area as the ridges
	rmPlaceObjectDefInArea(goldID, 0, bigIslandID, PlayerNum*2);
  ...
  rmAddObjectDefConstraint(silverID, shortAvoidImpassableLand);
	rmPlaceObjectDefInArea(silverID, 0, bigIslandID, PlayerNum*3);
  ...
  rmAddObjectDefConstraint(berriesID, avoidImportantItem);
	...
	rmPlaceObjectDefInArea(berriesID, 0, bigIslandID, PlayerNum*4);
  ...
  rmAddObjectDefConstraint(foodID1, avoidImportantItem);
	rmPlaceObjectDefInArea(foodID1, 0, bigIslandID, PlayerNum*4);
  ...
  rmAddObjectDefConstraint(foodID2, avoidHuntable2);
	rmPlaceObjectDefInArea(foodID2, 0, bigIslandID, PlayerNum*3);
  ...
  rmAddObjectDefConstraint(nugget1, avoidImportantItem);
	...
	rmPlaceObjectDefInArea(nugget1, 0, bigIslandID, PlayerNum*5);
  ...
  rmAddObjectDefConstraint(nugget3, avoidImportantItem);
	rmPlaceObjectDefInArea(nugget3, 0, bigIslandID, PlayerNum);
	...
	rmPlaceObjectDefAtLoc(whaleID, 0, 0.5, 0.5, PlayerNum*4);
	...
	rmPlaceObjectDefAtLoc(fish2ID, 0, 0.5, 0.5, 12*PlayerNum);

	if (PlayerNum <5)		// If less than 5 players, place extra fish.
	{
		rmPlaceObjectDefAtLoc(fish2ID, 0, 0.5, 0.5, 5*PlayerNum);
	}
