...
int stayEast = rmCreatePieConstraint("Stay East",0.53,0.47, rmXFractionToMeters(0.2),rmXFractionToMeters(0.5), rmDegreesToRadians(0),rmDegreesToRadians(180));
 ...
 // =============Player placement ======================= 
	...
	if (isObserverMode == true)
	{
		...
		{ //1v1
			rmSetRandomSpawn1v1(0.2, 0.67,  0.8, 0.33);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3))
		{ // 2v2
			rmSetRandomSpawn2v2(0.15, 0.58, 0.25, 0.19,  0.85, 0.42, 0.75, 0.81);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
		{ // 3v3
			rmSetRandomSpawn3v3(0.15, 0.58, 0.27, 0.19, 0.16, 0.38,    0.85, 0.42, 0.73, 0.81, 0.84, 0.62);
		}
	}
	else {
...
for(i=1; < PlayerNum + 1) {
    ...
    int PlayerArea1 = rmCreateArea("NeedLand1"+i);
        ...
        rmSetAreaBaseHeight(PlayerArea1, 0.0);
...
//=====================================================
    ...
    //    rmSetAreaBaseHeight(desertNW, -3.5);
    ...
    //    rmSetAreaBaseHeight(desertSW, -3.5);
...
//============================================
        ...
        rmSetAreaBaseHeight(pondSW, -1.0);
	...
	rmSetAreaMaxBlobDistance(pondSW, 8);
        ...
        rmSetAreaBaseHeight(pondNE, -1.0);
...
//	rmAddObjectDefConstraint(bisonBurgers, avoidCoin);
	...
	rmPlaceObjectDefAtLoc(bisonBurgers, 0, 0.5, 0.5, 9*PlayerNum);
}


    int pronghornHunts = rmCreateObjectDef("pronghornHunts");
	rmAddObjectDefItem(pronghornHunts, "pronghorn", rmRandInt(5,5), 5.0);
...
int avoidTPSocket =rmCreateTypeDistanceConstraint("avoid trade socket", "SocketTradeRoute", 14.0);
		...
		rmPlaceObjectDefAtLoc(mapTrees, 0, 0.5, 0.5, 25*PlayerNum);
...
//	  	rmAddObjectDefConstraint(bonusTrees, circleConstraint);
	...
	rmPlaceObjectDefInArea(bonusTrees, 0, continent2, 10*PlayerNum);
	...
	rmPlaceObjectDefInArea(desertTrees, 0, desertNW, 10*PlayerNum);
	rmPlaceObjectDefInArea(desertTrees, 0, desertSW, 10*PlayerNum);
	...
	//rmPlaceObjectDefAtLoc(BigBoys, 0, 0.5, 0.5, 3*PlayerNum);
	rmPlaceObjectDefInArea(BigBoys, 0, desertNW, 1.5*PlayerNum);
	rmPlaceObjectDefInArea(BigBoys, 0, desertSW, 1.5*PlayerNum);
...
//desertNW
	...
	rmPlaceObjectDefAtLoc(randomVultureTreeID, 0, 0.5, 0.5, 2.5*PlayerNum);
	...
	rmPlaceObjectDefAtLoc(buzzardFlockID, 0, 0.5, 0.5, 3*PlayerNum);
	...
	rmAddObjectDefConstraint(Props, avoidProps);
		rmPlaceObjectDefAtLoc(Props, 0, 0.5, 0.5, 30*PlayerNum);
