...
//	rmEnableLocalWater(false);
	...
	rmTerrainInitialize("california\ground_clifftop_cal", 6.0); // ground8_cal
	...
	int avoidPatch = rmCreateClassDistanceConstraint("patch avoid patch", classPatch, 10.0+2*PlayerNum);
	int avoidPatch2 = rmCreateClassDistanceConstraint("patch avoid patch2", classPatch2, 2.0+2*PlayerNum);
	...
	// VP avoidance
	int avoidTradeRoute = rmCreateTradeRouteDistanceConstraint("trade route", 5.0+0.2*PlayerNum);
	...
	if (isObserverMode == true)
	{
		...
		{ //1v1
			rmSetRandomSpawn1v1(0.68, 0.82,  0.32, 0.20);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3))
		{ // 2v2
			rmSetRandomSpawn2v2(0.81, 0.79, 0.62, 0.90,  0.38, 0.10, 0.19, 0.21);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
		{ // 3v3
			rmSetRandomSpawn3v3(0.91, 0.60, 0.58, 0.91, 0.79, 0.81,  0.42, 0.87, 0.21, 0.19, 0.93, 0.40);
		}
	}
	else
	{
	...
	if (TeamNum <= 2)
	{
		...
		rmSetAreaSmoothDistance(NEareaID, 3+1*PlayerNum);
	...
	//	rmSetAreaElevationPersistence(NEareaID, 0.4);
		...
		rmSetAreaCliffEdge(NEareaID, 1, 0.20-0.008*PlayerNum, 0.0, 0.0, 1); 
		rmSetAreaCliffType(NEareaID, "california"); 
	//	rmSetAreaMix(NEareaID, "california_desert2");
		rmSetAreaTerrainType(NEareaID, "california\desert4_cal");	
		rmAddAreaTerrainLayer(NEareaID, "california\desert6_cal", 0, 2);
		rmAddAreaTerrainLayer(NEareaID, "california\desert5_cal", 3, 6);
	...
	//	rmSetAreaTerrainType(NEareaID, "new_england\ground2_cliff_ne"); // for testing	
		...
		rmSetAreaSmoothDistance(SWareaID, 3+1*PlayerNum);
	...
	//	rmSetAreaElevationPersistence(SWareaID, 0.4);
		...
		rmSetAreaCliffEdge(SWareaID, 1, 0.20-0.008*PlayerNum, 0.0, 0.0, 1); 
		rmSetAreaCliffType(SWareaID, "california"); 
	//	rmSetAreaMix(SWareaID, "california_desert2");
		rmSetAreaTerrainType(SWareaID, "california\desert4_cal");	
		rmAddAreaTerrainLayer(SWareaID, "california\desert6_cal", 0, 2);
		rmAddAreaTerrainLayer(SWareaID, "california\desert5_cal", 3, 6);
	...
	else
	{
		...
		rmSetAreaSmoothDistance(FFAareaID, 3+1*PlayerNum);
	...
	//	rmSetAreaElevationPersistence(FFAareaID, 0.4);
		...
		rmSetAreaCliffEdge(FFAareaID, 1, 0.26-0.008*PlayerNum, 0.0, 0.0, 1); 
		rmSetAreaCliffType(FFAareaID, "california"); 
	//	rmSetAreaMix(FFAareaID, "california_desert2");
		rmSetAreaTerrainType(FFAareaID, "california\desert4_cal");	
		rmAddAreaTerrainLayer(FFAareaID, "california\desert6_cal", 0, 2);
		rmAddAreaTerrainLayer(FFAareaID, "california\desert5_cal", 3, 6);
		...
		rmSetAreaObeyWorldCircleConstraint(FFAareaID, true);
	...
	// Desert patches
	int desertpatchcount = 8+5*PlayerNum;
	if (TeamNum > 2)
		desertpatchcount = 8+2*PlayerNum;
	...
	for (i=0; < desertpatchcount)
    {
		...
		rmSetAreaTerrainType(patch2ID, "california\desert3_cal");
...
//		rmAddAreaConstraint(patch2ID, avoidImpassableLandMin);
	...
	for (i=0; < 10+6*PlayerNum)
    {
		...
		rmSetAreaTerrainType(patchID, "california\ground_clifftop_cal");
...
//	{	
		...
		for (i = 0;  < cliffcount)
		{
			...
			rmSetAreaSize(CliffID, 0.013-0.0005*PlayerNum, 0.013-0.0005*PlayerNum);
			rmSetAreaWarnFailure(CliffID, false);
			rmSetAreaObeyWorldCircleConstraint(CliffID, false);
			rmSetAreaCoherence(CliffID, 0.50);
			rmSetAreaSmoothDistance(CliffID, 3+1*PlayerNum);
			rmSetAreaCliffType(CliffID, "californiacoast");
			rmSetAreaTerrainType(CliffID, "california\ground_clifftop_cal");
...
//			rmBuildArea(natArea2);
	...
	rmAddObjectDefItem(playerHuntID, "Elk", rmRandInt(5,5), 3.0);
	rmSetObjectDefMinDistance(playerHuntID, 10.0);
	rmSetObjectDefMaxDistance(playerHuntID, 12.0);
    ...
    rmSetObjectDefMinDistance(playerHunt2ID, 37);
    rmSetObjectDefMaxDistance(playerHunt2ID, 38);
...
//	rmAddObjectDefToClass(playerNuggetID, classStartingResource);
	...
	// ********** Mines ***********
		...
		int goldmineCount = 1*PlayerNum;  // 3,3 		
		int silvermineCount = 1+2*PlayerNum;  // 3,3 
	...
	for(i=0; < goldmineCount)
	{
		...
		if (TeamNum <= 2)
			rmSetObjectDefMaxDistance(goldmineID, rmXFractionToMeters(0.0175*PlayerNum));
		else
			rmSetObjectDefMaxDistance(goldmineID, rmXFractionToMeters(0.0250*PlayerNum));
	...
	// Forest
	int forestcount = 4+7*PlayerNum; // 14*PlayerNum/2
	int stayInForest = -1;
...
//		rmAddAreaConstraint(forestID, avoidEdge);
		...
		for (j=0; < rmRandInt(6,7)+PlayerNum) //11,12
		{
	...
	// Random trees
	int randomforestcount = 3+6*PlayerNum; 
...
//		rmAddObjectDefConstraint(RandomtreeID, avoidEdge);
	...
	//Bighorn hunts
	int Bighorncount = 1+1*PlayerNum;
	
	for(i=0; < Bighorncount)
	{
		int BighornID = rmCreateObjectDef("Bighorn hunt"+i);
		rmAddObjectDefItem(BighornID, "bighornsheep", rmRandInt(11,11), 8.0);
	...
	//Elk hunts
	int Elkcount = 1+3*PlayerNum;
	
	for(i=0; < Elkcount)
	{
		int ElkID = rmCreateObjectDef("Elk hunt"+i);
		rmAddObjectDefItem(ElkID, "Elk", rmRandInt(6,6), 4.0);
	...
	//Deer hunts
	int Deercount = 2*PlayerNum;
	
	for(i=0; < Deercount)
	{
		int DeerID = rmCreateObjectDef("Deer hunt"+i);
		rmAddObjectDefItem(DeerID, "deer", rmRandInt(8,8), 5.0);
	...
	int nugget1count = 4+0.5*PlayerNum-nugget0count;
	int nugget2count = 3+0.5*PlayerNum; 
