...
// ------------------------------------------------------ Contraints ---------------------------------------------------------------------------
	...
	int classNugget = rmDefineClass("nuggets");
...
// X marks the spot
   ...
   int nuggetPlayerConstraint=rmCreateClassDistanceConstraint("nuggets stay away from players a lot", classStartingUnit, 50.0);
   ...
   int patchConstraint=rmCreateClassDistanceConstraint("patch vs. patch", classPatch, 5.0);

   // Unit avoidance - for things that aren't in the starting resources.
   int avoidStartingUnits=rmCreateClassDistanceConstraint("objects avoid starting units", classStartingUnit, 30.0);
   int avoidStartingUnitsTree=rmCreateClassDistanceConstraint("objects avoid starting units1", classStartingUnit, 10.0);
   int avoidStartingUnitsSmall=rmCreateClassDistanceConstraint("objects avoid starting units small", classStartingUnit, 5.0);
   int avoidTownCenterFar=rmCreateTypeDistanceConstraint("avoid Town Center Far", "townCenter", 58.0);
   int avoidTownCenterFar1=rmCreateTypeDistanceConstraint("avoid Town Center Far 1", "townCenter", 25.0);
   ...
   int avoidImportantItem=rmCreateClassDistanceConstraint("important stuff avoids each other", classImportantItem, 15.0);
   int avoidSocket=rmCreateClassDistanceConstraint("socket avoidance", classSocket, 6.0);
   int avoidSocketMore=rmCreateClassDistanceConstraint("bigger socket avoidance", classSocket, 10.0);
   ...
   int avoidCliffs1 = rmCreateClassDistanceConstraint("avoid Cliffs1", classCliff, 7.0);
   int avoidCliffsFar = rmCreateClassDistanceConstraint("avoid Cliffs far", classCliff, 15.0);
   int avoidPatch = rmCreateClassDistanceConstraint("avoid patch", classPatch, 2.0);
	
   // natives avoid natives
   int avoidNatives = rmCreateClassDistanceConstraint("avoid Natives", classNative, 10.0);
   int avoidNativesWood = rmCreateClassDistanceConstraint("avoid Natives wood", classNative, 6.0);
   int avoidNativesNuggets = rmCreateClassDistanceConstraint("nuggets avoid Natives", classNative, 20.0);
...
// ------------------------------------------------------ Cliffs ----------------------------------------------------------------------------------
   ...
   rmSetAreaCoherence(areaCliff1, 0.85);
   ...
   rmSetAreaObeyWorldCircleConstraint(areaCliff1, false);
   	rmAddAreaToClass(areaCliff1, classPatch);
   rmSetAreaTerrainType(areaCliff1, "texas\ground1_tex");
   rmSetAreaMix(areaCliff1, "texas_grass");
   ...
   rmSetAreaCoherence(areaCliff2, 0.85);
   ...
   rmAddAreaToClass(areaCliff2, classPatch);
   rmSetAreaTerrainType(areaCliff2, "texas\ground1_tex");
   rmSetAreaMix(areaCliff2, "texas_grass");
   ...
   rmAddAreaToClass(areaCliff3, classPatch);
   rmSetAreaTerrainType(areaCliff3, "texas\ground1_tex");
   rmSetAreaMix(areaCliff3, "texas_grass");
   rmBuildArea(areaCliff3);
	...
	rmAddAreaToClass(cliffIDL, classCliff);
	...
	rmAddAreaToClass(cliffIDR, classCliff);
   ...
   rmSetAreaMix(PatchC, "texas_grass");
   rmBuildArea(PatchC);
   ...
   rmSetAreaMix(PatchB, "texas_grass");
   rmBuildArea(PatchB);
   ...
   rmSetAreaMix(PatchA, "texas_grass");
   rmBuildArea(PatchA);
...
// ------------------------------------------------------ Player Location and Specific Area ---------------------------------------------------------------------------
    ...
    int VillageID =-1;
	VillageID = rmCreateGrouping("center location", "bonnie_springs_middle_city");
...
// ------------------------------------------------------ Starting Ressources ---------------------------------------------------------------------------
	...
	if (isObserverMode == true)
	{
		...
		{ //1v1
			rmSetRandomSpawn1v1(0.27,0.19,  0.73,0.81);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3))
		{ // 2v2
			rmSetRandomSpawn2v2(0.55,0.87,  0.79,0.74,  0.45,0.13,  0.21,0.26);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
		{ // 3v3
			rmSetRandomSpawn3v3(0.79,0.74,  0.64,0.85,  0.45,0.87,  0.55,0.13,  0.36,0.15,  0.21,0.26);
		}
	}
	else
	{
	...
	rmAddObjectDefToClass(playerNuggetID, classNugget);
...
// ***************************** Herds ********************************	
	...
	int numberOfHunts=PlayerNum*2+1;
	int IbexPlacement=0;
...
//	---------------------------------------------------- Nuggets ----------------------------------------------------------------------------------------
		...
		rmAddObjectDefToClass(nugget3, classNugget);
		...
		rmPlaceObjectDefAtLoc(nugget3, 0, 0.5, 0.5, 2*PlayerNum);
		...
		rmAddObjectDefToClass(nugget4, classNugget);
		...
		rmAddObjectDefConstraint(nugget4, avoidCoinShort);
		rmPlaceObjectDefAtLoc(nugget4, 0, 0.5, 0.5, 3*PlayerNum);
