...
string oceanType = "Texas Pond";
string groundType = "great_plains\ground5_gp";
string groundIsland = "sonora\ground2_son";
...
rmSetAreaSize(centralPond, 0.05, 0.05);
rmSetAreaLocation(centralPond, 0.5, 0.5);
rmAddAreaInfluenceSegment(centralPond, 0.4, 0.6, 0.6, 0.4);
...
int avoid_pond =rmCreateClassDistanceConstraint("avoid center pond", rmClassID("pond"), 5.0);
...
{  
  ...
  IncasVillageAID = rmCreateGrouping("incan village A", "native inca village "+4);
...
{   
  ...
  LakotaVillageAID = rmCreateGrouping("Lakota village A", "native Lakota village "+2);
...
{   
  ...
  LakotaVillageID = rmCreateGrouping("Lakota village", "native Lakota village "+2);
...
{   
  ...
  IncasVillageBID = rmCreateGrouping("incan village B", "native inca village "+4);
...
if (isObserverMode == true)
{
	...
	{ // 2v2
		rmSetRandomSpawn2v2(0.5,0.9,  0.9,0.5,  0.5,0.1,  0.1,0.5);
	}
	else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
	{ // 3v3
		rmSetRandomSpawn3v3(0.5,0.9,  0.9,0.5,  0.75,0.75,  0.25,0.25,  0.5,0.1,  0.1,0.5);
	}
}
else
{
...
rmAddObjectDefItem(StartMoose, hunt2, rmRandInt(8,9), 9.0);
...
numTries=20*PlayerNum;
int failCount=0;


	int bisoncount = 4*PlayerNum/2;
	int pronghorncount = 1*PlayerNum/2;
	...
	int goldCount = 4*PlayerNum/2;  // 3,3 
...
rmPlaceObjectDefAtLoc(nuggetID1, 0, 0.5, 0.5, PlayerNum*4);
