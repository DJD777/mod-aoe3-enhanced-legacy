...
int stayEast = rmCreatePieConstraint("Stay East",0.53,0.47, rmXFractionToMeters(0.2),rmXFractionToMeters(0.5), rmDegreesToRadians(0),rmDegreesToRadians(180));
 ...
 // =============Player placement ======================= 
	...
	if (isObserverMode == true)
	{
		...
		{ //1v1
			rmSetRandomSpawn1v1(0.3, 0.3,  0.7, 0.7);
		}
		else if (rmGetPlayerTeam(2)!=rmGetPlayerTeam(3))
		{ // 2v2
			rmSetRandomSpawn2v2(0.2, 0.65, 0.2, 0.35,  0.8, 0.35, 0.8, 0.65);
		}
		else if (rmGetPlayerTeam(3)!=rmGetPlayerTeam(4))
		{ // 3v3
			rmSetRandomSpawn3v3(0.2, 0.65, 0.2, 0.35, 0.28, 0.5,  0.8, 0.35, 0.8, 0.65, 0.72, 0.5);
		}
	}
	else {
...
//spawnSwitch = 0.1;
 ...
 rmSetAreaBaseHeight(continent2, 0.0);
        ...
        rmSetAreaTerrainType(patchID, "amazon\cliff_top1_ama");
...
*/
	...
	socketLoc2 = rmGetTradeRouteWayPoint(tradeRouteID, 0.9);
        ...
        rmSetAreaWaterType(oceanNW, "yucatan Coast");
...
//team koth
	...
	if (PlayerNum >= 3 && TeamNum == 2){

int kingSwitch = rmRandInt(1,2);
...
if (kingSwitch == 2){
	yLoc = 0.04;
		...
		ypKingsHillLandfill(xLoc, yLoc, 0.0048, 0.24, "amazon grass", 0);
...
//================ temple placement ========
    ...
    int southTemple = rmCreateArea("temple placement point S");
        rmSetAreaSize(southTemple, .007, .008);
//	   rmSetAreaMix(southTemple, "borneo_grass_a");
        rmSetAreaLocation(southTemple, 0.21, 0.21);
        rmSetAreaCoherence(southTemple, 0.9);
        rmBuildArea(southTemple);

    int northTemple = rmCreateArea("temple placement point N");
        rmSetAreaSize(northTemple, .007, .008);
//	   rmSetAreaMix(northTemple, "borneo_grass_a");
        rmSetAreaLocation(northTemple, 0.79, 0.79);
        rmSetAreaCoherence(northTemple, 0.9);
        rmBuildArea(northTemple);

		int playerTemple = rmCreateObjectDef("aztec temple");
         rmAddObjectDefItem(playerTemple, "SPCAztecTemple", 1, 3.0);
         rmSetObjectDefMinDistance(playerTemple, 0);
         rmSetObjectDefMaxDistance(playerTemple, 1);

	if (PlayerNum == 2)
{
    for(i=1; < PlayerNum + 1) {

    int tempLoc = rmFindCloserArea(rmPlayerLocXFraction(i), rmPlayerLocZFraction(i), southTemple, northTemple);


       rmPlaceObjectDefAtAreaLoc(playerTemple, i, tempLoc, 1);
} 

	}else{


         rmSetObjectDefMinDistance(playerTemple, 26);
         rmSetObjectDefMaxDistance(playerTemple, 27);
	rmAddObjectDefConstraint(playerTemple, avoidWaterShort);
	rmAddObjectDefConstraint(playerTemple, circleConstraint);

    for(i=1; < PlayerNum + 1) {

        rmPlaceObjectDefAtLoc(playerTemple, i, rmPlayerLocXFraction(i), rmPlayerLocZFraction(i));
}
} 
...
// Regicide Triggers
    ...
    rmSetTriggerConditionParamInt("SrcObject", rmGetUnitPlacedOfPlayer(playerTemple, i), false);
...
//Aztecs are awesome
   ...
   if (rmAllocateSubCivs(4) == true)
   {
	...
	nativeID0 = rmCreateGrouping("native site 1", "native aztec village 1");
	...
	nativeID1 = rmCreateGrouping("native site 2", "native aztec village 2");
...
//========place=====
	...
	if (TeamNum == 2){
	rmPlaceGroupingAtLoc(nativeID1, 0, 0.3, 0.8);	rmPlaceGroupingAtLoc(nativeID0, 0, 0.7, 0.2);
	if (PlayerNum >= 7)
{
	rmPlaceGroupingAtLoc(nativeID1, 0, 0.43, 0.63);
...
//=========2 player fixed mines============
	...
	rmAddObjectDefConstraint(foodID2, avoidTradeRouteSmall);
        ...
        rmAddObjectDefItem(foodID3, "turkey", 8, 8.0);
        rmSetObjectDefMinDistance(foodID3, 59.0);
...
//	rmAddObjectDefConstraint(capybaraWater, avoidTownCenter);
	rmAddObjectDefConstraint(capybaraWater, avoidHunt);		rmAddObjectDefConstraint(capybaraWater, waterHunt);
...
//	rmAddObjectDefConstraint(capybaraWater, avoidWaterShort);	
	...
	rmAddObjectDefConstraint(capybaraWater, avoidTemple);		
	rmPlaceObjectDefAtLoc(capybaraWater, 0, 0.5, 0.5, 7*PlayerNum);


    int elkHunts = rmCreateObjectDef("elkHunts");
	rmAddObjectDefItem(elkHunts, "turkey", rmRandInt(6,7), 8.0);
	...
	rmAddObjectDefConstraint(elkHunts, avoidHunt);	//rmAddObjectDefConstraint(elkHunts, waterHunt);
	...
	rmAddObjectDefConstraint(elkHunts, avoidTemple);	
	rmPlaceObjectDefAtLoc(elkHunts, 0, 0.5, 0.5, 7*PlayerNum);
...
//	rmAddObjectDefConstraint(nuggetID, circleConstraint);
	...
	rmPlaceObjectDefAtLoc(nuggetID, 0, 0.5, 0.5, 2*PlayerNum);   
//weaker nuggets
	...
	rmSetStatusText("",0.7);
		...
		rmAddObjectDefItem(mapTrees, "TreeYucatan", rmRandInt(12,13), rmRandFloat(10.0,12.0));
		rmAddObjectDefItem(mapTrees, "UnderbrushYucatan", rmRandInt(2,3), rmRandFloat(20.0,24.0));
	...
	rmAddObjectDefConstraint(mapTrees, circleConstraint);
		rmPlaceObjectDefAtLoc(mapTrees, 0, 0.5, 0.5, 60*PlayerNum);
		...
		rmAddObjectDefItem(bonusTrees, "OpenbrushAmazon", rmRandInt(4,5), rmRandFloat(8.0,9.0));
...
//		rmPlaceObjectDefAtLoc(bonusTrees, 0, 0.5, 0.5, 16*PlayerNum);
...
rmPlaceObjectDefInArea(fishID, 0, oceanNW, 6*PlayerNum);
rmPlaceObjectDefInArea(fishID, 0, oceanSW, 6*PlayerNum);
rmPlaceObjectDefInArea(fishID, 0, oceanNE, 6*PlayerNum);
rmPlaceObjectDefInArea(fishID, 0, oceanSE, 6*PlayerNum);
